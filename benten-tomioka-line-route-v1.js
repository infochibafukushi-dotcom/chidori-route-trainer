// 弁天・富岡線（系統番号14・route-14）走行シミュレーション／停留所編集モジュール。
// 停留所順：京成バスナビ個別便通過時刻表で確認（evidence/route14-benten-tomioka-implementation-2026-07-26/official-stop-orders.json）。
...
(() => {
  ...
  const SYSTEM_RESOLVED_VERSIONS = {
    '14-maihama': '2026-07-26-benten-tomioka-maihama-v1',
    '14-chidori-garage': '2026-07-26-benten-tomioka-chidori-garage-v1',
    '14-shinurayasu-maihama': '2026-07-26-benten-tomioka-shinurayasu-maihama-v1',
    '14-shinurayasu-chidori': '2026-07-26-benten-tomioka-shinurayasu-chidori-v1',
  };
  ...
  const NAMES_14_SHINURAYASU_MAIHAMA = [
    "舞浜駅",
    "オリエンタルランド本社前",
    "運動公園",
    "舞浜三丁目",
  ...
  const NAMES_14_SHINURAYASU_CHIDORI = [
    "千鳥車庫",
    "運動公園",
    "舞浜三丁目",
    "見明川",
  ...
  const SYSTEM_DEFINITIONS = {
    '14-maihama': {
      key: '14-maihama', displayCode: DISPLAY_CODE, directionGroup: 'outbound',
      title: '舞浜駅行き',
      summary: '新浦安駅 → 順天堂病院前 → 中央公園 → 弁天中央 → 運動公園 → 舞浜駅',
      relationId: 18323926,
      timetableSymbol: '無印',
      naviCourse: '0008200274',
      names: NAMES_14_MAIHAMA,
    },
    '14-chidori-garage': {
      key: '14-chidori-garage', displayCode: DISPLAY_CODE, directionGroup: 'outbound',
      title: '千鳥車庫行き',
      summary: '新浦安駅 → 順天堂病院前 → 中央公園 → 弁天中央 → 運動公園 → 千鳥車庫',
      relationId: 18419877,
      timetableSymbol: 'ち',
      naviCourse: '0008200276',
      names: NAMES_14_CHIDORI_GARAGE,
    },
    '14-shinurayasu-maihama': {
      key: '14-shinurayasu-maihama', displayCode: DISPLAY_CODE, directionGroup: 'inbound',
      title: '新浦安駅行き（舞浜駅発）',
      summary: '舞浜駅 → 運動公園 → 弁天中央 → 中央公園 → 順天堂病院前 → 新浦安駅',
      relationId: 9983017,
      timetableSymbol: '無印',
      naviCourse: '0008200273',
      names: NAMES_14_SHINURAYASU_MAIHAMA,
    },
    '14-shinurayasu-chidori': {
      key: '14-shinurayasu-chidori', displayCode: DISPLAY_CODE, directionGroup: 'inbound',
      title: '新浦安駅行き（千鳥車庫発）',
      summary: '千鳥車庫 → 運動公園 → 弁天中央 → 中央公園 → 順天堂病院前 → 新浦安駅',
      relationId: 18419876,
      timetableSymbol: 'し',
      naviCourse: '0008200275',
      names: NAMES_14_SHINURAYASU_CHIDORI,
    },
  };
  ...
  function platformsForSystem(systemKey) {
    const bank = window.BENTEN_TOMIOKA_LINE_PLATFORMS_V1 || {};
    return bank[systemKey] || {};
  }

  function pathDataForSystem(systemKey) {
    const bank = window.BENTEN_TOMIOKA_LINE_PATH_V1 || {};
    return bank[systemKey] || null;
  }

  /** 系統キー単位の停留所IDへ揃える。 */
  function migrateStopId(definition, stop, index) {
    const newId = `benten-tomioka-${definition.key}-${String(index + 1).padStart(2, '0')}`;
  ...
  function makeStop(definition, name, index) {
    ...
    return {
      id: `benten-tomioka-${definition.key}-${String(index + 1).padStart(2, '0')}`,
      name,
  ...
  function ensureRoute() {
    ...
    const systems = {};
    let changed = route.bentenTomiokaLineVersion !== VERSION;
    ...
    route.bentenTomiokaLineVersion = VERSION;
    ...
    if (!route.bentenTomiokaLineStopImages) {
      route.bentenTomiokaLineStopImages = {};
      changed = true;
    }
  ...
  function pathIntegrityFail(system, definition, reasons) {
    const policy = window.BENTEN_TOMIOKA_LINE_PATH_POLICY_V1;
  ...
  async function resolveSystem(key, { force = false, statusCallback = null } = {}) {
    ...
    if (!route || !definition || !system) throw new Error('弁天・富岡線の系統データがありません。');
  ...
  function migrateStopImageBank(route) {
    const bank = route.bentenTomiokaLineStopImages;
  ...
  function showStopImage(route, definition, stop, street) {
    ...
    const existing = street.querySelector('.stop-image-display-v25.bentenTomiokaLine-stop-image');
    existing?.remove();
    if (!entry?.dataUrl) return false;
    const overlay = document.createElement('div');
    overlay.className = 'stop-image-display-v25 bentenTomiokaLine-stop-image';
  ...
  function hideStopImage(street) {
    street?.querySelector('.bentenTomiokaLine-stop-image')?.remove();
  }
  ...
  async function drawGuidance(route, definition, system, token) {
    ...
    try {
      ...
      const message = base.includes('弁天・富岡線の走行データを確認できません')
        ? base
        : [
          '弁天・富岡線の走行データを確認できません。',
          `系統：${definition.key}`,
    ...
    const vehicle = new googleApi.maps.Marker({ map, position: center, icon: busIcon(googleApi), zIndex: 1000, title: `弁天・富岡線 ${definition.title}` });
    ...
    const publishDebugState = () => {
      window.BENTEN_TOMIOKA_LINE_DRIVE_STATE = {
        ...simulation,
        ...
        try {
          streetViewService.getPanorama(request, (data, status) => {
            try {
              ...
              console.warn('[bentenTomiokaLine] panorama update failed', error);
            }
          });
        } catch (error) {
          console.warn('[bentenTomiokaLine] getPanorama failed', error);
          applyPovOnly();
        }
        return true;
      }
    ...
    const updateDrivingVisual = (now, force = false) => {
      ...
      if (!point?.position || !Number.isFinite(point.position.lat) || !Number.isFinite(point.position.lng)) {
        ...
        console.error('[bentenTomiokaLine] invalid drive position', point, simulation);
        return;
      }
    ...
    const failStart = (reason) => {
      ...
      console.error('[bentenTomiokaLine] drive start failed', reason, {
        systemKey: definition.key,
        pathLength: path.length,
        metricsTotal: metrics.total,
    ...
    status.textContent = `弁天・富岡線｜系統${DISPLAY_CODE}｜${definition.title}｜${stops.length}停留所｜道路形状：OSM relation採用（要走行確認）`;
    publishDebugState();
    cleanupGuidance = () => {
      simulation.running = false;
      simulation.paused = false;
      ...
      telop.remove();
      hideStopImage(street);
      if (window.BENTEN_TOMIOKA_LINE_DRIVE_STATE?.systemKey === definition.key) {
        window.BENTEN_TOMIOKA_LINE_DRIVE_STATE = null;
      }
    };
  }

  routes = function routesBentenTomiokaLineV1() {
    const route = ensureRoute();
    ...
    try { window.HOKUEI_GUIDANCE_V22?.cleanup?.(); } catch (error) {
      console.warn('[bentenTomiokaLine] hokuei cleanup failed', error);
    }
    ...
    shell(`<section class="guidance-page-v26">
      ...
      <div class="split guidance-v22-split">
        <div class="guidance-map-wrap-v22"><div id="routeMap" class="map guidance-map-v22"></div><div class="guidance-version-v22">弁天・富岡線・系統${DISPLAY_CODE}</div></div>
        <div id="street" class="street guidance-street-v22"></div>
      </div>
      <p id="mapStatus" class="status" hidden aria-hidden="true">弁天・富岡線を準備しています…</p>
    </section>`);
    ...
    drawGuidance(route, definition, system, token).catch((error) => {
      ...
      if (node) {
        ...
        node.textContent = message.includes('弁天・富岡線の走行データを確認できません') || message.includes('系統')
          ? message
          : (`系統=${key} | ${message}`);
      }
      console.error('[bentenTomiokaLine] drawGuidance failed', definition.key, error);
    });
  };
  ...
  function openRoute14StopEditor(systemKey, stopIndex) {
    closeEditorDialog();
    const route = ensureRoute();
    ...
    const currentImage = route.bentenTomiokaLineStopImages?.[bankKey]?.dataUrl || '';
    ...
    dialog.innerHTML = `<section class="stop-edit-dialog" role="dialog" aria-modal="true">
      <div class="stop-edit-header"><div><h2>弁天・富岡線 停留所設定</h2><p>${DISPLAY_CODE}｜${escHtml(definition.title)}・${stopIndex + 1} / ${system.stops.length}</p></div><button type="button" id="closeStopEditor" class="stop-edit-close">×</button></div>
    ...
    const saveCurrent = () => {
      ...
      if (!route.bentenTomiokaLineStopImages) route.bentenTomiokaLineStopImages = {};
      if (state.removeImage) delete route.bentenTomiokaLineStopImages[bankKey];
      else if (state.dataUrl) route.bentenTomiokaLineStopImages[bankKey] = { dataUrl: state.dataUrl, updatedAt: new Date().toISOString(), sourceSystem: systemKey, sourceStopId: stop.id };
      route.bentenTomiokaLineStopImageUpdatedAt = new Date().toISOString();
      save();
      return true;
    };
    ...
    document.getElementById('previousStopEdit').onclick = () => { if (!saveCurrent()) return; close(); openRoute14StopEditor(systemKey, stopIndex - 1); };
    document.getElementById('nextStopEdit').onclick = () => { if (!saveCurrent()) return; close(); openRoute14StopEditor(systemKey, stopIndex + 1); };
  ...
  function renderBentenTomiokaStopList() {
    const route = ensureRoute();
    ...
    box.querySelectorAll('[data-uhd-edit]').forEach((button) => { button.onclick = () => openRoute14StopEditor(definition.key, Number(button.dataset.uhdEdit)); });
  }

  function route14StopEditor() {
    ensureRoute();
    editorRouteId = ROUTE_ID;
  ...
  stopEditor = function stopEditorBentenTomiokaLineV1() {
