...
async function main() {
  ...
  try {
    ...
    await page.goto(`http://127.0.0.1:${PORT}/index.html?nocache=pathhash`, {
      waitUntil: 'domcontentloaded',
      timeout: 60000,
    });
    await page.waitForFunction(() => window.TOMIOKA_ROUTE_V1 && window.TOMIOKA_PATH_V1 && window.TOMIOKA_PATH_POLICY_V1, null, { timeout: 30000 });
    await page.waitForTimeout(800);

    for (const key of SYSTEMS) {
      const result = await page.evaluate(async (systemKey) => {
        const api = window.TOMIOKA_ROUTE_V1;
        api.setSelectedSystemKey(systemKey);
        try {
          const system = await api.resolveSystem(systemKey, { force: true });
          const bank = window.TOMIOKA_PATH_V1[systemKey];
    ...
    report.tamper = await page.evaluate(async () => {
      ...
      const bank = window.TOMIOKA_PATH_V1[key];
      ...
      const live = data.routes.find((r) => r.id === 'route-4')?.systems?.[key];
      ...
      while (Date.now() < deadline) {
        ...
        if (statusText.includes('pathHash不一致') || statusText.includes('富岡線の走行データを確認できません')) break;
      ...
      return {
        ...
        messageHasTomiokaBanner: String(threw || '').includes('富岡線の走行データを確認できません')
          || statusText.includes('富岡線の走行データを確認できません'),
        statusText,
        afterButton,
