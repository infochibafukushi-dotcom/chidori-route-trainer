'use strict';
/**
 * Continuous Start→end drive for route-5 (6 systems) + basic regression hashes.
 */
...
function startServer() {
  return new Promise((resolve, reject) => {
    const server = http.createServer((req, res) => {
      ...
      if (rel.startsWith('horie-route-v1.js')) {
...
async function readSys(page, key) {
  return page.evaluate((systemKey) => {
    const route = window.HORIE_ROUTE_V1?.ensureRoute?.();
    const sys = route?.systems?.[systemKey];
    const st = window.HORIE_DRIVE_STATE || {};
...
async function main() {
  ...
  try {
    ...
    await page.goto(`http://127.0.0.1:${PORT}/index.html?nocache=r5`, {
    ...
    await page.screenshot({ path: path.join(OUT, 'screenshots', 'ui-pc-route5.png') });
    await context.close();
    ...
    await spPage.goto(`http://127.0.0.1:${PORT}/index.html?nocache=r5sp`, { waitUntil: 'domcontentloaded' });
    ...
    await spPage.selectOption('#routeSelect', 'route-5');
    await spPage.waitForTimeout(800);
    fs.mkdirSync(path.join(OUT, 'screenshots'), { recursive: true });
    await spPage.screenshot({ path: path.join(OUT, 'screenshots', 'ui-sp390-route5.png') });
    await sp.close();
  } finally {
    await browser.close();
    server.close();
