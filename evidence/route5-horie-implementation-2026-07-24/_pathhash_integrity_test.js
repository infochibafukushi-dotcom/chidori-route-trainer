...
async function main() {
  ...
  try {
    ...
    await page.goto(`http://127.0.0.1:${PORT}/index.html?nocache=r5pathhash`, {
      waitUntil: 'domcontentloaded',
      timeout: 60000,
    });
    await page.waitForFunction(() => window.HORIE_ROUTE_V1 && window.HORIE_PATH_V1 && window.HORIE_PATH_POLICY_V1, null, { timeout: 30000 });
    await page.waitForTimeout(800);

    for (const key of SYSTEMS) {
      const result = await page.evaluate(async (systemKey) => {
        const api = window.HORIE_ROUTE_V1;
        api.setSelectedSystemKey(systemKey);
        try {
          const system = await api.resolveSystem(systemKey, { force: true });
          const bank = window.HORIE_PATH_V1[systemKey];
    ...
    report.tamper = await page.evaluate(async () => {
      ...
      const live = data.routes.find((r) => r.id === 'route-5')?.systems?.[key];
      const invalidAfterResolve = Boolean(live?.pathInvalid);
      const issuesAfterResolve = (live?.pathIssues || []).map((i) => i.message).join(' / ');
      routes();
      // wait until mapStatus shows failure (drawGuidance async)
      const deadline = Date.now() + 8000;
      let statusText = '';
      while (Date.now() < deadline) {
        statusText = document.getElementById('mapStatus')?.textContent || '';
        if (statusText.includes('pathHash不一致') || statusText.includes('堀江線の走行データを確認できません')) break;
      ...
      return {
        ...
        messageHasTomiokaBanner: String(threw || '').includes('堀江線の走行データを確認できません')
          || statusText.includes('堀江線の走行データを確認できません'),
        statusText,
        afterButton,
    ...
    const tamperOk = Boolean(
      ...
      && report.tamper?.messageHasTomiokaBanner
