'use strict';
/**
 * pathHash integrity for route-12 舞浜リゾート線: intact systems pass; in-memory mutation blocks driving.
...
async function main() {
  ...
  try {
    ...
    await page.goto(`http://127.0.0.1:${PORT}/index.html?nocache=r12pathhash`, {
      waitUntil: 'domcontentloaded',
      timeout: 60000,
    });
    await page.waitForFunction(
      () => window.MAIHAMA_RESORT_LINE_ROUTE_V1 && window.MAIHAMA_RESORT_LINE_PATH_V1 && window.MAIHAMA_RESORT_LINE_PATH_POLICY_V1,
      null,
      { timeout: 30000 },
    );
    ...
    for (const key of SYSTEMS) {
      const result = await page.evaluate(async (systemKey) => {
        const api = window.MAIHAMA_RESORT_LINE_ROUTE_V1;
        api.setSelectedSystemKey(systemKey);
        try {
          const system = await api.resolveSystem(systemKey, { force: true });
          const bank = window.MAIHAMA_RESORT_LINE_PATH_V1[systemKey];
    ...
    report.tamper = await page.evaluate(async () => {
      ...
      const live = data.routes.find((r) => r.id === 'route-12')?.systems?.[key];
      ...
      while (Date.now() < deadline) {
        ...
        if (statusText.includes('pathHash不一致') || statusText.includes('舞浜リゾート線の走行データを確認できません')) break;
      ...
      return {
        ...
        messageHasMaihamaResortBanner: String(threw || '').includes('舞浜リゾート線の走行データを確認できません')
          || statusText.includes('舞浜リゾート線の走行データを確認できません'),
        statusText,
        afterButton,
    ...
    const tamperOk = Boolean(
      ...
      && report.tamper?.messageHasMaihamaResortBanner
