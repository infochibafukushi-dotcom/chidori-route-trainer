'use strict';
/**
 * Geometry / intersection audit for route-12 舞浜リゾート線 path banks.
...
const BUILD_SUMMARY = JSON.parse(fs.readFileSync(path.join(OUT_DIR, '_build_summary.json'), 'utf8'));
...
const VISUAL_Z19_BY_SYSTEM = {
  '12-maihama-via-resort': [
    '12-maihama-urayasu-start-z19.png',
    '12-maihama-cityhall-entry-z19.png',
    '12-maihama-juntendo-z19.png',
    '12-maihama-undokoen-branch-z20.png',
    '12-maihama-tds-entry-z20.png',
    '12-maihama-tds-platform-z20.png',
    '12-maihama-tds-exit-z20.png',
    '12-maihama-bayside-z20.png',
    '12-maihama-hotel-south-z20.png',
    '12-maihama-hotel-north-z20.png',
    '12-maihama-maihama-end-z19.png',
  ],
  '12-urayasu-via-resort': [
    '12-urayasu-maihama-start-z19.png',
    '12-urayasu-hotel-north-z20.png',
    '12-urayasu-hotel-south-z20.png',
    '12-urayasu-bayside-z20.png',
    '12-urayasu-tds-platform-z20.png',
    '12-urayasu-undokoen-z19.png',
    '12-urayasu-cityhall-z19.png',
    '12-urayasu-urayasu-end-z19.png',
  ],
};
...
const PATH_BANK = loadWindow('maihama-resort-line-path-v1.js').MAIHAMA_RESORT_LINE_PATH_V1;
      ...
      const req = https.request(
        {
          ...
          headers: {
            ...
            'User-Agent': 'chidori-route-trainer/route9-geometry-audit (local evidence; contact: github.com/infochibafukushi-dotcom/chidori-route-trainer)',
...
function countPathFeatureIntersections(pathPoints, features) {
  const hits = new Set();
...
function classifyWayTags(used) {
  ...
  for (const w of used) {
    ...
    const restricted = tags.access === 'no' || tags.access === 'private' || tags.motor_vehicle === 'no';
    const busOk = tags.bus === 'yes' || tags.bus === 'designated' || tags.psv === 'yes' || tags.psv === 'designated';
    // TDR service roads are access=no + bus/psv=yes; do not count as wrong-way.
    if (restricted && !busOk) wrongWayCount += 1;
    if (samples.length < 8) {
      samples.push({
        wayId: w.wayId,
        highway: hw || null,
  ...
  return { pedestrianWayCount, wrongWayCount, usedWayCount: used.length, wayTagSamples: samples };
}

function visualZ19Status(systemKey) {
  const files = VISUAL_Z19_BY_SYSTEM[systemKey] || [];
  const existing = files.filter((f) => fs.existsSync(path.join(SCREENSHOT_DIR, f)));
  return {
    files,
    existingCount: existing.length,
    missing: files.filter((f) => !fs.existsSync(path.join(SCREENSHOT_DIR, f))),
    buildingCrossingObserved: null,
    groundGreenCrossingObserved: null,
    note: 'Manual z19 review from screenshots only — not used as automated pass/fail counts',
  };
}
...
async function analyzeSystem(systemKey) {
  ...
  const row = {
    ...
    visualZ19: visualZ19Status(systemKey),
...
out body;`;
    ...
    row.buildingIntersectionCount = countPathFeatureIntersections(pathPoints, buildings);
    row.groundGreenIntersectionCount = countPathFeatureIntersections(pathPoints, groundGreen);
    row.bridgeGreenIntersectionCount = countPathFeatureIntersections(pathPoints, bridgeWays);
    row.overpassOk = true;
...
async function main() {
  ...
  const report = {
    ...
    route: 'route-12',
    lineName: '舞浜リゾート線',
    pathFiles: ['maihama-resort-line-path-v1.js', '_path_bank.json'],
    systems: {},
    pass: false,
  };
  ...
  const allOverpassOk = SYSTEMS.every((k) => report.systems[k].overpassOk);
  const geometryOk = SYSTEMS.every((k) => {
    const s = report.systems[k];
    return s.pointsFarFromWaysCount === 0
      && s.buildingIntersectionCount === 0
      && s.groundGreenIntersectionCount === 0
      && s.pedestrianWayCount === 0
      && s.wrongWayCount === 0;
  });
  // Self-intersections expected at TDS/hotel one-way loop (see screenshots *bayside* / *hotel* / *tds*).
  report.selfIntersectionPolicy = 'expected-at-tds-hotel-loop';
  report.selfIntersectionTotals = Object.fromEntries(SYSTEMS.map((k) => [k, report.systems[k].selfIntersectionCount]));
  report.pass = allOverpassOk && geometryOk;
  report.anyReviewRequired = SYSTEMS.some((k) => report.systems[k].reviewRequired);
  report.allOverpassOk = allOverpassOk;

  for (const [k, row] of Object.entries(report.systems || {})) {
  const acc = BUILD_SUMMARY.systems[k]?.access || {};
  row.restrictedAccessWayCount = acc.restrictedAccessWayCount ?? null;
  row.busPermittedRestrictedWayCount = acc.busPermittedRestrictedWayCount ?? null;
  row.unresolvedRestrictedWayCount = acc.unresolvedRestrictedWayCount ?? null;
  row.reviewRequired = Boolean(BUILD_SUMMARY.systems[k]?.reviewRequired);
  row.platformDistException = BUILD_SUMMARY.systems[k]?.bayside || null;
}
fs.writeFileSync(OUT, JSON.stringify(report, null, 2));
  console.log('wrote', OUT);
  console.log(JSON.stringify({ pass: report.pass, allOverpassOk, anyReviewRequired: report.anyReviewRequired }, null, 2));
