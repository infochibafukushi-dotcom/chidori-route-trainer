'use strict';
/**
 * PWA offline report for route-12 + existing routes.
...
async function main() {
  ...
  const report = {
    ...
    cacheName,
    expectCache: 'chidori-route-map-v74',
    cacheOk: cacheName === 'chidori-route-map-v74',
    missingInIndex,
    missingInShell,
    shellNotInIndex,
    indexNotInShell,
  ...
  try {
    ...
    await page.goto(`http://127.0.0.1:${PORT}/index.html?nocache=r12pwa`, {
      waitUntil: 'domcontentloaded',
      timeout: 60000,
    });
    await page.waitForFunction(
      () => window.MAIHAMA_RESORT_LINE_ROUTE_V1 && window.MAIHAMA_LINE_ROUTE_V1 && window.MAIHAMA_RESORT_LINE_PATH_V1,
      null,
      { timeout: 30000 },
    );
    ...
    const online = await page.evaluate(async () => {
      ...
      const r11 = routesList.find((r) => r.id === 'route-12');
      let swState = null;
      try {
      ...
      return {
        routeIds: ids,
        hasRoute10: ids.includes('route-12'),
        ...
        swState,
        takasuApi: Boolean(window.MAIHAMA_RESORT_LINE_ROUTE_V1),
      };
    });
    ...
    try {
      ...
      offlineDetail = await page.evaluate(() => ({
        hasTakasu: Boolean(window.MAIHAMA_RESORT_LINE_ROUTE_V1),
