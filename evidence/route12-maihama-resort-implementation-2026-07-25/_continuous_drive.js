'use strict';
/**
 * Continuous Start→end drive for route-12 舞浜リゾート線 (6 systems).
 */
...
function startServer() {
  return new Promise((resolve, reject) => {
    const server = http.createServer((req, res) => {
      ...
      if (rel.startsWith('maihama-resort-line-route-v1.js')) {
...
async function readSys(page, key) {
  return page.evaluate((systemKey) => {
    ...
    const st = window.MAIHAMA_RESORT_LINE_DRIVE_STATE || {};
...
async function main() {
  ...
  try {
    ...
    await page.goto(`http://127.0.0.1:${PORT}/index.html?nocache=r12drive`, {
      waitUntil: 'domcontentloaded',
      timeout: 60000,
    });
    await page.waitForFunction(
      () => window.MAIHAMA_RESORT_LINE_ROUTE_V1 && window.MAIHAMA_RESORT_LINE_PATH_V1,
      null,
      { timeout: 30000 },
    );
