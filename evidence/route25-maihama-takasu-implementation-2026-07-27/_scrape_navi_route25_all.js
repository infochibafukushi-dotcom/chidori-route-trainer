'use strict';
/**
 * Keisei Bus Navi discovery scrape for 系統25 舞浜・高洲線 (route-25).
 * Gate 【２５系統】. Separate from 10/15/18/19 at shared berths.
 */
const fs = require('fs');
...
const ROUTE_NUM = '25';
const SIBLING_ROUTES = ['10', '15', '18', '19', '3', '23', '9', '11', '22'];
...
const KNOWN_IDS = {
  maihama: '00020617',
  sogo: '00020745',
  takasu_seaside: '00020855',
};

const SEARCH_WORDS = [
  '舞浜駅', '総合公園', '高洲海浜公園', '高洲西児童公園', '潮音の街',
  '今川消防署', '順天堂大学浦安キャンパス', '海園の街',
];
...
function emptyReport() {
  return {
    ...
    lineName: '舞浜・高洲線',
    note: 'Route 25 discovery. Gate 【２５系統】. Separate from 10/15/18/19 at shared berths.',
    knownIds: { ...KNOWN_IDS },
    searches: {},
    terminals: {},
    terminalPlan: [],
...
async function main() {
  ...
  try {
    ...
    const idByKey = {
      ...
      takasu_seaside: KNOWN_IDS.takasu_seaside || pick('高洲海浜公園', /高洲海浜公園/),
    };

    const terminals = [
      { key: 'maihama', label: '舞浜駅', id: idByKey.maihama },
      { key: 'sogo', label: '総合公園', id: idByKey.sogo },
      { key: 'takasu_seaside', label: '高洲海浜公園', id: idByKey.takasu_seaside },
    ].filter((t) => t.id);
    ...
    for (const term of terminals) {
      ...
      console.log('  route25:', courses.route25.length, '/ mixed:', courses.mixedWithSiblings?.length || 0);
      for (const c of courses.route25) {
    ...
    report.stats = {
      ...
      route25Courses: Object.values(report.terminals).reduce((n, t) => n + (t.route25?.length || 0), 0),
  ...
  console.log('=== ROUTE 25 COURSES ===');
  for (const [key, t] of Object.entries(report.terminals)) {
    for (const c of t.route25 || []) {
