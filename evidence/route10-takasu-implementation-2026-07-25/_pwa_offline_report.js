'use strict';
/**
 * PWA offline report for route-10 + existing routes.
...
async function main() {
  ...
  const missingInIndex = takasuAssets.filter((u) => !indexUrls.includes(u));
  const missingInShell = takasuAssets.filter((u) => !appShell.includes(u));
  const shellNotInIndex = appShell.filter((u) => u.startsWith('./takasu-') && !indexUrls.includes(u));
  const indexNotInShell = indexUrls.filter(
    (u) => (u.includes('takasu-') || u.includes('hokuei-no-uturn')) && !appShell.includes(u),
  );

  const report = {
    startedAt: new Date().toISOString(),
    cacheName,
    expectCache: 'chidori-route-map-v72',
    cacheOk: cacheName === 'chidori-route-map-v72',
    missingInIndex,
    missingInShell,
    shellNotInIndex,
    indexNotInShell,
  ...
  try {
    ...
    await page.goto(`http://127.0.0.1:${PORT}/index.html?nocache=r10pwa`, {
      waitUntil: 'domcontentloaded',
      timeout: 60000,
    });
    await page.waitForFunction(
      () => window.TAKASU_LINE_ROUTE_V1 && window.MAIHAMA_LINE_ROUTE_V1 && window.TAKASU_LINE_PATH_V1,
      null,
      { timeout: 30000 },
    );
    ...
    const online = await page.evaluate(async () => {
      ...
      const r10 = routesList.find((r) => r.id === 'route-10');
      let swState = null;
      try {
      ...
      return {
        routeIds: ids,
        hasRoute10: ids.includes('route-10'),
        hasRoute9: ids.includes('route-9'),
        hasRoute1: ids.includes('route-1'),
        route10Name: r10?.name || null,
        systems: r10?.systems ? Object.keys(r10.systems) : [],
        swState,
        takasuApi: Boolean(window.TAKASU_LINE_ROUTE_V1),
      };
    });
    ...
    try {
      ...
      offlineDetail = await page.evaluate(() => ({
        hasTakasu: Boolean(window.TAKASU_LINE_ROUTE_V1),
