'use strict';
/**
 * pathHash integrity for route-10 高洲線: intact systems pass; in-memory mutation blocks driving.
...
async function main() {
  ...
  try {
    ...
    await page.goto(`http://127.0.0.1:${PORT}/index.html?nocache=r10pathhash`, {
      waitUntil: 'domcontentloaded',
      timeout: 60000,
    });
    await page.waitForFunction(
      () => window.TAKASU_LINE_ROUTE_V1 && window.TAKASU_LINE_PATH_V1 && window.TAKASU_LINE_PATH_POLICY_V1,
      null,
      { timeout: 30000 },
    );
    ...
    report.tamper = await page.evaluate(async () => {
      ...
      const live = data.routes.find((r) => r.id === 'route-10')?.systems?.[key];
      ...
      while (Date.now() < deadline) {
        ...
        if (statusText.includes('pathHash不一致') || statusText.includes('高洲線の走行データを確認できません')) break;
      ...
      return {
        ...
        messageHasTakasuBanner: String(threw || '').includes('高洲線の走行データを確認できません')
          || statusText.includes('高洲線の走行データを確認できません'),
        statusText,
        afterButton,
    ...
    const tamperOk = Boolean(
      ...
      && report.tamper?.messageHasTakasuBanner
