'use strict';
/**
 * Continuous Start→end drive for route-10 高洲線 (6 systems).
 */
...
function startServer() {
  return new Promise((resolve, reject) => {
    const server = http.createServer((req, res) => {
      ...
      if (rel.startsWith('takasu-line-route-v1.js')) {
...
async function readSys(page, key) {
  return page.evaluate((systemKey) => {
    const route = window.TAKASU_LINE_ROUTE_V1?.ensureRoute?.();
    const sys = route?.systems?.[systemKey];
    const st = window.TAKASU_LINE_DRIVE_STATE || {};
...
async function main() {
  ...
  try {
    ...
    await page.goto(`http://127.0.0.1:${PORT}/index.html?nocache=r10drive`, {
      waitUntil: 'domcontentloaded',
      timeout: 60000,
    });
    await page.waitForFunction(
      () => window.TAKASU_LINE_ROUTE_V1 && window.TAKASU_LINE_PATH_V1,
      null,
      { timeout: 30000 },
    );
