'use strict';
/**
 * Keisei Bus Navi audit scrape for 系統15 潮音の街線 (route-15).
 ...
 *  - Require [15] / 【１５系統】 evidence on the trip page before accepting.
 *  - 新浦安駅のりばEのセルは [15] と [18] が混載される。ここでは候補として拾い、
...
const SEARCH_WORDS = ['新浦安駅', '高洲海浜公園', '高洲北小学校', '潮音の街', '東京学館', '高洲中央公園', '入船橋'];
...
function romajiKeyFromName(name) {
  ...
  if (/高洲海浜公園/.test(n)) return 'takasu-seaside';
  if (/高洲北小学校/.test(n)) return 'takasu-kita-shogakko';
  if (/潮音の街/.test(n)) return 'shione-no-machi';
  if (/東京学館/.test(n)) return 'tokyo-gakkan';
  if (/高洲中央公園/.test(n)) return 'takasu-chuo-koen';
  if (/入船橋/.test(n)) return 'irifunebashi';
...
async function listCourses(page, busstopId, label) {
  ...
  // deep pass reads `route15`
  return { label, busstopId, coursesUrl, title: pageInfo.title, heading: pageInfo.h, all, route15: routeMatches };
}
...
function emptyReport() {
  return {
    ...
    lineName: '潮音の街線',
    note: 'Route 15 discovery pass. 新浦安駅のりばEは[15]/[18]混載のため凡例ゲートを後段で必須とする。',
    knownIds: { ...KNOWN_IDS },
    searches: {},
    terminals: {},
    timetables: [],
...
async function main() {
  ...
  try {
    ...
    const terminals = [
      ...
      { key: 'takasuSeaside', label: '高洲海浜公園', id: pick('高洲海浜公園', /高洲海浜公園/) },
      { key: 'takasuKita', label: '高洲北小学校', id: pick('高洲北小学校', /高洲北小学校/) },
      { key: 'shioneNoMachi', label: '潮音の街', id: pick('潮音の街', /潮音の街/) },
      { key: 'tokyoGakkan', label: '東京学館', id: pick('東京学館', /東京学館/) },
    ].filter((t) => t.id);
    ...
    for (const term of terminals) {
      ...
      console.log('  route15 courses:', courses.route15.length, '/ all', courses.all.length);
      for (const c of courses.route15) {
      ...
      for (const course of courses.route15.slice(0, 12)) {
