'use strict';
/**
 * Local end-to-end validation for route-15 潮音の街線.
 *
 * Serves the repo, boots the real app with a Google Maps mock, selects route-15,
 ...
 *
 * Also asserts routes 12 and 14 still resolve after the loader change.
 */
...
const SYSTEMS = ['15-takasu-seaside', '15-shinurayasu'];
...
function startServer() {
  return new Promise((resolve, reject) => {
    const server = http.createServer((req, res) => {
      ...
      if (rel.startsWith('shione-no-machi-line-route-v1.js')) body = Buffer.from(speedUp(body.toString('utf8')), 'utf8');
      if (rel === 'index.html') {
...
async function readSys(page, key) {
  return page.evaluate((systemKey) => {
    const api = window.SHIONE_NO_MACHI_LINE_ROUTE_V1;
    const route = api?.ensureRoute?.();
    const sys = route?.systems?.[systemKey];
    const st = window.SHIONE_NO_MACHI_LINE_DRIVE_STATE || {};
...
async function main() {
  ...
  try {
    ...
    await page.goto(`http://127.0.0.1:${PORT}/index.html?nocache=r15`, { waitUntil: 'domcontentloaded', timeout: 60000 });
    await page.waitForTimeout(1200);
    await page.evaluate(() => { if (typeof go === 'function') go('routes'); });
    await page.waitForSelector('#routeSelect', { timeout: 15000 });

    const hasOption = await page.evaluate(() => !!document.querySelector('#routeSelect option[value="route-15"]'));
    report.route15OptionPresent = hasOption;
    if (!hasOption) report.failures.push('route-15 not present in #routeSelect');

    await page.selectOption('#routeSelect', 'route-15');
    await page.waitForFunction(() => window.SHIONE_NO_MACHI_LINE_ROUTE_V1 && window.SHIONE_NO_MACHI_LINE_PATH_V1 && window.SHIONE_NO_MACHI_LINE_PLATFORMS_V1 && window.SHIONE_NO_MACHI_LINE_PATH_POLICY_V1, null, { timeout: 30000 });
    await page.waitForSelector('#driveStartPause', { timeout: 25000 });
    await page.waitForTimeout(900);

    await page.evaluate(async () => {
      await window.SHIONE_NO_MACHI_LINE_ROUTE_V1.resolveAllSystems(true);
    });
    ...
    for (const key of SYSTEMS) {
      ...
      const runtime = await page.evaluate(async (k) => {
        ...
        const validation = policy.validateRuntimePath({
          ...
          expectedPathHash: window.SHIONE_NO_MACHI_LINE_PATH_V1[k].pathHash,
      ...
      const entry = {
        ...
        stopIdPrefixOk: runtime.stopIds.every((id) => String(id).startsWith(`shione-no-machi-${key}-`)),
      };
