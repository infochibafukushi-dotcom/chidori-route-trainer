'use strict';
/**
 * Deep pass: open EVERY 通過時刻表 (/stops?) link on the route-15 course timetables
 * across weekday / saturday / sunday, so short-turn variants (e.g. 高洲北小学校行)
 * cannot be missed by sampling.
 *
 * 新浦安駅のりばE のセルは [15] / [18] / [深夜] 混載なので、ここでは候補を広く拾い、
 * 系統確定は _verify_signatures.js の凡例ゲート（符号→【Ｎ系統】）に委ねる。
 *
 * Also discovers 高洲海浜公園 / 高洲北小学校 busstop ids from accepted trips and scans
 * their course lists for [15] departures (inbound 新浦安駅行).
 *
...
const FOLLOW_UP_STOPS = ['高洲海浜公園', '高洲北小学校', '潮音の街'];
...
const hasRouteMark = (t) => /\[15\]|【\s*15\s*系統\s*】/.test(toAsciiDigits(String(t || '')));
...
async function listCourses(page, busstopId) {
  ...
  return { busstopId, coursesUrl, ...info, all, route15: all.filter((l) => hasRouteMark(l.text)) };
}
...
const out = {
  ...
  lineName: '潮音の街線',
  note: 'Exhaustive per-course trip enumeration. Every /stops? link on each route-15 candidate timetable is opened.',
  courses: [],
  timetables: [],
  signatures: {},
  trips: [],
...
async function processTimetable(page, ttMeta) {
  ...
  for (let i = 0; i < links.length; i++) {
    ...
    if (!confirmed) {
      out.rejected.push({ url: link.absHref, cellText: link.cellText, reason: 'no-15-evidence', stopNames: raw.stopNames });
      continue;
    }
...
async function main() {
  ...
  try {
    ...
    for (const stopName of FOLLOW_UP_STOPS) {
      ...
      out.followUps[stopName] = { busstopId: id, courses: null, route15Courses: null, note: null };
      if (!id) {
      ...
      out.followUps[stopName].route15Courses = c.route15.map((x) => ({ text: x.text, berth: x.berth, absHref: x.absHref }));
      if (!c.route15.length) {
        out.followUps[stopName].note = `${stopName}発の[15]公開便コースは見つからなかった。`;
        continue;
      }
      for (const course of c.route15) {
        for (const day of DATES) {
          if (pastHardTimeout()) break;
          await processTimetable(page, {
