 ...
 *
 * (A) だけでは不十分。高洲北小学校のりば01/02の凡例は [10]/[15]/[19] 混載セルなのに
 * 符号が2つ（無印／み）しか無く、19系統の便まで「無印…【１５系統】」に吸われてしまう。
 * 出発のりば（新浦安駅のりばF＝【１９系統】、高洲海浜公園のりば19＝【１９系統】）まで
 * 遡って初めて系統が一意に決まる。
 *
 * ACCEPT 条件: (B) が 15 に解決し、(A) が 15 以外に解決していないこと。
 *
...
/** Parse legend lines of the form "無印…【１５系統】東京学館前経由　高洲海浜公園行き" */
...
async function main() {
  ...
  const result = {
    ...
    policy: '(A) 掲載時刻表の凡例 と (B) 出発のりばの時刻表の凡例 の二段で系統を判定する。'
      + '新浦安駅のりばEは[15]/[18]/深夜バス混載、高洲北小学校のりば01/02は[10]/[15]/[19]混載で凡例が不完全なため、'
      + '(B) 出発のりばの凡例（新浦安駅のりばE=15 / のりばF=19、高洲海浜公園のりば03=15 / のりば19=19）を決定打とする。',
    timetableLegends: {},
    signatures: [],
  };
  ...
  for (const [sigKey, sig] of Object.entries(DEEP.signatures)) {
    ...
    const depIs15 = depProof.resolved && depProof.routeNumber === ROUTE_NUM;
    const listingContradicts = listingRouteNumbers.length > 0 && !listingRouteNumbers.includes(ROUTE_NUM);
    let verdict;
    if (depIs15 && !listingContradicts) verdict = `ACCEPT-route${ROUTE_NUM}`;
