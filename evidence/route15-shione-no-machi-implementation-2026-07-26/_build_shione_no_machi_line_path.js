'use strict';
/**
 * Build platforms + pathPoints for route-15 潮音の街線 from OSM relations.
 *
 * Rules:
 *  - Stop order comes from official-stop-orders.json (navi). Never reversed/truncated.
 *  - Each system gets its OWN relation. 往路18419865 / 復路18419864 are built
 ...
 *  - route-10/18/19 の既存 path や停留所順は corridor が重なっても流用しない。
 *
 * Writes shione-no-machi-line-platforms-v1.js and shione-no-machi-line-path-v1.js at repo root,
...
const MAX_GAP_M = 30;
const MAX_JOIN_M = 1;
const GENERATED = '2026-07-26-shione-no-machi-v1';

/**
 * OSM ways that are access-restricted but legitimately driven by route 15.
...
const SYSTEMS = {
  '15-takasu-seaside': {
    ...
    note: 'outbound 新浦安駅→高洲海浜公園（無印＝【１５系統】東京学館前経由）。18系統(ゆ/た)の便やpathは流用禁止。',
  },
  '15-shinurayasu': {
    relationId: 18419864,
    resolvedVersion: '2026-07-26-shione-no-machi-shinurayasu-v1',
    pathSource: 'osm-relation-18419864+startHint-takasu-seaside',
    note: 'inbound 高洲海浜公園→新浦安駅。往路18419865の反転は禁止（中央分離帯道路で往復の車線が異なる）。',
  },
};
...
function main() {
  ...
  const summary = {
    ...
    generated: GENERATED,
    policy: {
      stopOrderSource: 'official-stop-orders.json（京成バスナビ個別便通過時刻表・凡例【１５系統】でゲート済み）',
      ...
      otherRouteReuseForbidden: 'route-10 / route-18 / route-19 の path・停留所順は corridor が重なっても流用しない',
      maxGap_m: MAX_GAP_M,
  ...
  summary.reverseChecks = [{ a: '15-takasu-seaside', b: '15-shinurayasu' }]
  ...
  fs.writeFileSync(
    path.join(ROOT, 'shione-no-machi-line-platforms-v1.js'),
    '// Auto-generated OSM platforms for 潮音の街線 (route-15).\n'
    + '// Official stop order: Keisei Bus Navi 個別便通過時刻表 2026-07-26（凡例 無印=【１５系統】でゲート）。\n'
    + '// 新浦安駅のりばEは[15]/[18]混載のため、18系統の便・停留所は採用していない。\n'
    + '// Each system uses the platforms of its own OSM relation; outbound/inbound platforms differ.\n'
    + `// Generated: ${GENERATED}\n`
    + `(() => {\n  window.SHIONE_NO_MACHI_LINE_PLATFORMS_V1 = ${JSON.stringify(platformsBank, null, 2)};\n})();\n`,
    'utf8',
  );
  fs.writeFileSync(
    path.join(ROOT, 'shione-no-machi-line-path-v1.js'),
    ...
    + `// Generated: ${GENERATED}\n`
    + `(() => {\n  window.SHIONE_NO_MACHI_LINE_PATH_V1 = ${JSON.stringify(pathBank, null, 2)};\n})();\n`,
    'utf8',
  );
  ...
  console.log('wrote shione-no-machi-line-platforms-v1.js and shione-no-machi-line-path-v1.js');
}
