'use strict';
/**
 * Geometry QA for route-16 paths.
 *
 * Checks, per system:
 ...
 *   - route-17 relations must not appear in the shipped bank
 */
const fs = require('fs');
...
function main() {
  const pathBank = loadShippedBank('hinode-line-path-v1.js', 'HINODE_LINE_PATH_V1');
  const platformsBank = loadShippedBank('hinode-line-platforms-v1.js', 'HINODE_LINE_PLATFORMS_V1');

  const report = {
    checkedAt: new Date().toISOString(),
    routeId: 'route-16',
    thresholds: { maxGap_m: MAX_GAP_M, uturnDeg: UTURN_DEG, platformSoft_m: PLATFORM_SOFT, platformHard_m: PLATFORM_HARD },
    route17RelationsUsed: [],
    systems: {},
    failures: [],
    reviewRequired: [],
  };
  ...
  for (const [key, bank] of Object.entries(pathBank)) {
    if (ROUTE17_RELATIONS.includes(bank.relationId)) report.route17RelationsUsed.push({ key, relationId: bank.relationId });
  ...
  if (report.route17RelationsUsed.length) {
    report.failures.push(`route-17 relations used: ${JSON.stringify(report.route17RelationsUsed)}`);
  }
