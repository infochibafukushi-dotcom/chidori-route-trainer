'use strict';
/**
 * Deep pass: open EVERY 通過時刻表 (/stops?) link on the route-16 course timetables
 * across weekday / saturday / sunday, so short-turn variants (e.g. ベイシティ浦安行)
 * cannot be missed by sampling.
 *
 * 新浦安駅のりばC のセルは [16] / [17] 混載（どちらも 日の出七丁目 行）なので、ここでは
 * 候補を広く拾い、系統確定は _verify_signatures.js の凡例ゲート（符号→【Ｎ系統】）に委ねる。
 *
 * Also discovers 日の出七丁目 / プラウド新浦安パークマリーナ / ベイシティ浦安 の busstop id を
 * 確定便から取得し、それぞれの course 一覧を [16] で走査して復路（新浦安駅行）を拾う。
 *
...
const hasRouteMark = (t) => /\[16\]|【\s*16\s*系統\s*】/.test(toAsciiDigits(String(t || '')));
...
async function listCourses(page, busstopId) {
  ...
  return { busstopId, coursesUrl, ...info, all, route16: all.filter((l) => hasRouteMark(l.text)) };
}
...
const out = {
  ...
  lineName: '日の出線',
  note: 'Exhaustive per-course trip enumeration. Every /stops? link on each route-16 candidate timetable is opened. 17系統（日の出東経由）は同じ日の出七丁目発着のため凡例ゲートで最終判定する。',
  courses: [],
  timetables: [],
  signatures: {},
  trips: [],
...
async function processTimetable(page, ttMeta) {
  ...
  for (let i = 0; i < links.length; i++) {
    ...
    if (!confirmed) {
      out.rejected.push({ url: link.absHref, cellText: link.cellText, reason: 'no-16-evidence', stopNames: raw.stopNames });
      continue;
    }
...
async function main() {
  ...
  try {
    ...
    for (const stopName of FOLLOW_UP_STOPS) {
      ...
      out.followUps[stopName] = { busstopId: id, courses: null, route16Courses: null, note: null };
      if (!id) {
      ...
      out.followUps[stopName].route16Courses = c.route16.map((x) => ({ text: x.text, berth: x.berth, absHref: x.absHref }));
      if (!c.route16.length) {
        out.followUps[stopName].note = `${stopName}発の[16]公開便コースは見つからなかった。`;
        continue;
      }
      for (const course of c.route16) {
        for (const day of DATES) {
          if (pastHardTimeout()) break;
          await processTimetable(page, {
