'use strict';
/**
 * Local end-to-end validation for route-16 日の出線.
 *
 * Serves the repo, boots the real app with a Google Maps mock, selects route-16,
 * then for each of the 2 systems:
 ...
 *   5. no route-17-only stop may appear
 *   6. continuous drive from 始発 to 終点 with no position jumps
 *
 * Also asserts routes 15, 14 and 12 still resolve after the loader change.
 */
...
const SYSTEMS = ['16-hinode-nanachome', '16-shinurayasu'];
...
function startServer() {
  return new Promise((resolve, reject) => {
    const server = http.createServer((req, res) => {
      ...
      if (rel.startsWith('hinode-line-route-v1.js')) body = Buffer.from(speedUp(body.toString('utf8')), 'utf8');
      if (rel === 'index.html') {
...
async function readSys(page, key) {
  return page.evaluate((systemKey) => {
    const api = window.HINODE_LINE_ROUTE_V1;
    const route = api?.ensureRoute?.();
    const sys = route?.systems?.[systemKey];
    const st = window.HINODE_LINE_DRIVE_STATE || {};
...
async function main() {
  ...
  try {
    ...
    await page.goto(`http://127.0.0.1:${PORT}/index.html?nocache=r16`, { waitUntil: 'domcontentloaded', timeout: 60000 });
    await page.waitForTimeout(1200);
    await page.evaluate(() => { if (typeof go === 'function') go('routes'); });
    await page.waitForSelector('#routeSelect', { timeout: 15000 });

    const hasOption = await page.evaluate(() => !!document.querySelector('#routeSelect option[value="route-16"]'));
    report.route16OptionPresent = hasOption;
    if (!hasOption) report.failures.push('route-16 not present in #routeSelect');

    await page.selectOption('#routeSelect', 'route-16');
    await page.waitForFunction(() => window.HINODE_LINE_ROUTE_V1 && window.HINODE_LINE_PATH_V1 && window.HINODE_LINE_PLATFORMS_V1 && window.HINODE_LINE_PATH_POLICY_V1, null, { timeout: 30000 });
    ...
    await page.evaluate(async () => {
      await window.HINODE_LINE_ROUTE_V1.resolveAllSystems(true);
    });
    ...
    for (const key of SYSTEMS) {
      ...
      const runtime = await page.evaluate(async (k) => {
        ...
        const validation = policy.validateRuntimePath({
          ...
          expectedPathHash: window.HINODE_LINE_PATH_V1[k].pathHash,
      ...
      const route17Leak = runtime.stopNames.filter((n) => ROUTE17_ONLY_STOPS.some((x) => n.includes(x)));
      const entry = {
        ...runtime,
      ...
      if (route17Leak.length) report.failures.push(`${key}: route-17 stop leaked ${route17Leak.join(',')}`);
    ...
    for (const t of regressionTargets) {
      try {
        ...
        console.log(`${t.routeId} regression`, JSON.stringify(r));
      } catch (e) {
