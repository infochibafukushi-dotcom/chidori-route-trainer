'use strict';
/**
 * Fetch full OSM relations for 日の出線 route 16.
 *   18396563 新浦安駅⇒プラウド新浦安パークマリーナ⇒日の出七丁目 (ref=16)
 *   18396562 日の出七丁目⇒プラウド新浦安パークマリーナ⇒新浦安駅 (ref=16)
 *
 * Also probes ref=16 AND ref=17 bus relations / route_master in the Urayasu bbox.
 * ref=17 is fetched ONLY as a separation guard: route 17 (日の出東経由) is a
 * DIFFERENT system that also terminates at 日の出七丁目. We record its ids/tags so
 * we can prove we never mixed 17 geometry into 16.
 *
...
const OUT = __dirname;
const IDS = [18396563, 18396562];
const LABELS = {
  18396563: 'route16 新浦安駅⇒プラウド新浦安パークマリーナ⇒日の出七丁目',
  18396562: 'route16 日の出七丁目⇒プラウド新浦安パークマリーナ⇒新浦安駅',
};
const UA = 'chidori-route-trainer/route16-hinode-research';
...
async function main() {
  const summary = {
    ...
    route: 'hinode-route16',
    lineName: '日の出線',
    knownPathSourceIds: IDS,
    separationGuard: 'ref=17 (日の出東経由) relations are listed but NEVER used for route-16 geometry.',
    relations: [],
  };
  ...
  console.log('probe ref=16 relations in bbox');
  summary.refProbe16 = await probeRefRelations('16');
  await new Promise((r) => setTimeout(r, 1500));
  console.log('probe ref=17 relations in bbox (separation guard)');
  summary.refProbe17 = await probeRefRelations('17');

  const interest = [
    '新浦安駅', '日の出七丁目', 'プラウド新浦安パークマリーナ', '海風の街', '日航東京',
    '日の出中学校', '順天堂大学', '日の出正門', '日の出南小学校', 'ベイシティ浦安',
    '明海大学', '日の出公民館', '日の出東',
  ];
  summary.nameChecks = {};
  ...
  // outbound vs inbound platform-name comparison (is inbound the exact reverse?)
  const [a, b] = summary.relations;
  if (a?.ok && b?.ok) {
    const an = a.platformNames || [];
    const bn = b.platformNames || [];
    const bRev = [...bn].reverse();
    summary.reverseCheck = {
      outboundId: a.id, inboundId: b.id,
      outboundCount: an.length, inboundCount: bn.length,
      inboundReversedEqualsOutbound: an.length === bn.length && an.every((n, i) => n === bRev[i]),
      outboundOnly: an.filter((n) => !bn.includes(n)),
      inboundOnly: bn.filter((n) => !an.includes(n)),
    };
  }
  ...
  console.log(JSON.stringify(summary.reverseCheck, null, 2));
  console.log('=== ref=16 probe ===');
  for (const p of summary.refProbe16 || []) console.log(p.type, p.id, JSON.stringify(p.tags || p));
  console.log('=== ref=17 probe (NOT USED) ===');
