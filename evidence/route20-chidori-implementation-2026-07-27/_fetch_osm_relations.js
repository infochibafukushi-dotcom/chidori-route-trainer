'use strict';
/**
 * Fetch full OSM relations for 千鳥線 route 20.
...
const OUT = __dirname;
const IDS = [13764790, 18323971, 18323972, 18351939, 18351940];
const LABELS = {
  13764790: 'route20 舞浜駅⇒千鳥東⇒千鳥車庫',
  18323971: 'route20 千鳥車庫⇒千鳥東⇒舞浜駅',
  18323972: 'route20 舞浜駅⇒千鳥循環',
  18351939: 'route20 クリーンセンター⇒千鳥車庫⇒舞浜駅',
  18351940: 'route20 舞浜駅⇒千鳥車庫⇒クリーンセンター',
};
const UA = 'chidori-route-trainer/route20-chidori-research';
...
relation["route"="bus"]["ref"="${ref}"](35.61,139.88,35.67,139.95);
out tags;`;
  try {
...
async function main() {
  const summary = {
    ...
    route: 'chidori-route20',
    lineName: '千鳥線',
    knownPathSourceIds: IDS,
    separationGuard: 'ref=22 (若潮通り線 / 22千鳥東) relations are listed but NEVER used for route-20 geometry.',
    relations: [],
  };
  ...
  console.log('probe ref=20 relations in bbox');
  summary.refProbe20 = await probeRefRelations('20');
  await new Promise((r) => setTimeout(r, 1500));
  console.log('probe ref=22 relations in bbox (separation guard)');
  summary.refProbe22 = await probeRefRelations('22');
