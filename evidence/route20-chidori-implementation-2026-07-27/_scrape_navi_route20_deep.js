'use strict';
/**
 * Deep pass: open EVERY 通過時刻表 (/stops?) link on route-20 course timetables.
...
async function listCourses(page, busstopId) {
  ...
  return { busstopId, coursesUrl, all, route20: all.filter((l) => hasRouteMark(l.text)) };
}
...
const out = {
  ...
  lineName: '千鳥線',
  note: 'Exhaustive per-course trip enumeration for route-20. Mixed cells with [22] require legend gate.',
  courses: [],
  timetables: [],
  signatures: {},
  trips: [],
...
async function processTimetable(page, ttMeta) {
  ...
  for (let i = 0; i < links.length; i++) {
    ...
    if (!confirmed) {
      out.rejected.push({ url: link.absHref, cellText: link.cellText, reason: 'no-20-evidence', stopNames: raw.stopNames });
      continue;
    }
...
async function main() {
  ...
  try {
    ...
    for (const stopName of FOLLOW_UP_STOPS) {
      ...
      out.followUps[stopName] = { busstopId: id, courses: null, route20Courses: null, note: null };
      if (!id) {
      ...
      out.followUps[stopName].route20Courses = c.route20.map((x) => ({ text: x.text, berth: x.berth, absHref: x.absHref }));
      if (!c.route20.length) {
        out.followUps[stopName].note = `${stopName}発の[20]公開便コースは見つからなかった。`;
        continue;
      }
      for (const course of c.route20) {
        for (const day of DATES) {
          if (pastHardTimeout()) break;
          await processTimetable(page, {
