'use strict';
/**
 * Continuous drive + integrity for route-20 千鳥線.
 */
...
const SIBLING_EXACT = ['新浦安駅', '新浦安駅北口', '若潮公園', '順天堂病院前', 'サンコーポ東口', 'サンコーポ西口', '弁天第二', '見明川中学校前', '見明川住宅', '舞浜三丁目'];
...
function startServer() {
  return new Promise((resolve, reject) => {
    const server = http.createServer((req, res) => {
      ...
      if (rel.startsWith('chidori-line-route-v1.js')) body = Buffer.from(speedUp(body.toString('utf8')), 'utf8');
      if (rel === 'index.html') {
...
async function readSys(page, key) {
  return page.evaluate((systemKey) => {
    const api = window.CHIDORI_LINE_ROUTE_V1;
    const route = api?.ensureRoute?.();
    const sys = route?.systems?.[systemKey];
    const st = window.CHIDORI_LINE_DRIVE_STATE || {};
...
(async () => {
  ...
  try {
    ...
    await page.goto(`http://127.0.0.1:${PORT}/index.html?nocache=r20`, { waitUntil: 'domcontentloaded', timeout: 60000 });
    ...
    await page.waitForFunction(() => window.CHIDORI_LINE_ROUTE_V1 && window.CHIDORI_LINE_PATH_V1 && window.CHIDORI_LINE_PATH_POLICY_V1, null, { timeout: 30000 });
    await page.waitForSelector('#driveStartPause', { timeout: 25000 });
    await page.waitForTimeout(900);
    await page.evaluate(async () => { await window.CHIDORI_LINE_ROUTE_V1.resolveAllSystems(true); });
    ...
    for (const key of SYSTEMS) {
      ...
      const runtime = await page.evaluate(async (k) => {
        ...
        const validation = policy.validateRuntimePath({
          ...
          expectedPathHash: window.CHIDORI_LINE_PATH_V1[k].pathHash,
    ...
    for (const [id, g] of [['route-19', 'TAKASU_MINAMI_LINE_ROUTE_V1'], ['route-22', null]]) {
      if (id === 'route-22') {
        report.regressions = report.regressions || {};
        report.regressions[id] = 'skipped-no-pack';
        continue;
      }
