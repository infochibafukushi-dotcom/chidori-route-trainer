'use strict';
/**
 * Fetch full OSM relations for 明海・高洲線 route 18 (夢海の街・高洲橋経由).
 * Known seed:
 *   18417590 新浦安駅⇒潮音の街⇒高洲北小学校 (ref=18)
 * All other ref=18 relations (高洲海浜公園行 / 新浦安駅行 / 浦安駅入口発着) are
 * DISCOVERED via Overpass, never assumed.
 *
 * Also probes ref=15 / 19 / 10 / 25 bus relations in the Urayasu bbox.
 * Those are fetched-as-tags ONLY as a separation guard: 15系統（東京学館前経由）,
 * 19系統（高洲四丁目・浦安南高校・特養経由）, 10系統（みなと南）and 25系統 share
 * 高洲海浜公園 / 高洲北小学校 / 潮音の街 / 高洲四丁目 with 18. We record their ids so we
 * can prove none of their geometry was mixed into 18.
 *
...
const OUT = __dirname;
const SEED_IDS = [18417590];
/** Sibling-route relations. Fetching or using any of these for route-18 geometry is forbidden. */
const FORBIDDEN_SIBLING_IDS = new Set([18419864, 18419865]);
const LABELS = {
  18417590: 'route18 新浦安駅⇒潮音の街⇒高洲北小学校 (seed)',
};
const UA = 'chidori-route-trainer/route18-akemi-takasu-research';
...
function get(url) {
  return new Promise((resolve, reject) => {
    const req = https
      ...
      .on('error', reject);
  // Overpass mirrors occasionally accept the connection and then never respond, which would
  // hang the whole fetch. Cap every request so a stalled mirror only costs one timeout.
...
async function fetchRelation(id) {
  if (FORBIDDEN_SIBLING_IDS.has(id)) {
    throw new Error(`sibling-route relation ${id} must never be fetched for route-18 geometry`);
  }
...
/** Any bus relation touching 夢海の街 / 高洲橋 / 海風の街 — catches ref-less or oddly tagged variants. */
async function probeByPlatformName() {
  const q = `[out:json][timeout:150];
node["highway"="bus_stop"]["name"~"夢海の街|高洲橋|海風の街|明海大学前"](${BBOX})->.p;
rel(bn.p)["route"="bus"];
out tags;`;
  try {
    const json = await overpass(q);
...
async function main() {
  const summary = {
    ...
    route: 'akemi-takasu-route18',
    lineName: '明海・高洲線',
    seedIds: SEED_IDS,
    separationGuard: 'ref=15 (東京学館前経由・relations 18419864/18419865) / ref=19 / ref=10 / ref=25 relations are '
      + 'listed as tags only and NEVER used for route-18 geometry.',
    relations: [],
  };
  ...
  console.log('probe ref=18 relations in bbox');
  summary.refProbe18 = await probeRefRelations('18');
  checkpoint('refProbe18');
  await new Promise((r) => setTimeout(r, 1500));

  const discovered = (summary.refProbe18 || [])
  ...
  console.log('probe by route-18 characteristic platform names');
  summary.platformProbe = await probeByPlatformName();
  checkpoint('platformProbe');
  await new Promise((r) => setTimeout(r, 1500));

  // Sibling guards run last: they only record ids/tags to prove non-use, so a flaky
  // mirror here must never cost us the route-18 geometry fetched above.
  for (const sib of ['15', '19', '10', '25']) {
  ...
  const interest = [
    '新浦安駅', '浦安駅入口', '高洲海浜公園', '高洲北小学校', '潮音の街', '夢海の街', '高洲橋',
    '高洲中央公園', '明海大学前', '海風の街', '入船中央エステート', '消防本部前', '神明裏', '猫実',
    '海楽', '美浜東団地', '高洲四丁目', '高洲八丁目', '高洲三丁目', '高洲二丁目', '高洲西児童公園',
    '順天堂大学入口', '東京学館前', '明海交差点', '入船橋', '高洲', '浦安南高校', 'みなと南',
  ];
  summary.nameChecks = {};
  ...
  console.log('=== ref=18 probe ===');
  for (const p of summary.refProbe18 || []) console.log(p.type, p.id, JSON.stringify(p.tags || p));
  for (const sib of ['15', '19', '10', '25']) {
