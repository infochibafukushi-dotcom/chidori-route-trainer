'use strict';
/**
 * Deep pass: open EVERY 通過時刻表 (/stops?) link on the route-18 course timetables
...
const ROUTE_NUM = '18';
const SIBLING_ROUTES = ['15', '19', '10', '25', '11', '23', '3'];
...
async function listCourses(page, busstopId) {
  ...
  return { busstopId, coursesUrl, ...info, all, route18: all.filter((l) => hasRouteMark(l.text)) };
}
...
async function processTimetable(page, ttMeta) {
  ...
  for (let i = 0; i < links.length; i++) {
    ...
    if (!confirmed) {
      out.rejected.push({ url: link.absHref, cellText: link.cellText, reason: 'no-18-evidence', stopNames: raw.stopNames });
      continue;
    }
...
async function main() {
  ...
  try {
    ...
    // pass has read real trips. Scan each for its own [18] departure courses.
    for (const stopName of FOLLOW_UP_STOPS) {
      if (pastHardTimeout()) break;
      const id = out.discoveredBusstopIds[stopName] || null;
      out.followUps[stopName] = { busstopId: id, courses: null, route18Courses: null, note: null };
      if (!id) {
      ...
      out.followUps[stopName].route18Courses = c.route18.map((x) => ({ text: x.text, berth: x.berth, absHref: x.absHref }));
      if (!c.route18.length) {
        out.followUps[stopName].note = `${stopName}発の[18]公開便コースは見つからなかった。`;
        continue;
      }
      for (const course of c.route18) {
        for (const day of DATES) {
          if (pastHardTimeout()) break;
          await processTimetable(page, {
