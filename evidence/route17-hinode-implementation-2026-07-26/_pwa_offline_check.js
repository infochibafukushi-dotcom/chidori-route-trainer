'use strict';
/**
 * PWA / service-worker check for the route-17 pack under CACHE_NAME v109.
 *
 * 1. Serve the repo over http://127.0.0.1 so the service worker can register.
 * 2. Load the app online, let the SW install and claim, then open route-17 so the
 *    on-demand pack (hinode-line-17-*.js/css?v=109) lands in the v109 cache.
 * 3. Assert the cache is named chidori-route-map-v109, that no older
 *    chidori-route-map-* cache survived activation, and that every route-17 pack URL
 *    is present with the ?v=109 query intact (the SW matches code assets exactly).
 * 4. Flip the context offline, reload, and re-open route-17 — the 3 systems must
 *    still resolve with the same pathHashes as the shipped bank.
 * 5. Assert the route-16 pack cache entries are untouched (still ?v=108).
 */
...
const EXPECTED_CACHE = 'chidori-route-map-v109';
const ROUTE17_PACK = [
  './hinode-line-17-stop-images-v1.css?v=109',
...
async function openRoute17(page) {
  ...
  await page.waitForFunction(
    () => window.HINODE_LINE_17_ROUTE_V1 && window.HINODE_LINE_17_PATH_V1
      && window.HINODE_LINE_17_PLATFORMS_V1 && window.HINODE_LINE_17_PATH_POLICY_V1,
    null, { timeout: 40000 },
  );
  await page.waitForSelector('#driveStartPause', { timeout: 25000 });
  await page.waitForTimeout(900);
  await page.evaluate(async () => { await window.HINODE_LINE_17_ROUTE_V1.resolveAllSystems(true); });
  await page.waitForTimeout(400);
  return page.evaluate((keys) => {
    const route = window.HINODE_LINE_17_ROUTE_V1.ensureRoute();
    const out = {};
    for (const k of keys) {
...
async function main() {
  ...
  if (report.indexLoaderTag !== '109') report.failures.push(`index.html loader ?v=${report.indexLoaderTag}`);
  ...
  try {
    ...
    report.online = await openRoute17(page);
    await page.screenshot({ path: path.join(SHOTS, 'pwa-online-route17-sp390.png'), fullPage: false });

    report.caches = await page.evaluate(async ({ expected, pack16, pack17 }) => {
      ...
      return {
        ...
        route17Missing: pack17.filter((u) => !has(u)),
        route16Present: pack16.filter(has),
        route17WrongVersion: entries.filter((u) => /hinode-line-17-/.test(u) && !/v=109/.test(u)),
        route16WrongVersion: entries.filter((u) => /hinode-line-(?!17-)/.test(u) && !/v=108/.test(u)),
      };
    }, { expected: EXPECTED_CACHE, pack16: ROUTE16_PACK, pack17: ROUTE17_PACK });

    if (report.caches.staleAppCaches.length) report.failures.push(`stale caches survived: ${report.caches.staleAppCaches.join(',')}`);
    if (report.caches.route17Missing.length) report.failures.push(`route-17 pack not cached: ${report.caches.route17Missing.join(',')}`);
    ...
    if (report.caches.route16WrongVersion.length) report.failures.push(`route-16 cached at wrong version: ${report.caches.route16WrongVersion.join(',')}`);

    // Offline replay: cut the network and reload from the v109 cache only.
    ...
    try {
      report.offline = await openRoute17(page);
      await page.screenshot({ path: path.join(SHOTS, 'pwa-offline-route17-sp390.png'), fullPage: false });
    } catch (e) {
      report.offline = { error: String(e.message || e) };
      report.failures.push(`offline route-17 failed: ${e.message || e}`);
    }
  ...
  console.log('route17 cached', report.caches?.route17Present?.length, '/', ROUTE17_PACK.length);
