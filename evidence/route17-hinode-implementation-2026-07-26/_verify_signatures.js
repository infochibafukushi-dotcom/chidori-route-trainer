 ...
 *
 * ★route-17 固有の危険:
 *   16系統（プラウド新浦安パークマリーナ・海風の街経由）も 日の出七丁目 発着で、
 *   プラウド新浦安パークマリーナ / 日の出中学校 / ベイシティ浦安 / 日の出西 /
 *   順天堂大学・日の出 正門 などの中間停留所では 16 と 17 が同じのりばに混載される。
 *   コース名や中間停留所の凡例だけでは 16/17 を分離できない。出発のりばまで遡る。
 *
 * ACCEPT 条件: (B) が 17 に解決し、(A) が 17 以外に解決していないこと。
 *
...
const ROUTE_NUM = '17';
const SIBLING_ROUTE = '16';
const DEEP = JSON.parse(fs.readFileSync(path.join(OUT_DIR, '_navi_deep_raw.json'), 'utf8'));

/** route-16 exclusive stop. Its presence is an independent red flag, never a gate on its own. */
const ROUTE16_EXCLUSIVE_STOPS = ['海風の街'];
/** route-17 exclusive stops per OSM + navi legend. Used only as a cross-check, never as the gate. */
const ROUTE17_EXCLUSIVE_STOPS = ['日の出東', '東京電力', '日の出小学校', '日の出保育園入口', 'アールフォーラム', '順天堂大学・日の出 東口'];
...
/** Symbol prefix of a timetable cell, e.g. "41ベ" -> "ベ", "05★ベ" -> "★ベ", "37" -> "" (無印). */
...
/** Parse legend lines of the form "無印…【１７系統】日の出東経由　日の出七丁目行き" */
...
/** Cache: busstopId -> [{berthLabel, courseText, legendParsed, courseIds, symbolsByCourse}] */
...
async function main() {
  ...
  const result = {
    ...
    policy: '(A) 掲載時刻表の凡例 と (B) 出発のりばの時刻表の凡例 の二段で系統を判定する。'
      + '16系統（プラウド新浦安パークマリーナ・海風の街経由）も日の出七丁目発着で、中間停留所ののりばでは'
      + '16と17が混載されるため、(B) 出発のりば（新浦安駅のりば17 ほか）の凡例を決定打とする。',
    timetableLegends: {},
    signatures: [],
  };
  ...
  for (const [sigKey, sig] of Object.entries(DEEP.signatures)) {
    ...
    const depIs17 = depProof.resolved && depProof.routeNumber === ROUTE_NUM;
    const listingContradicts = listingRouteNumbers.length > 0 && !listingRouteNumbers.includes(ROUTE_NUM);
    let verdict;
    if (depIs17 && !listingContradicts) verdict = `ACCEPT-route${ROUTE_NUM}`;
    ...
    const route16StopsPresent = ROUTE16_EXCLUSIVE_STOPS.filter((n) => sig.stopNames.includes(n));
    const route17StopsPresent = ROUTE17_EXCLUSIVE_STOPS.filter((n) => sig.stopNames.includes(n));
    ...
    result.signatures.push({
      ...
      route16ExclusiveStopsPresent: route16StopsPresent,
      route17ExclusiveStopsPresent: route17StopsPresent,
      verdict,
  ...
  for (const s of result.signatures) {
    ...
    lines.push(`    route16-exclusive stops: ${s.route16ExclusiveStopsPresent.join(',') || '(none)'} | route17-exclusive stops: ${s.route17ExclusiveStopsPresent.join(',') || '(none)'}`);
