'use strict';
/**
 * Build platforms + pathPoints for route-17 日の出線 from OSM relations.
 *
 * Rules:
 *  - Stop order comes from official-stop-orders.json (navi). Never reversed/truncated.
 *  - Each system gets its OWN relation. 18396569 / 18396583 / 18396568 are built
 *    independently; none is derived by reversing or truncating another.
 *  - densify only inside a single OSM way; way joins need a shared node or ≤1m.
 *  - Google Directions is never used.
 *  - ★ route-16（プラウド新浦安パークマリーナ・海風の街経由・relation 18396562/18396563）の
 *    path・停留所は同じ 日の出七丁目 発着でも一切流用しない。
 *
 * Writes hinode-line-17-platforms-v1.js and hinode-line-17-path-v1.js at repo root,
...
const MAX_GAP_M = 30;
const MAX_JOIN_M = 1;
const GENERATED = '2026-07-26-hinode-line-17-v1';

/** OSM relations belonging to route 16. Present only so the guard below can assert non-use. */
const FORBIDDEN_ROUTE16_RELATIONS = [18396562, 18396563];
/** route-16 exclusive stop. Its appearance in generated data is a build failure. */
const FORBIDDEN_ROUTE16_STOPS = ['海風の街'];

/**
 * OSM ways that are access-restricted but legitimately driven by route 17.
...
const SYSTEMS = {
  '17-hinode-nanachome': {
    ...
    note: 'outbound 新浦安駅→日の出七丁目（のりば17・[17]日の出東経由）。16系統（プラウド新浦安パークマリーナ・海風の街経由）の便やpathは流用禁止。',
  },
  '17-baycity-urayasu': {
    relationId: 18396583,
    resolvedVersion: '2026-07-26-hinode17-baycity-v1',
    pathSource: 'osm-relation-18396583+startHint-shinurayasu',
    note: 'outbound 新浦安駅→ベイシティ浦安（のりば17・符号ベ／★ベ）。日の出七丁目行きpathの延長・切詰めではなく専用relationから生成する。',
  },
  '17-shinurayasu': {
    relationId: 18396568,
    resolvedVersion: '2026-07-26-hinode17-shinurayasu-v1',
    pathSource: 'osm-relation-18396568+startHint-hinode-nanachome',
    note: 'inbound 日の出七丁目→新浦安駅（東京電力経由）。往路18396569の反転は禁止（往復で別車線・別ノードのplatformを持つ）。',
  },
};
...
function buildSystem(key, def) {
  ...
  for (const bad of FORBIDDEN_ROUTE16_STOPS) {
    if (names.includes(bad)) throw new Error(`${key}: route-16 exclusive stop ${bad} present in official order`);
  }
...
function main() {
  ...
  const summary = {
    ...
    generated: GENERATED,
    policy: {
      stopOrderSource: 'official-stop-orders.json（京成バスナビ個別便通過時刻表・凡例【１７系統】でゲート済み）',
      ...
      otherRouteReuseForbidden: 'route-16（プラウド新浦安パークマリーナ・海風の街経由）の path・停留所順は同じ日の出七丁目発着でも流用しない',
      forbiddenRoute16Relations: FORBIDDEN_ROUTE16_RELATIONS,
      forbiddenRoute16Stops: FORBIDDEN_ROUTE16_STOPS,
      maxGap_m: MAX_GAP_M,
  ...
  for (const key of SYSTEM_ORDER) {
    ...
    if (FORBIDDEN_ROUTE16_RELATIONS.includes(def.relationId)) {
      throw new Error(`${key}: relation ${def.relationId} belongs to route 16 and must never be used for route 17`);
    }
    ...
    for (const name of Object.keys(sys.platforms)) {
      if (FORBIDDEN_ROUTE16_STOPS.includes(name)) summary.blockers.push(`${key}: route-16 exclusive stop ${name} leaked into platforms`);
    }
  ...
  const isPrefixOf = (a, b) => a.length <= b.length
    && a.every((p, i) => Math.abs(p.lat - b[i].lat) < 1e-9 && Math.abs(p.lng - b[i].lng) < 1e-9);

  summary.reverseChecks = [
    { a: '17-hinode-nanachome', b: '17-shinurayasu' },
    { a: '17-baycity-urayasu', b: '17-shinurayasu' },
  ...
  summary.prefixChecks = [{ a: '17-hinode-nanachome', b: '17-baycity-urayasu' }]
    .map(({ a, b }) => ({ a, b, isExactPrefix: isPrefixOf(pathBank[a].pathPoints, pathBank[b].pathPoints) }));
  for (const p of summary.prefixChecks) {
    if (p.isExactPrefix) summary.warnings.push(`${p.a} path is an exact prefix of ${p.b} — verify both came from their own relation`);
  }
  ...
  fs.writeFileSync(
    path.join(ROOT, 'hinode-line-17-platforms-v1.js'),
    '// Auto-generated OSM platforms for 日の出線 系統17 (route-17).\n'
    + '// Official stop order: Keisei Bus Navi 個別便通過時刻表 2026-07-26（凡例でゲート）。\n'
    + '// 新浦安駅のりば17の [17]（日の出東経由）日の出七丁目行／ベイシティ浦安行 と\n'
    + '// 日の出七丁目のりば17の [17]（東京電力経由）新浦安駅行 のみを採用。\n'
    + '// 16系統（プラウド新浦安パークマリーナ・海風の街経由）は同じ日の出七丁目発着だが別系統であり、\n'
    + '// 停留所は採用していない（海風の街は route-17 に存在しない）。\n'
    + '// Each system uses the platforms of its own OSM relation; outbound/inbound platforms differ.\n'
    + `// Generated: ${GENERATED}\n`
    + `(() => {\n  window.HINODE_LINE_17_PLATFORMS_V1 = ${JSON.stringify(platformsBank, null, 2)};\n})();\n`,
    'utf8',
  );
  fs.writeFileSync(
    path.join(ROOT, 'hinode-line-17-path-v1.js'),
    ...
    + `// Generated: ${GENERATED}\n`
    + `(() => {\n  window.HINODE_LINE_17_PATH_V1 = ${JSON.stringify(pathBank, null, 2)};\n})();\n`,
    'utf8',
  );
  ...
  console.log('wrote hinode-line-17-platforms-v1.js and hinode-line-17-path-v1.js');
}
