'use strict';
/**
 * Deep pass: open EVERY 通過時刻表 (/stops?) link on the route-17 course timetables
 * across weekday / saturday / sunday, so short-turn variants (e.g. ベイシティ浦安行 vs
 * 日の出七丁目行) and 深夜 variants cannot be missed by sampling.
 *
 * 新浦安駅のりば17 のセルは [17] / [深夜] 混載（日の出七丁目行 と ベイシティ浦安行）なので、
 * ここでは候補を広く拾い、系統確定は _verify_signatures.js の凡例ゲート（符号→【Ｎ系統】）に委ねる。
 *
 * Also discovers 日の出七丁目 / ベイシティ浦安 / 日の出東 の busstop id を確定便から取得し、
 * それぞれの course 一覧を [17] で走査して復路（新浦安駅行）を拾う。
 *
...
const HARD_TIMEOUT_MS = 75 * 60 * 1000;
...
async function listCourses(page, busstopId) {
  ...
  return { busstopId, coursesUrl, ...info, all, route17: all.filter((l) => hasRouteMark(l.text)) };
}
...
const out = {
  ...
  siblingRouteNumber: '16',
  lineName: '日の出線',
  note: 'Exhaustive per-course trip enumeration. Every /stops? link on each route-17 candidate timetable is opened. 16系統（プラウド新浦安パークマリーナ・海風の街経由）は同じ日の出七丁目発着のため凡例ゲートで最終判定する。',
  courses: [],
  timetables: [],
  signatures: {},
  trips: [],
...
async function processTimetable(page, ttMeta) {
  ...
  console.log('TT', ttMeta.terminal, ttMeta.dayLabel, 'links', links.length, 'legend', JSON.stringify(meta.legend.slice(0, 8)));
  ...
  for (let i = 0; i < links.length; i++) {
    ...
    if (!confirmed) {
      out.rejected.push({ url: link.absHref, cellText: link.cellText, reason: 'no-17-evidence', stopNames: raw.stopNames });
      continue;
    }
...
async function main() {
  ...
  try {
    ...
    // Follow-ups: inbound courses depart from the far terminals, whose busstop ids
    // are only known after the outbound trips have been read.
    for (const stopName of FOLLOW_UP_STOPS) {
      if (pastHardTimeout()) break;
      const id = out.discoveredBusstopIds[stopName] || null;
      out.followUps[stopName] = { busstopId: id, courses: null, route17Courses: null, note: null };
      if (!id) {
      ...
      out.followUps[stopName].route17Courses = c.route17.map((x) => ({ text: x.text, berth: x.berth, absHref: x.absHref }));
      if (!c.route17.length) {
        out.followUps[stopName].note = `${stopName}発の[17]公開便コースは見つからなかった。`;
        continue;
      }
      for (const course of c.route17) {
        for (const day of DATES) {
          if (pastHardTimeout()) break;
          await processTimetable(page, {
