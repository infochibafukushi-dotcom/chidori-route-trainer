'use strict';
/**
 * Geometry QA for route-17 paths.
 *
 * Checks, per system:
 ...
 *   - route-16 relations and the route-16 exclusive stop must not appear in the shipped bank
...
function main() {
  const pathBank = loadShippedBank('hinode-line-17-path-v1.js', 'HINODE_LINE_17_PATH_V1');
  const platformsBank = loadShippedBank('hinode-line-17-platforms-v1.js', 'HINODE_LINE_17_PLATFORMS_V1');

  const report = {
    checkedAt: new Date().toISOString(),
    routeId: 'route-17',
    thresholds: { maxGap_m: MAX_GAP_M, uturnDeg: UTURN_DEG, platformSoft_m: PLATFORM_SOFT, platformHard_m: PLATFORM_HARD },
    route16RelationsUsed: [],
    route16StopsUsed: [],
    systems: {},
    failures: [],
    reviewRequired: [],
  ...
  for (const [key, bank] of Object.entries(pathBank)) {
    if (ROUTE16_RELATIONS.includes(bank.relationId)) report.route16RelationsUsed.push({ key, relationId: bank.relationId });
    for (const name of Object.keys(platformsBank[key] || {})) {
      if (ROUTE16_EXCLUSIVE_STOPS.includes(name)) report.route16StopsUsed.push({ key, name });
    }
  ...
  if (report.route16RelationsUsed.length) {
    report.failures.push(`route-16 relations used: ${JSON.stringify(report.route16RelationsUsed)}`);
  }
  if (report.route16StopsUsed.length) {
    report.failures.push(`route-16 exclusive stops used: ${JSON.stringify(report.route16StopsUsed)}`);
  }

  report.pass = report.failures.length === 0;
  fs.writeFileSync(path.join(OUT, '_geometry_qa_report.json'), JSON.stringify(report, null, 2), 'utf8');
  console.log('route16RelationsUsed', JSON.stringify(report.route16RelationsUsed));
  console.log('route16StopsUsed', JSON.stringify(report.route16StopsUsed));
