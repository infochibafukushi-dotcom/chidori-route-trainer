'use strict';
/**
 * Local end-to-end validation for route-14 弁天・富岡線.
 *
 * Serves the repo, boots the real app with a Google Maps mock, selects route-14,
 * then for each of the 4 systems:
 ...
 *   5. continuous drive from 始発 to 終点 with no position jumps
 *
 * Also asserts routes 1–12 modules are untouched by checking route-12 still resolves.
 */
...
const SYSTEMS = ['14-maihama', '14-chidori-garage', '14-shinurayasu-maihama', '14-shinurayasu-chidori'];
...
function startServer() {
  return new Promise((resolve, reject) => {
    const server = http.createServer((req, res) => {
      ...
      if (rel.startsWith('benten-tomioka-line-route-v1.js')) body = Buffer.from(speedUp(body.toString('utf8')), 'utf8');
      if (rel === 'index.html') {
...
async function readSys(page, key) {
  return page.evaluate((systemKey) => {
    const api = window.BENTEN_TOMIOKA_LINE_ROUTE_V1;
    const route = api?.ensureRoute?.();
    const sys = route?.systems?.[systemKey];
    const st = window.BENTEN_TOMIOKA_LINE_DRIVE_STATE || {};
...
async function main() {
  ...
  try {
    ...
    await page.goto(`http://127.0.0.1:${PORT}/index.html?nocache=r14`, { waitUntil: 'domcontentloaded', timeout: 60000 });
    await page.waitForTimeout(1200);
    await page.evaluate(() => { if (typeof go === 'function') go('routes'); });
    await page.waitForSelector('#routeSelect', { timeout: 15000 });

    const hasRoute14Option = await page.evaluate(() => !!document.querySelector('#routeSelect option[value="route-14"]'));
    report.route14OptionPresent = hasRoute14Option;
    if (!hasRoute14Option) report.failures.push('route-14 not present in #routeSelect');

    await page.selectOption('#routeSelect', 'route-14');
    await page.waitForFunction(() => window.BENTEN_TOMIOKA_LINE_ROUTE_V1 && window.BENTEN_TOMIOKA_LINE_PATH_V1 && window.BENTEN_TOMIOKA_LINE_PLATFORMS_V1 && window.BENTEN_TOMIOKA_LINE_PATH_POLICY_V1, null, { timeout: 30000 });
    await page.waitForSelector('#driveStartPause', { timeout: 25000 });
    await page.waitForTimeout(900);

    // Force a full resolve so every system has path + hash computed.
    await page.evaluate(async () => {
      await window.BENTEN_TOMIOKA_LINE_ROUTE_V1.resolveAllSystems(true);
    });
    ...
    for (const key of SYSTEMS) {
      ...
      const runtime = await page.evaluate(async (k) => {
        ...
        const validation = policy.validateRuntimePath({
          ...
          expectedPathHash: window.BENTEN_TOMIOKA_LINE_PATH_V1[k].pathHash,
      ...
      const entry = {
        ...
        stopIdPrefixOk: runtime.stopIds.every((id) => String(id).startsWith(`benten-tomioka-${key}-`)),
      };
      ...
      if (!entry.stopIdPrefixOk) report.failures.push(`${key}: stop id prefix wrong`);
    ...
    // Regression: route-12 must still load and resolve after the loader change.
    try {
      await page.selectOption('#routeSelect', 'route-12');
      await page.waitForFunction(() => window.MAIHAMA_RESORT_LINE_ROUTE_V1, null, { timeout: 25000 });
      await page.waitForTimeout(800);
      report.route12Regression = await page.evaluate(() => {
        const route = window.MAIHAMA_RESORT_LINE_ROUTE_V1.ensureRoute();
        const sys = route.systems['12-maihama-via-resort'];
        return { stops: sys?.stops?.length || 0, pathPoints: sys?.path?.length || 0, pathHash: sys?.pathHash || null };
      });
      if (!report.route12Regression.stops) report.failures.push('route-12 regression: no stops');
      console.log('route-12 regression', JSON.stringify(report.route12Regression));
    } catch (e) {
      report.route12Regression = { error: String(e.message || e) };
      report.failures.push(`route-12 regression: ${e.message || e}`);
    }
  } finally {
    await browser.close();
    server.close();
