'use strict';
/**
 * Keisei Bus Navi audit scrape for 系統14 弁天・富岡線 (route-14).
 ...
 *  - Require [14] / 【１４系統】 evidence on the trip page before accepting.
 *  - Reject route 4 (富岡線), 20 (千鳥線), 9/25 (舞浜線系) contamination.
 *
...
const SEARCH_WORDS = ['新浦安駅', '舞浜駅', '千鳥車庫', '中央公園', '弁天中央', '順天堂病院前'];
...
function hasRoute14Mark(text) {
  const t = toAsciiDigits(String(text || ''));
  return /\[14\]|【\s*14\s*系統\s*】/.test(t);
}
...
function romajiKeyFromName(name) {
  ...
  if (/千鳥車庫/.test(n)) return 'chidori-garage';
  if (/中央公園/.test(n)) return 'chuo-koen';
  if (/弁天中央/.test(n)) return 'benten-chuo';
  if (/順天堂病院前/.test(n)) return 'juntendo';
...
async function listCourses(page, busstopId, label) {
  ...
  const route14 = all.filter((l) => hasRoute14Mark(l.text));
  return { label, busstopId, coursesUrl, title: pageInfo.title, heading: pageInfo.h, all, route14 };
}
...
async function scrapeTrip(page, url, meta) {
  ...
  try {
    ...
    if (!out.systemNumberHint && hasRoute14Mark(data.body)) out.systemNumberHint = ROUTE_NUM;
    try {
...
function emptyReport() {
  return {
    ...
    lineName: '弁天・富岡線',
    note: 'Route 14 only. Require [14]/【１４系統】 on trip page. Stop orders are read from 通過時刻表 only.',
    knownIds: { ...KNOWN_IDS },
    searches: {},
    terminals: {},
    timetables: [],
...
async function openTripQueue(page, reportObj, toOpen, phaseLabel) {
  ...
  for (let i = 0; i < toOpen.length; i++) {
    ...
    if (link.otherRoute && link.otherRoute !== ROUTE_NUM && !hasRoute14Mark(link.cellText)) {
    ...
    const confirmed14 = raw.systemNumberHint === ROUTE_NUM
      || hasRoute14Mark(link.cellText)
      || hasRoute14Mark(raw.heading)
      || hasRoute14Mark(raw.title)
      || hasRoute14Mark(raw.bodySnippet)
      || hasRoute14Mark(link.courseText);
    if (!confirmed14) {
    ...
    const trip = {
      ...
      heading: raw.heading, title: raw.title, systemNumberHint: raw.systemNumberHint, confirmed14: true,
...
async function main() {
  ...
  try {
    ...
    // Harvest terminal ids from searches
    const pick = (word, re) => {
    ...
    const terminals = [
      ...
      { key: 'maihama', label: '舞浜駅', id: KNOWN_IDS.maihama || pick('舞浜駅', /舞浜駅/) },
      { key: 'chidoriGarage', label: '千鳥車庫', id: pick('千鳥車庫', /千鳥車庫/) },
      { key: 'chuoKoen', label: '中央公園', id: pick('中央公園', /中央公園/) },
    ].filter((t) => t.id);
    ...
    for (const term of terminals) {
      ...
      console.log('  route14 courses:', courses.route14.length, '/ all', courses.all.length);
      for (const c of courses.route14) {
        console.log('   -', c.berthLetter || c.berth, (c.text || '').slice(0, 120));
        report.berthsSeen.push({ terminal: term.label, busstopId: term.id, berth: c.berthLetter || c.berth, courseText: (c.text || '').slice(0, 240), href: c.absHref });
      }
      for (const course of courses.route14.slice(0, 12)) {
    ...
    for (const job of timetableJobs) {
      ...
      // Interleave opening so we discover signatures early
      if (queue.length >= 20) {
  ...
  for (const [sig, v] of Object.entries(report.tripSignatures)) {
