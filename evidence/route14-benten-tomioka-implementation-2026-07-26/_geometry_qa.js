'use strict';
/**
 * Geometry QA for route-14 paths.
 *
 * Checks, per system:
...
function main() {
  const pathBank = loadShippedBank('benten-tomioka-line-path-v1.js', 'BENTEN_TOMIOKA_LINE_PATH_V1');
  const platformsBank = loadShippedBank('benten-tomioka-line-platforms-v1.js', 'BENTEN_TOMIOKA_LINE_PLATFORMS_V1');
  ...
  for (const [key, bank] of Object.entries(pathBank)) {
    ...
    // --- way classification & oneway compliance ---
    ...
    // --- U-turns (ignore the first/last 60m where a terminal turnaround is legitimate) ---
    const cum = [0];
