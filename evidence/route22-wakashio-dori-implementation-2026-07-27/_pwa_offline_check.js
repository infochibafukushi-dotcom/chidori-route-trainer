...
const EXPECTED_CACHE = 'chidori-route-map-v113';
const ROUTE22_PACK = [
...
(async () => {
  ...
  const report = { routeId: 'route-22', expectedCache: EXPECTED_CACHE, failures: [], pass: false, d1PutCount: 0 };
  ...
  try {
    ...
    await page.goto(`http://127.0.0.1:${PORT}/index.html?nocache=r22pwa`, { waitUntil: 'domcontentloaded', timeout: 60000 });
    await page.waitForTimeout(1500);
    await page.evaluate(() => { if (typeof go === 'function') go('routes'); });
    await page.selectOption('#routeSelect', 'route-22');
    await page.waitForFunction(() => window.WAKASHIO_DORI_LINE_22_ROUTE_V1 && window.WAKASHIO_DORI_LINE_22_PATH_V1, null, { timeout: 30000 });
    await page.waitForTimeout(1000);
    await page.screenshot({ path: path.join(SHOTS, 'route22-pc1280.png'), fullPage: false });
    const cacheInfo = await page.evaluate(async (pack) => {
      const names = await caches.keys();
      const cache = await caches.open(names.find((n) => n.includes('v113')) || names[0]);
    ...
    if (!String(report.loaderTag).includes('v=113')) report.failures.push('loader not v113');
    await ctx.setOffline(true);
    await page.reload({ waitUntil: 'domcontentloaded' });
    await page.waitForTimeout(1200);
    await page.evaluate(() => { if (typeof go === 'function') go('routes'); });
    await page.selectOption('#routeSelect', 'route-22');
    await page.waitForFunction(() => window.WAKASHIO_DORI_LINE_22_ROUTE_V1, null, { timeout: 20000 });
    const offlineHashes = await page.evaluate((keys) => {
      const route = window.WAKASHIO_DORI_LINE_22_ROUTE_V1.ensureRoute();
      return Object.fromEntries(keys.map((k) => [k, route.systems[k]?.pathHash || window.WAKASHIO_DORI_LINE_22_PATH_V1[k]?.pathHash]));
    }, SYSTEMS);
