'use strict';
/**
 * Continuous drive + integrity for route-22 若潮通り線.
 */
...
function startServer() {
  return new Promise((resolve, reject) => {
    const server = http.createServer((req, res) => {
      ...
      if (rel.startsWith('wakashio-dori-line-22-route-v1.js')) body = Buffer.from(speedUp(body.toString('utf8')), 'utf8');
      if (rel === 'index.html') {
...
async function readSys(page, key) {
  return page.evaluate((systemKey) => {
    ...
    const st = window.WAKASHIO_DORI_LINE_22_DRIVE_STATE || {};
...
(async () => {
  ...
  try {
    ...
    await page.goto(`http://127.0.0.1:${PORT}/index.html?nocache=r22`, { waitUntil: 'domcontentloaded', timeout: 60000 });
    ...
    await page.waitForFunction(() => window.WAKASHIO_DORI_LINE_22_ROUTE_V1 && window.WAKASHIO_DORI_LINE_22_PATH_V1 && window.WAKASHIO_DORI_LINE_22_PATH_POLICY_V1, null, { timeout: 30000 });
    await page.evaluate(() => { routeState.routeId = 'route-22'; if (typeof routes === 'function') routes(); });
    await page.waitForSelector('#driveStartPause', { timeout: 60000 });
    await page.waitForTimeout(900);
    await page.evaluate(async () => { await window.WAKASHIO_DORI_LINE_22_ROUTE_V1.resolveAllSystems(true); });
    ...
    for (const key of SYSTEMS) {
      ...
      const runtime = await page.evaluate(async (k) => {
        ...
        const validation = policy.validateRuntimePath({
          ...
          expectedPathHash: window.WAKASHIO_DORI_LINE_22_PATH_V1[k].pathHash,
