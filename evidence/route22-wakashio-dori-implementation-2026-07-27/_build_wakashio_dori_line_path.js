'use strict';
/**
 * Build platforms + pathPoints for route-22 若潮通り線（22千鳥東） from OSM relations.
 *
 * Rules:
...
const MAX_GAP_M = 30;
const MAX_JOIN_M = 1;
const GENERATED = '2026-07-27-wakashio-dori-line-22-v1';
...
function main() {
  ...
  const summary = {
    ...
    generated: GENERATED,
    policy: {
      stopOrderSource: 'official-stop-orders.json（京成バスナビ個別便通過時刻表・2段凡例ゲートで【２２系統】に確定）',
      pathSource: 'OSM route relation way members（方向補正あり）。18396547／18396546 を独立構築',
      googleDirectionsUsed: false,
      reverseReuseForbidden: true,
      truncationReuseForbidden: true,
      siblingRouteSeparation: 'route-20 relations 13764790/18323971/18323972/18351939/18351940 は使用禁止',
  ...
  for (const key of SYSTEM_ORDER) {
    ...
    for (const rid of relIds) {
      if (allForbiddenRelations.includes(rid)) {
        throw new Error(`${key}: relation ${rid} belongs to a sibling route and must never be used for route 22`);
      }
    }
  ...
  summary.reverseChecks = [
    { a: '22-shinurayasu-chidori-garage', b: '22-chidori-garage-shinurayasu' },
  ...
  summary.spliceChecks = [
    { a: '22-shinurayasu-chidori-garage', b: '22-chidori-garage-shinurayasu' },
    { a: '22-chidori-garage-shinurayasu', b: '22-shinurayasu-chidori-garage' },
  ].map(({ a, b }) => {
  ...
  fs.writeFileSync(
    path.join(ROOT, 'wakashio-dori-line-22-platforms-v1.js'),
    ...
    + `// Generated: ${GENERATED}\n`
    + `(() => {\n  window.WAKASHIO_DORI_LINE_22_PLATFORMS_V1 = ${JSON.stringify(platformsBank, null, 2)};\n})();\n`,
    'utf8',
  );
  fs.writeFileSync(
    path.join(ROOT, 'wakashio-dori-line-22-path-v1.js'),
    ...
    + `// Generated: ${GENERATED}\n`
    + `(() => {\n  window.WAKASHIO_DORI_LINE_22_PATH_V1 = ${JSON.stringify(pathBank, null, 2)};\n})();\n`,
    'utf8',
  );
  ...
  console.log('wrote wakashio-dori-line-22-platforms-v1.js and wakashio-dori-line-22-path-v1.js');
}
