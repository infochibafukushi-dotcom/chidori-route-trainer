'use strict';
/**
 * Continuous Start→end drive for route-11 シンボルロード線 (6 systems).
 */
...
function startServer() {
  return new Promise((resolve, reject) => {
    const server = http.createServer((req, res) => {
      ...
      if (rel.startsWith('symbol-road-line-route-v1.js')) {
...
async function readSys(page, key) {
  return page.evaluate((systemKey) => {
    const route = window.SYMBOL_ROAD_LINE_ROUTE_V1?.ensureRoute?.();
    const sys = route?.systems?.[systemKey];
    const st = window.SYMBOL_ROAD_LINE_DRIVE_STATE || {};
...
async function main() {
  ...
  try {
    ...
    await page.goto(`http://127.0.0.1:${PORT}/index.html?nocache=r11drive`, {
      waitUntil: 'domcontentloaded',
      timeout: 60000,
    });
    await page.waitForFunction(
      () => window.SYMBOL_ROAD_LINE_ROUTE_V1 && window.SYMBOL_ROAD_LINE_PATH_V1,
      null,
      { timeout: 30000 },
    );
