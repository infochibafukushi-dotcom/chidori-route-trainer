'use strict';
/**
 * Geometry / intersection audit for route-11 シンボルロード線 path banks.
...
const VISUAL_Z19_BY_SYSTEM = {
  ...
  "11-urayasu-sogo-via-hinode-kominkan": [
    "11-urayasu-sogo-short-end-z19.png"
  ],
  "11-urayasu-baypark": [
    "11-urayasu-baypark-short-end-z19.png"
  ],
  "11-urayasu-shinurayasu": [
    "11-urayasu-shinurayasu-end-z19.png"
  ],
  "11-shinurayasu-hinode": [
    "11-shinurayasu-hinode-start-z19.png"
  ],
  "11-shinurayasu-sogo": [
    "11-shinurayasu-sogo-end-z19.png"
  ],
  "11-shinurayasu-baypark": [
    "11-shinurayasu-baypark-end-z19.png"
  ],
  "11-hinode-urayasu": [
  ...
  "11-hinode-shinurayasu": [
    "11-hinode-shinurayasu-z19.png"
  ],
  "11-sogo-shinurayasu": [
    "11-sogo-shinurayasu-start-z19.png"
  ],
  "11-sogo-urayasu": [
    "11-sogo-urayasu-end-z19.png"
  ]
};
...
const PATH_BANK = loadWindow('symbol-road-line-path-v1.js').SYMBOL_ROAD_LINE_PATH_V1;
...
function overpassQuery(query, timeoutMs = 20000) {
...
function classifyWayTags(used) {
  ...
  for (const w of used) {
    ...
    const blocked = tags.access === 'no' || tags.access === 'private' || tags.motor_vehicle === 'no';
    if (blocked) wrongWayCount += 1;
    if (samples.length < 8) {
      samples.push({
        wayId: w.wayId,
        highway: hw || null,
...
async function main() {
  ...
  const report = {
    ...
    route: 'route-11',
    lineName: 'シンボルロード線',
    pathFiles: ['symbol-road-line-path-v1.js', '_path_bank.json'],
    systems: {},
    pass: false,
  };
  ...
  // 総合公園〜日の出南で同一OSM wayを複数回辿るため、営業経路上の自己交差は想定内。
  // 自動化の合否は「道路から離れていない / 歩行者道でない / 建物・緑地貫通なし」を必須とする。
  const geometryOk = SYSTEMS.every((k) => {
    const s = report.systems[k];
    return s.pointsFarFromWaysCount === 0
      && (s.pedestrianWayCount || 0) === 0
      && (s.buildingIntersectionCount || 0) === 0
      && (s.groundGreenIntersectionCount || 0) === 0;
  });
  const allOverpassOk = SYSTEMS.every((k) => report.systems[k].overpassOk);
  const screenshotsOk = SYSTEMS.every((k) => {
    const files = VISUAL_Z19_BY_SYSTEM[k] || [];
    return files.length > 0 && files.every((f) => fs.existsSync(path.join(SCREENSHOT_DIR, f)));
  });
  report.selfIntersectionPolicy = 'expected-at-sogo-hinode-loop';
  report.selfIntersectionTotals = Object.fromEntries(
    SYSTEMS.map((k) => [k, report.systems[k].selfIntersectionCount]),
  );
  report.pass = geometryOk && screenshotsOk && (allOverpassOk || screenshotsOk);
  report.anyReviewRequired = SYSTEMS.some((k) => report.systems[k].reviewRequired);
  report.allOverpassOk = allOverpassOk;
  report.screenshotsOk = screenshotsOk;
