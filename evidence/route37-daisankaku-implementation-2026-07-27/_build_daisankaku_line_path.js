'use strict';
/**
 * Build platforms + pathPoints for route-37 大三角線 from OSM relations.
 *
 * Rules:
 *  - Stop order comes from official-stop-orders.json (navi). Never reversed/truncated.
 *  - Two dedicated relations: 18323271 (outbound) / 18323272 (inbound).
 *  - route-9 (舞浜線) relations は使用禁止。南行徳↔舞浜/TDS は37専用OSM。
 *
 * Writes daisankaku-line-*-v1.js at repo root,
...
const MAX_GAP_M = 30;
const MAX_JOIN_M = 1;
const GENERATED = '2026-07-27-daisankaku-line-v1';

/** OSM relations belonging to sibling route-9 (舞浜線). NEVER use for route-37 geometry. */
const FORBIDDEN_SIBLING_RELATIONS = {
  9: [18320323, 3498220, 18419884, 18419885],
};

/** Stops exclusive to route-9 — must not appear in route-37 generated data. */
const FORBIDDEN_SIBLING_STOPS = {
  9: ['浦安駅入口', 'フラワー通り', '堀江三丁目', '南小入口', '東野一丁目', '東野プール', '東海大浦安高校入口'],
};
...
const SYSTEMS = {
  '37-minamigyotoku-tds': {
    relationId: 18323271,
    resolvedVersion: '2026-07-27-daisankaku37-minamigyotoku-tds-v1',
    pathSource: 'osm-relation-18323271+startHint-minamigyotoku',
    platformByIndex: true,
    note: 'outbound 南行徳→TDS full。relation 18323271（network=舞浜線 tagだがref=37）。',
  },
  '37-minamigyotoku-maihama': {
    relationId: 18323271,
    resolvedVersion: '2026-07-27-daisankaku37-minamigyotoku-maihama-v1',
    pathSource: 'osm-relation-18323271+slice-maihama-terminal',
    platformByIndex: true,
    note: 'outbound 南行徳→舞浜止まり。18323271 slice（符号無印）。',
  },
  '37-tds-minamigyotoku': {
    relationId: 18323272,
    resolvedVersion: '2026-07-27-daisankaku37-tds-minamigyotoku-v1',
    pathSource: 'osm-relation-18323272+startHint-tds',
    platformByIndex: true,
    note: 'inbound TDS→南行徳 full。relation 18323272（network=大三角線）。',
  },
  '37-maihama-minamigyotoku': {
    relationId: 18323272,
    resolvedVersion: '2026-07-27-daisankaku37-maihama-minamigyotoku-v1',
    pathSource: 'osm-relation-18323272+slice-maihama-departure',
    platformByIndex: true,
    note: 'inbound 舞浜→南行徳。18323272 slice。',
  },
  '37-tds-horie6': {
    relationId: 18323272,
    resolvedVersion: '2026-07-27-daisankaku37-tds-horie6-v1',
    pathSource: 'osm-relation-18323272+slice-horie6-terminal',
    platformByIndex: true,
    note: 'inbound short TDS→堀江六丁目。18323272 slice。',
  },
  '37-fujimi3-tds': {
    relationId: 18323271,
    resolvedVersion: '2026-07-27-daisankaku37-fujimi3-tds-v1',
    pathSource: 'osm-relation-18323271+slice-fujimi3-departure',
    platformByIndex: true,
    note: 'outbound short 富士見三丁目→TDS。18323271 slice。',
  },
  '37-horie6-tds': {
    relationId: 18323271,
    resolvedVersion: '2026-07-27-daisankaku37-horie6-tds-v1',
    pathSource: 'osm-relation-18323271+slice-horie6-departure',
    platformByIndex: true,
    note: 'outbound short 堀江六丁目→TDS。18323271 slice。',
  },
};
...
/** Verified Navi short-turn / terminal-split pairs on the same OSM relation. */
const ALLOWED_CONTIGUOUS_SLICE_PAIRS = [
  ['37-minamigyotoku-maihama', '37-minamigyotoku-tds'],
  ['37-maihama-minamigyotoku', '37-tds-minamigyotoku'],
  ['37-tds-horie6', '37-tds-minamigyotoku'],
  ['37-fujimi3-tds', '37-minamigyotoku-tds'],
  ['37-horie6-tds', '37-minamigyotoku-tds'],
  ['37-fujimi3-tds', '37-horie6-tds'],
];
...
function buildDedicatedSystem(key, def) {
  ...
  const entryPlatform = platforms.find((p) => /entry/.test(p.role || '')) || platforms[0];
  const pathBuild = buildPathFromWays(loaded.rel, loaded.nodes, loaded.ways, {
    lat: entryPlatform.lat, lng: entryPlatform.lng,
  });
...
function main() {
  ...
  const summary = {
    ...
    generated: GENERATED,
    policy: {
      stopOrderSource: 'official-stop-orders.json（京成バスナビ個別便通過時刻表・2段凡例ゲートで【３７系統】に確定）',
      pathSource: 'OSM route relation way members（方向補正あり）。18323271/72 を独立構築',
      googleDirectionsUsed: false,
      reverseReuseForbidden: true,
      truncationReuseForbidden: true,
      siblingRouteSeparation: 'route-9 (舞浜線) relations は使用禁止',
  ...
  for (const key of SYSTEM_ORDER) {
    ...
    for (const rid of relIds) {
      if (allForbiddenRelations.includes(rid)) {
        throw new Error(`${key}: relation ${rid} belongs to a sibling route and must never be used for route 37`);
      }
    }
  ...
  const allowedSliceSet = new Set(ALLOWED_CONTIGUOUS_SLICE_PAIRS.map(([a, b]) => `${a}|${b}`));

  summary.reverseChecks = [
    { a: '37-minamigyotoku-tds', b: '37-tds-minamigyotoku' },
    { a: '37-minamigyotoku-maihama', b: '37-maihama-minamigyotoku' },
  ...
  for (let i = 0; i < keys.length; i++) {
    for (let j = 0; j < keys.length; j++) {
      if (i === j) continue;
      splicePairs.push({ a: keys[i], b: keys[j] });
    }
  }
  summary.spliceChecks = splicePairs.map(({ a, b }) => {
    const isContiguousSlice = isContiguousSliceOf(pathBank[a].pathPoints, pathBank[b].pathPoints);
    const allowed = allowedSliceSet.has(`${a}|${b}`);
  ...
  fs.writeFileSync(
    path.join(ROOT, 'daisankaku-line-platforms-v1.js'),
    ...
    + `// Generated: ${GENERATED}\n`
    + `(() => {\n  window.DAISANKAKU_LINE_PLATFORMS_V1 = ${JSON.stringify(platformsBank, null, 2)};\n})();\n`,
    'utf8',
  );
  fs.writeFileSync(
    path.join(ROOT, 'daisankaku-line-path-v1.js'),
    '// Auto-generated OSM road path geometry for 大三角線 系統37 (route-37).\n'
    + '// Paths follow OSM route relation way members (direction-corrected). Google Directions not used.\n'
    + '// Own relations only: 18323271/72. 舞浜止まり vs TDS行き are separate systems.\n'
    + '// NEVER use route-9 relations or reverse one system for another.\n'
    + `// Generated: ${GENERATED}\n`
    + `(() => {\n  window.DAISANKAKU_LINE_PATH_V1 = ${JSON.stringify(pathBank, null, 2)};\n})();\n`,
    'utf8',
  );
  ...
  console.log('spliceChecks', JSON.stringify(summary.spliceChecks.filter((s) => s.isContiguousSlice)));
  console.log('warnings', summary.warnings);
  if (summary.blockers.length) {
    console.error('BLOCKERS', summary.blockers);
    process.exit(1);
  }
  console.log('wrote daisankaku-line-platforms-v1.js and daisankaku-line-path-v1.js');
}
