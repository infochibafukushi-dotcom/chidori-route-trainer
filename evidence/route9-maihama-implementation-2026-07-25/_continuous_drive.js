'use strict';
/**
 * Continuous Start→end drive for route-9 舞浜線 (6 systems).
 */
...
function startServer() {
  return new Promise((resolve, reject) => {
    const server = http.createServer((req, res) => {
      ...
      if (rel.startsWith('maihama-line-route-v1.js')) {
...
async function readSys(page, key) {
  return page.evaluate((systemKey) => {
    const route = window.MAIHAMA_LINE_ROUTE_V1?.ensureRoute?.();
    const sys = route?.systems?.[systemKey];
    const st = window.MAIHAMA_LINE_DRIVE_STATE || {};
...
async function main() {
  ...
  try {
    ...
    await page.goto(`http://127.0.0.1:${PORT}/index.html?nocache=r9drive`, {
      waitUntil: 'domcontentloaded',
      timeout: 60000,
    });
    await page.waitForFunction(
      () => window.MAIHAMA_LINE_ROUTE_V1 && window.MAIHAMA_LINE_PATH_V1,
      null,
      { timeout: 30000 },
    );
