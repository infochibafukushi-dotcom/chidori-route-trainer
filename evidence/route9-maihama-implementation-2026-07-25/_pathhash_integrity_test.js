'use strict';
/**
 * pathHash integrity for route-9 舞浜線: intact systems pass; in-memory mutation blocks driving.
...
async function main() {
  ...
  try {
    ...
    await page.goto(`http://127.0.0.1:${PORT}/index.html?nocache=r9pathhash`, {
      waitUntil: 'domcontentloaded',
      timeout: 60000,
    });
    await page.waitForFunction(
      () => window.MAIHAMA_LINE_ROUTE_V1 && window.MAIHAMA_LINE_PATH_V1 && window.MAIHAMA_LINE_PATH_POLICY_V1,
      null,
      { timeout: 30000 },
    );
    ...
    await page.selectOption('#routeSelect', 'route-9');
    ...
    report.tamper = await page.evaluate(async () => {
      ...
      const bank = window.MAIHAMA_LINE_PATH_V1[key];
      ...
      const live = data.routes.find((r) => r.id === 'route-9')?.systems?.[key];
      ...
      while (Date.now() < deadline) {
        ...
        if (statusText.includes('pathHash不一致') || statusText.includes('舞浜線の走行データを確認できません')) break;
      ...
      return {
        ...
        messageHasMaihamaBanner: String(threw || '').includes('舞浜線の走行データを確認できません')
          || statusText.includes('舞浜線の走行データを確認できません'),
        statusText,
        afterButton,
    ...
    const tamperOk = Boolean(
      ...
      && report.tamper?.messageHasMaihamaBanner
