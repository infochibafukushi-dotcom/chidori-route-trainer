'use strict';
/**
 * Keisei Bus Navi discovery scrape for 系統19 高洲南線 (route-19).
 ...
 *  - Require [19] / 【１９系統】 evidence on the course/trip before accepting as a candidate.
 *  - ★ 19 と 10 / 15 / 18 / 25 の分離が最重要。新浦安駅のりばF のセルは
 *    「[10]… [19]（東京学館前・高洲四丁目経由）高洲海浜公園・みなと南…」と 10 と 19 を同一セルに混載する。
...
const ROUTE_NUM = '19';
/** Systems that share stops / berths / cells with 19 and must be gated out later. */
...
const SEARCH_WORDS = [
  ...
  '浦安南高校', '特別養護老人ホーム', '高洲八丁目', 'みなと南', '潮音の街', '夢海の街',
];
...
async function listCourses(page, busstopId, label) {
  ...
  return {
    ...
    all,
    route19: all.filter((l) => hasRouteMark(l.text)),
...
function emptyReport() {
  return {
    ...
    lineName: '高洲南線',
    note: 'Route 19 discovery pass. 新浦安駅／高洲海浜公園／東京学館前／高洲四丁目 の [19] コース一覧を確定する。'
      + '新浦安駅のりばFでは 10 と 19 が同一セルに混載され、高洲海浜公園のりばでは 10/15/18/19 も混載されるため凡例ゲートを後段で必須とする。',
    knownIds: { ...KNOWN_IDS },
    searches: {},
    terminals: {},
    terminalPlan: [],
...
async function main() {
  ...
  try {
    ...
    const terminals = [
      ...
      { key: 'tokyoGakkan', label: '東京学館前', id: pick('東京学館前', /東京学館前/) },
      { key: 'takasu4chome', label: '高洲四丁目', id: pick('高洲四丁目', /高洲四丁目/) },
      { key: 'urayasuMinamiKoko', label: '浦安南高校', id: pick('浦安南高校', /浦安南高校/) },
    ].filter((t) => t.id);
    ...
    for (const term of terminals) {
      ...
      console.log('  route19 courses:', courses.route19.length, '/ sibling-only', courses.siblingOnly.length, '/ all', courses.all.length);
      for (const c of courses.all) {
        console.log('   [ALL]', c.berthLetter || c.berth, '|', (c.text || '').slice(0, 150));
      }
      for (const c of courses.route19) {
  ...
  console.log('=== ROUTE 19 COURSES ===');
  for (const [key, t] of Object.entries(report.terminals)) {
    for (const c of t.route19 || []) {
