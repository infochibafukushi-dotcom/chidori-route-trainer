 ...
 *  - 新浦安駅発着の短縮便2本は専用 relation が無いが、route-19 自身の relation だけで
...
const MAX_GAP_M = 30;
const MAX_JOIN_M = 1;
const GENERATED = '2026-07-26-takasu-minami-line-v1';
...
/**
 * Stops that belong exclusively to a sibling route. Their appearance in generated route-19
 * data is a build failure. Matching is exact — 高洲橋 / 高洲中央公園 / 高洲四丁目 are all
 * legitimate route-19 stops, so substring matching against 高洲 must never be used.
 */
...
function main() {
  ...
  for (const key of SYSTEM_ORDER) {
    ...
    for (const rid of relIds) {
      if (allForbiddenRelations.includes(rid)) {
        throw new Error(`${key}: relation ${rid} belongs to a sibling route and must never be used for route 19`);
      }
    }
  ...
  summary.reverseChecks = [
    { a: '19-takasu-seaside', b: '19-shinurayasu' },
  ...
  summary.spliceChecks = [
    { a: '19-takasu-seaside', b: '19-shinurayasu' },
    { a: '19-shinurayasu', b: '19-takasu-seaside' },
  ].map(({ a, b }) => {
  ...
  fs.writeFileSync(
    path.join(ROOT, 'takasu-minami-line-platforms-v1.js'),
    ...
    + `// Generated: ${GENERATED}\n`
    + `(() => {\n  window.TAKASU_MINAMI_LINE_PLATFORMS_V1 = ${JSON.stringify(platformsBank, null, 2)};\n})();\n`,
    'utf8',
  );
  fs.writeFileSync(
    path.join(ROOT, 'takasu-minami-line-path-v1.js'),
    '// Auto-generated OSM road path geometry for 高洲南線 系統19 (route-19).\n'
    ...
    + '// Own relations only: 18381771 新浦安駅⇒東京学館・高洲四丁目⇒高洲海浜公園 /\n'
    + '// 18381770 高洲海浜公園⇒高洲四丁目・東京学館⇒新浦安駅.\n'
    + '// NEVER reverse the outbound path for inbound, NEVER truncate route-10, NEVER reverse route-15,\n'
    + '// and NEVER reuse route-10/15/18/25 geometry (みなと南 / 潮音の街・高洲 / 夢海の街・高洲橋).\n'
    + `// Generated: ${GENERATED}\n`
    + `(() => {\n  window.TAKASU_MINAMI_LINE_PATH_V1 = ${JSON.stringify(pathBank, null, 2)};\n})();\n`,
    'utf8',
  );
  ...
  console.log('wrote takasu-minami-line-platforms-v1.js and takasu-minami-line-path-v1.js');
}
