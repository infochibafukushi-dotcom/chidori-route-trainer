...
/**
 * Stops that belong exclusively to a sibling system. Presence is an independent red flag,
 * never a gate on its own — the legend is always the decider.
 */
const SIBLING_EXCLUSIVE_STOPS = {
  10: ['みなと南', 'みなと第二', 'アライプロバンス', '鉄鋼団地入口'],
  // 15と19は明海交差点・入船橋を共有。15固有は潮音の街・高洲中央公園・高洲（停留所名そのもの）。
  15: ['潮音の街', '高洲中央公園', '高洲'],
...
/** Symbol prefix of a timetable cell, e.g. "41ゆ" -> "ゆ", "05★た" -> "★た", "37" -> "" (無印). */
...
/** Parse legend lines of the form "ゆ…【１８系統】夢海の街、高洲橋、潮音の街経由　高洲海浜公園行き" */
...
function siblingStopsPresent(stopNames) {
  const out = {};
  for (const [num, stops] of Object.entries(SIBLING_EXCLUSIVE_STOPS)) {
    const hits = stops.filter((n) => stopNames.includes(n));
    if (hits.length) out[num] = hits;
  }
  return out;
}
...
async function main() {
  ...
  for (const [sigKey, sig] of Object.entries(DEEP.signatures)) {
    ...
    const depIs18 = depProof.resolved && depProof.routeNumber === ROUTE_NUM;
    const listingContradicts = listingRouteNumbers.length > 0 && !listingRouteNumbers.includes(ROUTE_NUM);
    let verdict;
    if (depIs18 && !listingContradicts) verdict = `ACCEPT-route${ROUTE_NUM}`;
    ...
    result.signatures.push({
      ...
      siblingExclusiveStopsPresent: siblingStopsPresent(sig.stopNames),
  ...
  for (const s of result.signatures) {
    ...
    lines.push(`    sibling-exclusive stops: ${JSON.stringify(s.siblingExclusiveStopsPresent)} | route19-characteristic: ${s.route19CharacteristicStopsPresent.join(',') || '(none)'}`);
