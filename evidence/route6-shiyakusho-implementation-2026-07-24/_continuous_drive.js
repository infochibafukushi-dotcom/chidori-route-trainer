'use strict';
/**
 * Continuous Start→end drive for route-6 市役所線 (5 systems) + basic regression hashes.
 */
...
function startServer() {
  return new Promise((resolve, reject) => {
    const server = http.createServer((req, res) => {
      ...
      if (rel.startsWith('shiyakusho-route-v1.js')) {
...
async function readSys(page, key) {
  return page.evaluate((systemKey) => {
    const route = window.SHIYAKUSHO_ROUTE_V1?.ensureRoute?.();
    const sys = route?.systems?.[systemKey];
    const st = window.SHIYAKUSHO_DRIVE_STATE || {};
...
async function main() {
  ...
  try {
    ...
    await page.goto(`http://127.0.0.1:${PORT}/index.html?nocache=r6`, {
      waitUntil: 'domcontentloaded',
      timeout: 60000,
    });
    await page.waitForFunction(
      () => window.SHIYAKUSHO_ROUTE_V1 && window.SHIYAKUSHO_PATH_V1,
      null,
      { timeout: 30000 },
    );
    ...
    await page.waitForTimeout(800);
    report.regression.route5 = await page.evaluate(() => {
      const r = data.routes.find((x) => x.id === 'route-5');
      const out = {};
      for (const [k, s] of Object.entries(r?.systems || {})) {
        out[k] = { path: s.path?.length, pathHash: s.pathHash, resolvedVersion: s.resolvedVersion, stops: s.stops?.length };
      }
    ...
    await page.screenshot({ path: path.join(OUT, 'screenshots', 'ui-pc-route6.png') });
    await context.close();
    ...
    await spPage.goto(`http://127.0.0.1:${PORT}/index.html?nocache=r6sp`, { waitUntil: 'domcontentloaded' });
    ...
    await spPage.screenshot({ path: path.join(OUT, 'screenshots', 'ui-sp390-route6.png') });
    await sp.close();
  } finally {
    await browser.close();
    server.close();
