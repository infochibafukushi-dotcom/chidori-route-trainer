...
const EXPECTED_CACHE = 'chidori-route-map-v114';
const ROUTE23_PACK = [
  './urayasu-higashi-danchi-line-23-stop-images-v1.css?v=114',
...
(async () => {
  ...
  const report = { routeId: 'route-23', expectedCache: EXPECTED_CACHE, failures: [], pass: false, d1PutCount: 0 };
  ...
  try {
    ...
    await page.goto(`http://127.0.0.1:${PORT}/index.html?nocache=r23pwa`, { waitUntil: 'domcontentloaded', timeout: 60000 });
    await page.waitForTimeout(1500);
    await page.evaluate(() => { if (typeof go === 'function') go('routes'); });
    await page.selectOption('#routeSelect', 'route-23');
    await page.waitForFunction(() => window.URAYASU_HIGASHI_DANCHI_LINE_23_ROUTE_V1 && window.URAYASU_HIGASHI_DANCHI_LINE_23_PATH_V1, null, { timeout: 30000 });
    await page.waitForTimeout(1000);
    await page.screenshot({ path: path.join(SHOTS, 'route23-pc1280.png'), fullPage: false });
    const cacheInfo = await page.evaluate(async (pack) => {
      const names = await caches.keys();
      const cache = await caches.open(names.find((n) => n.includes('v114')) || names[0]);
    ...
    if (!String(report.loaderTag).includes('v=114')) report.failures.push('loader not v114');
    await ctx.setOffline(true);
    await page.reload({ waitUntil: 'domcontentloaded' });
    await page.waitForTimeout(1200);
    await page.evaluate(() => { if (typeof go === 'function') go('routes'); });
    await page.selectOption('#routeSelect', 'route-23');
    await page.waitForFunction(() => window.URAYASU_HIGASHI_DANCHI_LINE_23_ROUTE_V1, null, { timeout: 20000 });
    const offlineHashes = await page.evaluate((keys) => {
      const route = window.URAYASU_HIGASHI_DANCHI_LINE_23_ROUTE_V1.ensureRoute();
      return Object.fromEntries(keys.map((k) => [k, route.systems[k]?.pathHash || window.URAYASU_HIGASHI_DANCHI_LINE_23_PATH_V1[k]?.pathHash]));
    }, SYSTEMS);
