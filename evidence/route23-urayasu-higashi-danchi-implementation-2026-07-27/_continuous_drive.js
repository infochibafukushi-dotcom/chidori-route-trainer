'use strict';
/**
 * Continuous drive + integrity for route-23 浦安東団地線.
 */
...
const SIBLING_EXACT = ['浦安駅入口', 'シンボルロード・パークシティ', 'ベイパーク', 'ベイモール'];
...
function startServer() {
  return new Promise((resolve, reject) => {
    const server = http.createServer((req, res) => {
      ...
      if (rel.startsWith('urayasu-higashi-danchi-line-23-route-v1.js')) body = Buffer.from(speedUp(body.toString('utf8')), 'utf8');
      if (rel === 'index.html') {
...
async function readSys(page, key) {
  return page.evaluate((systemKey) => {
    const api = window.URAYASU_HIGASHI_DANCHI_LINE_23_ROUTE_V1;
    const route = api?.ensureRoute?.();
    const sys = route?.systems?.[systemKey];
    const st = window.URAYASU_HIGASHI_DANCHI_LINE_23_DRIVE_STATE || {};
...
(async () => {
  ...
  try {
    ...
    await page.goto(`http://127.0.0.1:${PORT}/index.html?nocache=r23`, { waitUntil: 'domcontentloaded', timeout: 60000 });
    await page.waitForTimeout(1200);
    await page.evaluate(() => { if (typeof go === 'function') go('routes'); });
    await page.waitForSelector('#routeSelect', { timeout: 15000 });
    await page.selectOption('#routeSelect', 'route-23');
    await page.waitForFunction(() => window.URAYASU_HIGASHI_DANCHI_LINE_23_ROUTE_V1 && window.URAYASU_HIGASHI_DANCHI_LINE_23_PATH_V1 && window.URAYASU_HIGASHI_DANCHI_LINE_23_PATH_POLICY_V1, null, { timeout: 30000 });
    await page.evaluate(() => { routeState.routeId = 'route-23'; if (typeof routes === 'function') routes(); });
    await page.waitForSelector('#driveStartPause', { timeout: 60000 });
    await page.waitForTimeout(900);
    await page.evaluate(async () => { await window.URAYASU_HIGASHI_DANCHI_LINE_23_ROUTE_V1.resolveAllSystems(true); });
    ...
    for (const key of SYSTEMS) {
      ...
      const runtime = await page.evaluate(async (k) => {
        ...
        const validation = policy.validateRuntimePath({
          ...
          expectedPathHash: window.URAYASU_HIGASHI_DANCHI_LINE_23_PATH_V1[k].pathHash,
    ...
    // Regression: route-3 pathHashes unchanged vs HEAD (4abd686)
    await page.selectOption('#routeSelect', 'route-3');
    await page.waitForFunction(() => window.URAYASU_HIGASHI_DANCHI_PATH_V1, null, { timeout: 30000 });
    const r3Ok = await page.evaluate(() => {
      ...
      return Object.entries(EXPECTED).every(([k, h]) => bank[k]?.pathHash === h);
    });
    report.regressions = report.regressions || {};
    report.regressions['route-3-pathHashes'] = r3Ok;
    if (!r3Ok) report.failures.push('route-3 pathHashes changed');
  ...
  fs.writeFileSync(path.join(OUT, 'continuous-drive-report.json'), JSON.stringify(report, null, 2));
  fs.writeFileSync(path.join(OUT, 'regression-report.json'), JSON.stringify({ regressions: report.regressions, pass: report.pass, failures: report.failures }, null, 2));
