'use strict';
/**
 * Keisei Bus Navi discovery scrape for 系統23 浦安東団地線（舞浜⇔総合公園） (route-23).
 ...
 *  - Require [23] / 【２３系統】 evidence before accepting as a candidate.
 *  - Separate from route-3 (浦安駅入口 terminal) which shares 新浦安 berth H (3 [23] co-listed).
 *
...
const SEARCH_WORDS = [
  '舞浜駅', '総合公園', '新浦安駅', '入船中央エステート', '明海大学前',
  '海風の街', '夢海の街', '望海の街', '明海五丁目', 'ベイサイドホテルエリア',
  '消防本部前', '浦安駅入口',
];
...
async function main() {
  ...
  try {
    ...
    const idByKey = {
      ...
      sogo: KNOWN_IDS.sogo || pick('総合公園', /^総合公園/),
    ...
    for (const term of terminals) {
      ...
      console.log('  route23:', courses.route23.length, '/ mixed:', courses.mixedWithSiblings?.length || 0, '/ sibling-only:', courses.siblingOnly.length, '/ all:', courses.all.length);
      for (const c of courses.all) {
        console.log('   [ALL]', c.berthLetter || c.berth, '| 23=', c.has23, '| sib=', (c.siblingsInCell || []).join(','), '|', (c.text || '').slice(0, 160));
      }
      for (const c of courses.route23) {
      ...
      for (const c of courses.siblingOnly) {
        if (/\[3\]|【\s*3\s*系統\s*】|^3\s*\[3\]/.test(toAsciiDigits(c.text || ''))) {
          report.rejectedSiblingCells.push({
            ...
            courseText: (c.text || '').slice(0, 300), href: c.absHref, gatedRoute: '3',
          });
        }
      }
    ...
    report.stats = {
      ...
      route23Courses: Object.values(report.terminals).reduce((n, t) => n + (t.route23?.length || 0), 0),
      mixedCells: report.berthsSeen.filter((b) => b.mixedCell).length,
      sibling3Cells: report.rejectedSiblingCells.length,
  ...
  console.log('=== ROUTE 23 COURSES ===');
  for (const [key, t] of Object.entries(report.terminals)) {
    for (const c of t.route23 || []) {
