'use strict';
/**
 * Build platforms + pathPoints for route-23 浦安東団地線（23系統・舞浜⇔総合公園） from OSM relations.
 *
 * Rules:
 *  - Stop order comes from official-stop-orders.json (navi). Never reversed/truncated.
 *  - Dedicated-relation systems (18396547 / 18396546) are built independently.
 *  - route-3 relation 18417570/18417571/18417579 は使用禁止。
 *
 * Writes urayasu-higashi-danchi-line-23-platforms-v1.js and urayasu-higashi-danchi-line-23-path-v1.js at repo root,
...
const MAX_GAP_M = 30;
const MAX_JOIN_M = 1;
const GENERATED = '2026-07-27-urayasu-higashi-danchi-line-23-v1';
...
/**
 * Stops that belong exclusively to route-20. Their appearance in generated route-22
 * data is a build failure.
 */
...
function main() {
  ...
  const summary = {
    ...
    generated: GENERATED,
    policy: {
      stopOrderSource: 'official-stop-orders.json（京成バスナビ個別便通過時刻表・2段凡例ゲートで【２３系統】に確定）',
      pathSource: 'OSM route relation way members（方向補正あり）。18419895／18419894 を独立構築',
      googleDirectionsUsed: false,
      reverseReuseForbidden: true,
      truncationReuseForbidden: true,
      siblingRouteSeparation: 'route-3 relations 18417570/18417571/18417579 は使用禁止',
  ...
  for (const key of SYSTEM_ORDER) {
    ...
    for (const rid of relIds) {
      if (allForbiddenRelations.includes(rid)) {
        throw new Error(`${key}: relation ${rid} belongs to a sibling route and must never be used for route 23`);
      }
    }
  ...
  summary.reverseChecks = [
    { a: '23-maihama-sogo', b: '23-sogo-maihama' },
  ...
  summary.spliceChecks = [
    { a: '23-maihama-sogo', b: '23-sogo-maihama' },
    { a: '23-sogo-maihama', b: '23-maihama-sogo' },
  ].map(({ a, b }) => {
  ...
  fs.writeFileSync(
    path.join(ROOT, 'urayasu-higashi-danchi-line-23-platforms-v1.js'),
    ...
    + `// Generated: ${GENERATED}\n`
    + `(() => {\n  window.URAYASU_HIGASHI_DANCHI_LINE_23_PLATFORMS_V1 = ${JSON.stringify(platformsBank, null, 2)};\n})();\n`,
    'utf8',
  );
  fs.writeFileSync(
    path.join(ROOT, 'urayasu-higashi-danchi-line-23-path-v1.js'),
    '// Auto-generated OSM road path geometry for 浦安東団地線 系統23 (route-23 / 舞浜⇔総合公園).\n'
    + '// Paths follow OSM route relation way members (direction-corrected). Google Directions not used.\n'
    + '// Own relations only: 18419895 舞浜駅⇒総合公園 / 18419894 総合公園⇒舞浜駅.\n'
    + '// NEVER use route-3 relations or reverse one system for the other.\n'
    + `// Generated: ${GENERATED}\n`
    + `(() => {\n  window.URAYASU_HIGASHI_DANCHI_LINE_23_PATH_V1 = ${JSON.stringify(pathBank, null, 2)};\n})();\n`,
    'utf8',
  );
  ...
  console.log('wrote urayasu-higashi-danchi-line-23-platforms-v1.js and urayasu-higashi-danchi-line-23-path-v1.js');
}
