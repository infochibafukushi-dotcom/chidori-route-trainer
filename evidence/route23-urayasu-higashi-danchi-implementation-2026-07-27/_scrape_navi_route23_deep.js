'use strict';
/**
 * Deep pass: open EVERY 通過時刻表 (/stops?) link on route-23 course timetables.
 * 千鳥車庫のりば02 / 舞浜駅のりば01 等は [20]/[22]/[2]/[4]/[14] 混載セルがある。
 * 候補を広く拾い、系統確定は _verify_signatures.js の二段凡例ゲートに委ねる。
 *
...
const ROUTE_NUM = '23';
const SIBLING_ROUTES = ['3', '11', '18', '6', '20', '22', '25', '9'];
...
function detectOtherRouteNumber(text) {
  ...
  for (const n of SIBLING_ROUTES) {
    if (new RegExp(`\\[${n}\\]|【\\s*${n}\\s*系統\\s*】|^${n}千鳥|^${n}\\s*\\[`).test(t)) return n;
  }
...
async function collectTripLinks(page, timetableUrl) {
  ...
  return page.evaluate(() => {
    ...
    body.split(/\n/).forEach((line) => {
      ...
      if (/…|･･･/.test(t) && /系統|行き|止まり|経由|車庫|循環|急行|直通/.test(t) && !legend.includes(t)) legend.push(t);
    });
...
async function listCourses(page, busstopId) {
  ...
  return { busstopId, coursesUrl, all, route22: all.filter((l) => hasRouteMark(l.text)) };
}
...
const out = {
  ...
  lineName: '若潮通り線',
  note: 'Exhaustive per-course trip enumeration for route-23. Mixed cells with [3] at 新浦安 berth H require legend gate.',
  courses: [],
  timetables: [],
  signatures: {},
  trips: [],
...
function recordSignature(trip, link, ttMeta) {
  ...
  if (sig.sampleUrls.length < 8) sig.sampleUrls.push(link.absHref);
  const t = (trip.stops[0] || {}).time;
  if (t && !sig.departureTimes.includes(t)) sig.departureTimes.push(t);
  if (link.cellText && sig.cellTexts.length < 8 && !sig.cellTexts.includes(link.cellText)) sig.cellTexts.push(link.cellText);
}
...
async function processTimetable(page, ttMeta) {
  ...
  console.log('TT', ttMeta.terminal, ttMeta.dayLabel, 'berth', ttMeta.berth, 'links', links.length);
  ...
  for (let i = 0; i < links.length; i++) {
    ...
    if (!confirmed) {
      out.rejected.push({ url: link.absHref, cellText: link.cellText, reason: 'no-22-evidence', stopNames: raw.stopNames });
      continue;
    }

    const paired = pairStopIds(raw.stopNames, raw.busstopLinks);
    const trip = { ...raw, ...paired, dayLabel: ttMeta.dayLabel, terminal: ttMeta.terminal, berth: ttMeta.berth, legend: meta.legend };
    const s = sigOf(trip.stopNames);
...
async function main() {
  ...
  try {
    ...
    for (const stopName of FOLLOW_UP_STOPS) {
      if (pastHardTimeout()) break;
      const id = out.discoveredBusstopIds[stopName] || prev.knownIds?.[Object.keys(prev.knownIds).find((k) => prev.terminals?.[k]?.label === stopName)] || null;
      out.followUps[stopName] = { busstopId: id, courses: null, route22Courses: null, note: null };
      if (!id) {
      ...
      out.followUps[stopName].route22Courses = c.route22.map((x) => ({ text: x.text, berth: x.berth, absHref: x.absHref }));
      if (!c.route22.length) {
        out.followUps[stopName].note = `${stopName}発の[20]公開便コースは見つからなかった。`;
        continue;
      }
      for (const course of c.route22) {
        for (const day of DATES) {
          if (pastHardTimeout()) break;
          await processTimetable(page, {
