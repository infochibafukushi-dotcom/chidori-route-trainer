'use strict';
/** Fetch OSM relations for route-38 明海クオン線. Probe return direction. */
const fs = require('fs');
...
const IDS = [18396354];
const LABELS = {
  18396354: 'route38 新浦安駅⇒クオン新浦安線',
};
const UA = 'chidori-route-trainer/route38-akemi-quon-research';
...
async function probeQuonNetwork() {
  const q = `[out:json][timeout:120];
relation["route"="bus"]["network"~"クオン"](35.61,139.85,35.68,139.96);
out tags;`;
  try {
    const json = await overpass(q);
    return (json.elements || []).map((e) => ({ id: e.id, type: e.type, tags: e.tags }));
...
async function main() {
  const summary = {
    ...
    route: 'akemi-quon-route38',
    lineName: '明海クオン線',
    knownPathSourceIds: IDS,
    separationGuard: 'ref=18/15/19/10 relations listed but NEVER used for route-38 geometry.',
    relations: [],
    refProbe38: [],
    quonNetworkProbe: [],
  };
  ...
  console.log('probe ref=38');
  summary.refProbe38 = await probeRefRelations('38');
  console.log('probe network=クオン');
  summary.quonNetworkProbe = await probeQuonNetwork();

  for (const relSum of summary.relations.filter((r) => r.ok)) {
    console.log('---', relSum.id, relSum.name);
    console.log('platforms:', (relSum.platformNames || []).join(' > '));
  }
