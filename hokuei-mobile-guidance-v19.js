(() => {
  ...
  const setSelectedCode = (code) => {
    localStorage.setItem(SYSTEM_KEY, code === '1-5' ? '1' : code);
  };
  ...
  function positionAtDistance(path, metrics, distance) {
    ...
    const length = metrics.cumulative[index] - before || 1;
    const ratio = (distance - before) / length;
  ...
  function mapPointsToPath(points, path, metrics) {
    let minimumIndex = 0;
    return points.map((point) => {
      const result = nearestPathDistance(point, path, metrics, minimumIndex);
  ...
  async function drawGuidance(route, system, token) {
    ...
    activeStops.forEach((stop) => {
      ...
      marker.addListener('click', () => {
        ...
        statusNode.textContent = `${stop.originalIndex + 1}. ${displayName(stop)}｜手動設定位置`;
      });
      markers.push(marker);
    });

    if (!activeStops.length) {
      statusNode.textContent = '緯度・経度が設定された停留所がありません。';
      cleanup = () => markers.forEach((marker) => marker.setMap(null));
      return;
    }

    if (activeStops.length === 1) {
      statusNode.textContent = `${displayName(activeStops[0])}のみ位置設定済みです。2か所以上でルートを生成します。`;
      cleanup = () => markers.forEach((marker) => marker.setMap(null));
      return;
    }
    ...
    const stopDistances = mapPointsToPath(activeStops, result.path, metrics);
    stopDistances[0] = 0;
    stopDistances[stopDistances.length - 1] = metrics.total;
    const turnDistances = mapPointsToPath(result.turns.map((turn) => turn.point), result.path, metrics);
    const turns = result.turns.map((turn, index) => ({ ...turn, routeDistance: turnDistances[index] }));
    ...
    stationTelop.className = 'station-name-telop station-name-telop-v19';
    street.appendChild(stationTelop);
    const turnGuide = document.createElement('div');
    turnGuide.className = 'driving-turn-guide driving-turn-guide-v19';
    ...
    const updateTurnGuide = () => {
      ...
      turnGuide.innerHTML = `<strong><span class="turn-arrow">${nextTurn.arrow}</span><span>${distanceText} ${nextTurn.label}</span></strong><span>${esc(nextTurn.instruction)}</span>`;
    };
    ...
    const update = (now, forceVisual = false) => {
      ...
      progress.textContent = `時速20km${dwellUntil ? '｜停車中' : ''}｜前：${previousStop}｜現在：${currentStop}｜次：${nextStop}`;
      updateButtons();
    };
    ...
    system.pathSource = '緯度・経度が登録された停留所を順番に通るGoogle Maps道路ルート';
    ...
    showStationTelop(0, DWELL_MS);
    update(performance.now(), true);
    statusNode.textContent = `系統${system.code}｜位置登録済み停留所は3秒停車｜右左折は${TURN_NOTICE_METERS}m手前から案内｜地図は約20m尺度`;
  ...
  routes = function routesMobileGuidanceV19() {
    ...
    shell(`<section>
      ...
      <section class="manual-mode-card mobile-guidance-summary">
        <div><strong>ルート案内モード</strong><span>時速20km・位置登録済み停留所で3秒停車・停留所名テロップ・右左折の距離案内・地図約20m尺度｜位置設定 ${positioned}/${system.stops.length}件</span></div>
        <button id="manualSettings" class="secondary" type="button">設定画面で位置を修正</button>
      </section>
      <div class="split mobile-guidance-split">
        <div class="mobile-map-wrap"><div id="routeMap" class="map mobile-guidance-map"></div><div class="map-zoom-badge">約20m尺度</div></div>
        <div id="street" class="street mobile-guidance-street"></div>
      </div>
  ...
  window.HOKUEI_MOBILE_GUIDANCE_V19 = {
    speedKmh: SPEED_KMH,
