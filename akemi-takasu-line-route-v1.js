// 明海・高洲線（系統番号18・route-18）走行シミュレーション／停留所編集モジュール。
// 停留所順：京成バスナビ個別便通過時刻表で確認（evidence/route18-akemi-takasu-implementation-2026-07-26/official-stop-orders.json）。
...
(() => {
  ...
  const SYSTEM_RESOLVED_VERSIONS = {
    '18-takasu-seaside': '2026-07-26-akemitakasu18-takasu-seaside-v1',
    '18-urayasu-eki-iriguchi': '2026-07-26-akemitakasu18-urayasu-eki-iriguchi-v1',
    '18-takasu-kita-shogakko': '2026-07-26-akemitakasu18-takasu-kita-shogakko-v1',
    '18-takasu-seaside-from-shinurayasu': '2026-07-26-akemitakasu18-takasu-seaside-from-shinurayasu-v1',
    '18-shinurayasu-from-takasu': '2026-07-26-akemitakasu18-shinurayasu-from-takasu-v1',
  };
  ...
  const NAMES_18_URAYASU_EKI_IRIGUCHI = [
    "高洲海浜公園",
    "潮音の街",
    "高洲中央公園",
    "高洲橋",
  ...
  const NAMES_18_TAKASU_KITA_SHOGAKKO = [
    "新浦安駅",
    "入船中央エステート",
    "明海大学前",
    "海風の街",
  ...
  const NAMES_18_TAKASU_SEASIDE_FROM_SHINURAYASU = [
    "新浦安駅",
    "入船中央エステート",
    "明海大学前",
    "海風の街",
  ...
  const NAMES_18_SHINURAYASU_FROM_TAKASU = [
    "高洲海浜公園",
    "潮音の街",
    "高洲中央公園",
    "高洲橋",
  ...
  const SYSTEM_DEFINITIONS = {
    '18-takasu-seaside': {
      key: '18-takasu-seaside', displayCode: DISPLAY_CODE, directionGroup: 'outbound',
      title: '高洲海浜公園行き（浦安駅入口発）',
      summary: '浦安駅入口 → 消防本部前 → 明海大学前 → 夢海の街 → 高洲橋 → 高洲中央公園 → 潮音の街 → 高洲海浜公園',
      relationId: 18352908,
      timetableSymbol: 'ゆ',
      naviCourse: '0008200286',
      names: NAMES_18_TAKASU_SEASIDE,
    },
    '18-urayasu-eki-iriguchi': {
      key: '18-urayasu-eki-iriguchi', displayCode: DISPLAY_CODE, directionGroup: 'inbound',
      title: '浦安駅入口行き（高洲海浜公園発）',
      summary: '高洲海浜公園 → 潮音の街 → 高洲中央公園 → 高洲橋 → 夢海の街 → 明海大学前 → 消防本部前 → 浦安駅入口',
      relationId: 18352907,
      timetableSymbol: 'う',
      naviCourse: '0008200285',
      names: NAMES_18_URAYASU_EKI_IRIGUCHI,
    },
    '18-takasu-kita-shogakko': {
      key: '18-takasu-kita-shogakko', displayCode: DISPLAY_CODE, directionGroup: 'night-shortturn',
      title: '高洲北小学校行き（新浦安駅発・夜間／深夜）',
      summary: '新浦安駅 → 明海大学前 → 夢海の街 → 高洲橋 → 高洲中央公園 → 潮音の街 → 高洲四丁目 → 東京学館前 → 高洲北小学校',
      relationId: 18417590,
      timetableSymbol: 'た',
      naviCourse: '0008200289',
      names: NAMES_18_TAKASU_KITA_SHOGAKKO,
    },
    '18-takasu-seaside-from-shinurayasu': {
      key: '18-takasu-seaside-from-shinurayasu', displayCode: DISPLAY_CODE, directionGroup: 'outbound-shortturn',
      title: '高洲海浜公園行き（新浦安駅発）',
      summary: '新浦安駅 → 明海大学前 → 夢海の街 → 高洲橋 → 高洲中央公園 → 潮音の街 → 高洲海浜公園',
      relationId: null,
      timetableSymbol: 'ゆ',
      naviCourse: '0008200288',
      names: NAMES_18_TAKASU_SEASIDE_FROM_SHINURAYASU,
    },
    '18-shinurayasu-from-takasu': {
      key: '18-shinurayasu-from-takasu', displayCode: DISPLAY_CODE, directionGroup: 'inbound-shortturn',
      title: '新浦安駅行き（高洲海浜公園発）',
      summary: '高洲海浜公園 → 潮音の街 → 高洲中央公園 → 高洲橋 → 夢海の街 → 明海大学前 → 新浦安駅',
      relationId: null,
      timetableSymbol: 'ゆ',
      naviCourse: '0008200287',
      names: NAMES_18_SHINURAYASU_FROM_TAKASU,
    },
  };

  const DEFAULT_SYSTEM_KEY = '18-takasu-seaside';
  ...
  function platformsForSystem(systemKey) {
    const bank = window.AKEMI_TAKASU_LINE_PLATFORMS_V1 || {};
    return bank[systemKey] || {};
  }

  function pathDataForSystem(systemKey) {
    const bank = window.AKEMI_TAKASU_LINE_PATH_V1 || {};
    return bank[systemKey] || null;
  }

  /** 系統キー単位の停留所IDへ揃える。 */
  function migrateStopId(definition, stop, index) {
    const newId = `akemi-takasu-${definition.key}-${String(index + 1).padStart(2, '0')}`;
    if (stop.id === newId) return false;
    if (!stop.id || /^akemi-takasu-18-[\w-]+-\d{2}$/.test(stop.id)) {
      stop.id = newId;
      return true;
    }
    return false;
  ...
  function makeStop(definition, name, index) {
    ...
    return {
      id: `akemi-takasu-${definition.key}-${String(index + 1).padStart(2, '0')}`,
      name,
  ...
  function ensureRoute() {
    ...
    const systems = {};
    let changed = route.akemiTakasuLineVersion !== VERSION;
    ...
    route.akemiTakasuLineVersion = VERSION;
    ...
    if (!route.akemiTakasuLineStopImages) {
      route.akemiTakasuLineStopImages = {};
      changed = true;
    }
  ...
  async function resolveSystem(key, { force = false, statusCallback = null } = {}) {
    ...
    if (!route || !definition || !system) throw new Error('明海・高洲線の系統データがありません。');
  ...
  async function resolveAllSystems(force = false, statusCallback = null) {
    const order = ['18-takasu-seaside', '18-urayasu-eki-iriguchi', '18-takasu-kita-shogakko', '18-takasu-seaside-from-shinurayasu', '18-shinurayasu-from-takasu'];
  ...
  function migrateStopImageBank(route) {
    const bank = route.akemiTakasuLineStopImages;
  ...
  function showStopImage(route, definition, stop, street) {
    ...
    const existing = street.querySelector('.stop-image-display-v25.akemiTakasuLine-stop-image');
    existing?.remove();
    if (!entry?.dataUrl) return false;
    const overlay = document.createElement('div');
    overlay.className = 'stop-image-display-v25 akemiTakasuLine-stop-image';
  ...
  function hideStopImage(street) {
    street?.querySelector('.akemiTakasuLine-stop-image')?.remove();
  }
  ...
  async function drawGuidance(route, definition, system, token) {
    ...
    try {
      ...
      const message = base.includes('明海・高洲線の走行データを確認できません')
        ? base
        : [
          '明海・高洲線の走行データを確認できません。',
          `系統：${definition.key}`,
    ...
    const vehicle = new googleApi.maps.Marker({ map, position: center, icon: busIcon(googleApi), zIndex: 1000, title: `明海・高洲線 ${definition.title}` });
    ...
    const publishDebugState = () => {
      window.AKEMI_TAKASU_LINE_DRIVE_STATE = {
        ...simulation,
        ...
        try {
          streetViewService.getPanorama(request, (data, status) => {
            try {
              ...
              console.warn('[akemiTakasuLine] panorama update failed', error);
            }
          });
        } catch (error) {
          console.warn('[akemiTakasuLine] getPanorama failed', error);
          applyPovOnly();
        }
        return true;
      }
    ...
    const updateDrivingVisual = (now, force = false) => {
      ...
      if (!point?.position || !Number.isFinite(point.position.lat) || !Number.isFinite(point.position.lng)) {
        ...
        console.error('[akemiTakasuLine] invalid drive position', point, simulation);
        return;
      }
    ...
    const failStart = (reason) => {
      ...
      console.error('[akemiTakasuLine] drive start failed', reason, {
        systemKey: definition.key,
        pathLength: path.length,
        metricsTotal: metrics.total,
    ...
    status.textContent = `明海・高洲線｜系統${DISPLAY_CODE}｜${definition.title}｜${stops.length}停留所｜道路形状：OSM relation採用（要走行確認）`;
    publishDebugState();
    cleanupGuidance = () => {
      simulation.running = false;
      simulation.paused = false;
      ...
      telop.remove();
      hideStopImage(street);
      if (window.AKEMI_TAKASU_LINE_DRIVE_STATE?.systemKey === definition.key) {
        window.AKEMI_TAKASU_LINE_DRIVE_STATE = null;
      }
    };
  }

  routes = function routesAkemiTakasuLineV1() {
    const route = ensureRoute();
    ...
    try { window.HOKUEI_GUIDANCE_V22?.cleanup?.(); } catch (error) {
      console.warn('[akemiTakasuLine] hokuei cleanup failed', error);
    }
    ...
    shell(`<section class="guidance-page-v26">
      ...
      <div class="split guidance-v22-split">
        <div class="guidance-map-wrap-v22"><div id="routeMap" class="map guidance-map-v22"></div><div class="guidance-version-v22">明海・高洲線・系統${DISPLAY_CODE}</div></div>
        <div id="street" class="street guidance-street-v22"></div>
      </div>
      <p id="mapStatus" class="status" hidden aria-hidden="true">明海・高洲線を準備しています…</p>
    </section>`);
    ...
    drawGuidance(route, definition, system, token).catch((error) => {
      ...
      if (node) {
        ...
        node.textContent = message.includes('明海・高洲線の走行データを確認できません') || message.includes('系統')
          ? message
          : (`系統=${key} | ${message}`);
      }
      console.error('[akemiTakasuLine] drawGuidance failed', definition.key, error);
    });
  };
  ...
  function openRoute18StopEditor(systemKey, stopIndex) {
    closeEditorDialog();
    const route = ensureRoute();
    ...
    const currentImage = route.akemiTakasuLineStopImages?.[bankKey]?.dataUrl || '';
    ...
    dialog.innerHTML = `<section class="stop-edit-dialog" role="dialog" aria-modal="true">
      <div class="stop-edit-header"><div><h2>明海・高洲線 停留所設定</h2><p>${DISPLAY_CODE}｜${escHtml(definition.title)}・${stopIndex + 1} / ${system.stops.length}</p></div><button type="button" id="closeStopEditor" class="stop-edit-close">×</button></div>
    ...
    const saveCurrent = () => {
      ...
      if (!route.akemiTakasuLineStopImages) route.akemiTakasuLineStopImages = {};
      if (state.removeImage) delete route.akemiTakasuLineStopImages[bankKey];
      else if (state.dataUrl) route.akemiTakasuLineStopImages[bankKey] = { dataUrl: state.dataUrl, updatedAt: new Date().toISOString(), sourceSystem: systemKey, sourceStopId: stop.id };
      route.akemiTakasuLineStopImageUpdatedAt = new Date().toISOString();
      save();
      return true;
    };
    ...
    document.getElementById('previousStopEdit').onclick = () => { if (!saveCurrent()) return; close(); openRoute18StopEditor(systemKey, stopIndex - 1); };
    document.getElementById('nextStopEdit').onclick = () => { if (!saveCurrent()) return; close(); openRoute18StopEditor(systemKey, stopIndex + 1); };
  ...
  function renderAkemiTakasuStopList() {
    const route = ensureRoute();
    ...
    box.querySelectorAll('[data-uhd-edit]').forEach((button) => { button.onclick = () => openRoute18StopEditor(definition.key, Number(button.dataset.uhdEdit)); });
  }

  function route18StopEditor() {
    ensureRoute();
    editorRouteId = ROUTE_ID;
  ...
  stopEditor = function stopEditorAkemiTakasuLineV1() {
