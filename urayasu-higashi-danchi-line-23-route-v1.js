// 浦安東団地線（系統番号23・route-23）走行シミュレーション／停留所編集モジュール。
// 停留所順：京成バスナビ個別便通過時刻表で確認（evidence/route23-urayasu-higashi-danchi-implementation-2026-07-27/official-stop-orders.json）。
...
(() => {
  ...
  const SYSTEM_RESOLVED_VERSIONS = {
    '23-maihama-sogo': '2026-07-27-urayasu23-maihama-sogo-v1',
    '23-sogo-maihama': '2026-07-27-urayasu23-sogo-maihama-v1',
  };
  ...
  const SYSTEM_DEFINITIONS = {
    '23-maihama-sogo': {
      key: '23-maihama-sogo', displayCode: DISPLAY_CODE, directionGroup: 'outbound',
      title: '総合公園行き（舞浜駅発）',
      summary: '舞浜駅 → 新浦安駅 → 明海 → 総合公園',
      relationId: 18419895,
      timetableSymbol: '',
      naviCourse: '0008200303',
      names: NAMES_23_MAIHAMA_SOGO,
    },
    '23-sogo-maihama': {
      key: '23-sogo-maihama', displayCode: DISPLAY_CODE, directionGroup: 'inbound',
      title: '舞浜駅行き（総合公園発）',
      summary: '総合公園 → 明海 → 新浦安駅 → 舞浜駅',
      relationId: 18419894,
      timetableSymbol: '',
      naviCourse: '0008200302',
      names: NAMES_23_SOGO_MAIHAMA,
    },
  };

  const DEFAULT_SYSTEM_KEY = '23-maihama-sogo';
  ...
  function platformsForSystem(systemKey) {
    const bank = window.URAYASU_HIGASHI_DANCHI_LINE_23_PLATFORMS_V1 || {};
  ...
  function pathDataForSystem(systemKey) {
    const bank = window.URAYASU_HIGASHI_DANCHI_LINE_23_PATH_V1 || {};
    return bank[systemKey] || null;
  }

  /** 系統キー単位の停留所IDへ揃える。 */
  function migrateStopId(definition, stop, index) {
    const newId = `urayasu23-${definition.key}-${String(index + 1).padStart(2, '0')}`;
    if (stop.id === newId) return false;
    if (!stop.id || /^urayasu23-[\w-]+-\d{2}$/.test(stop.id)) {
      stop.id = newId;
      return true;
    }
    return false;
  ...
  function makeStop(definition, name, index) {
    ...
    return {
      id: `urayasu23-${definition.key}-${String(index + 1).padStart(2, '0')}`,
      name,
  ...
  function ensureRoute() {
    ...
    const systems = {};
    let changed = route.urayasuHigashiDanchiLine23Version !== VERSION;
    ...
    route.urayasuHigashiDanchiLine23Version = VERSION;
    route.description = '浦安東団地線（23系統）：舞浜駅⇔総合公園（2運行。route-3とは別系統）';
    route.sourcePolicy = '停留所順は京成バスナビ個別便通過時刻表で確認（出発のりば凡例で【２３系統】を判定し3系統を除外）。座標・道路はOSM relation 18396546/18396547 採用。';
    if (!route.urayasuHigashiDanchiLine23StopImages) {
      route.urayasuHigashiDanchiLine23StopImages = {};
      changed = true;
    }
  ...
  function pathIntegrityFail(system, definition, reasons) {
    const policy = window.URAYASU_HIGASHI_DANCHI_LINE_23_PATH_POLICY_V1;
  ...
  async function resolveAllSystems(force = false, statusCallback = null) {
    const order = ['23-maihama-sogo', '23-sogo-maihama'];
  ...
  function migrateStopImageBank(route) {
    const bank = route.urayasuHigashiDanchiLine23StopImages;
  ...
  function showStopImage(route, definition, stop, street) {
    ...
    const existing = street.querySelector('.stop-image-display-v25.urayasuHigashiDanchiLine23-stop-image');
    existing?.remove();
    if (!entry?.dataUrl) return false;
    const overlay = document.createElement('div');
    overlay.className = 'stop-image-display-v25 urayasuHigashiDanchiLine23-stop-image';
  ...
  function hideStopImage(street) {
    street?.querySelector('.urayasuHigashiDanchiLine23-stop-image')?.remove();
  }
  ...
  async function drawGuidance(route, definition, system, token) {
    ...
    try {
      ...
      const message = base.includes('浦安東団地線の走行データを確認できません')
        ? base
        : [
          '浦安東団地線の走行データを確認できません。',
          `系統：${definition.key}`,
    ...
    const publishDebugState = () => {
      window.URAYASU_HIGASHI_DANCHI_LINE_23_DRIVE_STATE = {
        ...simulation,
        ...
        try {
          streetViewService.getPanorama(request, (data, status) => {
            try {
              ...
              console.warn('[urayasuHigashiDanchiLine23] panorama update failed', error);
            }
          });
        } catch (error) {
          console.warn('[urayasuHigashiDanchiLine23] getPanorama failed', error);
          applyPovOnly();
        }
        return true;
      }
    ...
    const updateDrivingVisual = (now, force = false) => {
      ...
      if (!point?.position || !Number.isFinite(point.position.lat) || !Number.isFinite(point.position.lng)) {
        ...
        console.error('[urayasuHigashiDanchiLine23] invalid drive position', point, simulation);
        return;
      }
    ...
    const failStart = (reason) => {
      ...
      console.error('[urayasuHigashiDanchiLine23] drive start failed', reason, {
        systemKey: definition.key,
        pathLength: path.length,
        metricsTotal: metrics.total,
    ...
    cleanupGuidance = () => {
      ...
      telop.remove();
      hideStopImage(street);
      if (window.URAYASU_HIGASHI_DANCHI_LINE_23_DRIVE_STATE?.systemKey === definition.key) {
        window.URAYASU_HIGASHI_DANCHI_LINE_23_DRIVE_STATE = null;
      }
    };
  }

  routes = function routesUrayasuHigashiDanchiLine23LineV1() {
    const route = ensureRoute();
    ...
    try { window.HOKUEI_GUIDANCE_V22?.cleanup?.(); } catch (error) {
      console.warn('[urayasuHigashiDanchiLine23] hokuei cleanup failed', error);
    }
    ...
    drawGuidance(route, definition, system, token).catch((error) => {
      ...
      if (node) {
        ...
        node.textContent = message.includes('浦安東団地線の走行データを確認できません') || message.includes('系統')
          ? message
          : (`系統=${key} | ${message}`);
      }
      console.error('[urayasuHigashiDanchiLine23] drawGuidance failed', definition.key, error);
    });
  };
  ...
  function openRoute23StopEditor(systemKey, stopIndex) {
    closeEditorDialog();
    const route = ensureRoute();
    ...
    const currentImage = route.urayasuHigashiDanchiLine23StopImages?.[bankKey]?.dataUrl || '';
    ...
    const saveCurrent = () => {
      ...
      if (!route.urayasuHigashiDanchiLine23StopImages) route.urayasuHigashiDanchiLine23StopImages = {};
      if (state.removeImage) delete route.urayasuHigashiDanchiLine23StopImages[bankKey];
      else if (state.dataUrl) route.urayasuHigashiDanchiLine23StopImages[bankKey] = { dataUrl: state.dataUrl, updatedAt: new Date().toISOString(), sourceSystem: systemKey, sourceStopId: stop.id };
      route.urayasuHigashiDanchiLine23StopImageUpdatedAt = new Date().toISOString();
      save();
      return true;
    };
    ...
    document.getElementById('previousStopEdit').onclick = () => { if (!saveCurrent()) return; close(); openRoute23StopEditor(systemKey, stopIndex - 1); };
    document.getElementById('nextStopEdit').onclick = () => { if (!saveCurrent()) return; close(); openRoute23StopEditor(systemKey, stopIndex + 1); };
  ...
  function renderUrayasuHigashiDanchiLine23StopList() {
    const route = ensureRoute();
    ...
    box.querySelectorAll('[data-uhd-edit]').forEach((button) => { button.onclick = () => openRoute23StopEditor(definition.key, Number(button.dataset.uhdEdit)); });
  }

  function route23StopEditor() {
    ensureRoute();
    editorRouteId = ROUTE_ID;
    ...
    renderUrayasuHigashiDanchiLine23StopList();
  ...
  stopEditor = function stopEditorUrayasuHigashiDanchiLine23LineV1() {
  ...
  window.URAYASU_HIGASHI_DANCHI_LINE_23_ROUTE_V1 = {
    VERSION,
