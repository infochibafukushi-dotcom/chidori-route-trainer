// 堀江線（系統番号5・route-5）走行シミュレーション／停留所編集モジュール。
// 停留所順：京成バスナビ通過時刻表で確認（evidence/route5-horie-implementation-2026-07-24/official-stop-orders.json）。
// 停留所座標：OSM platform採用（往復で別platform）。
// 道路形状：OSM relation way鎖（18417632 outbound / 18417631 inbound）。Google Directionsは使用しない。
// UI構造・走行シミュレーションは tomioka-route-v1.js パターンを移植。
(() => {
  const ROUTE_ID = 'route-5';
  const VERSION = '2026-07-24-horie-v1';
  const SYSTEM_RESOLVED_VERSIONS = {
    '5-shinurayasu': '2026-07-24-horie-shinurayasu-v1',
    '5-ntt': '2026-07-24-horie-ntt-v1',
    '5-urayasu': '2026-07-24-horie-urayasu-v1',
    '5-tokai': '2026-07-24-horie-tokai-v1',
    '5-higashino-chuo': '2026-07-24-horie-higashino-chuo-v1',
  };
  const SYSTEM_KEY = 'chidori-horie-system-v1';
  const DISPLAY_CODE = '5';
  const SPEED_KMH = 20;
  const DWELL_MS = 3000;
  const MAP_ZOOM = 18;
  ...
  const SYSTEM_DEFINITIONS = {
    '5-shinurayasu': {
      key: '5-shinurayasu', displayCode: DISPLAY_CODE, directionGroup: 'outbound',
      title: '新浦安駅行き',
      summary: '浦安駅入口 → ＮＴＴ浦安前 → 新浦安駅',
      relationId: 18417632,
      names: NAMES_SHINURAYASU,
    },
    '5-ntt': {
      key: '5-ntt', displayCode: DISPLAY_CODE, directionGroup: 'outbound',
      title: 'ＮＴＴ浦安前行き',
      summary: '浦安駅入口 → 東野中央 → ＮＴＴ浦安前',
      relationId: 18417632,
      names: NAMES_NTT,
    },
    '5-urayasu': {
      key: '5-urayasu', displayCode: DISPLAY_CODE, directionGroup: 'inbound',
      title: '浦安駅入口行き',
      summary: '新浦安駅 → 堀江一丁目 → 浦安駅入口',
      relationId: 18417631,
      names: NAMES_URAYASU,
    },
    '5-tokai': {
      key: '5-tokai', displayCode: DISPLAY_CODE, directionGroup: 'inbound',
      title: '東海大浦安高校前行き',
      summary: '新浦安駅 → 東野中央 → 東海大浦安高校前',
      relationId: 18417631,
      names: NAMES_TOKAI,
    },
    '5-higashino-chuo': {
      key: '5-higashino-chuo', displayCode: DISPLAY_CODE, directionGroup: 'inbound',
      title: '東野中央行き',
      summary: '新浦安駅 → ＮＴＴ浦安前 → 東野中央',
      relationId: 18417631,
      names: NAMES_HIGASHINO_CHUO,
    },
  };

  const DEFAULT_SYSTEM_KEY = '5-shinurayasu';
  ...
  function platformsForSystem(systemKey) {
    const bank = window.HORIE_PLATFORMS_V1 || {};
    return bank[systemKey] || {};
  }

  function pathDataForSystem(systemKey) {
    const bank = window.HORIE_PATH_V1 || {};
    return bank[systemKey] || null;
  }

  /** 系統キー単位の停留所IDへ揃える。 */
  function migrateStopId(definition, stop, index) {
    const newId = `horie-${definition.key}-${String(index + 1).padStart(2, '0')}`;
    if (stop.id === newId) return false;
    if (!stop.id || /^horie-5-\d{2}$/.test(stop.id)) {
      stop.id = newId;
      return true;
    }
    return false;
  ...
  function makeStop(definition, name, index) {
    ...
    return {
      id: `horie-${definition.key}-${String(index + 1).padStart(2, '0')}`,
      name,
  ...
  function ensureRoute() {
    ...
    const systems = {};
    let changed = route.horieVersion !== VERSION;
  ...
  function pathIntegrityFail(system, definition, reasons) {
    const policy = window.HORIE_PATH_POLICY_V1;
  ...
  async function resolveSystem(key, { force = false, statusCallback = null } = {}) {
    ...
    if (!route || !definition || !system) throw new Error('堀江線の系統データがありません。');
  ...
  async function resolveAllSystems(force = false, statusCallback = null) {
    const order = ['5-shinurayasu','5-ntt','5-urayasu','5-tokai','5-higashino-chuo'];
  ...
  function migrateStopImageBank(route) {
    const bank = route.horieStopImages;
  ...
  function showStopImage(route, definition, stop, street) {
    ...
    const existing = street.querySelector('.stop-image-display-v25.horie-stop-image');
    existing?.remove();
    if (!entry?.dataUrl) return false;
    const overlay = document.createElement('div');
    overlay.className = 'stop-image-display-v25 horie-stop-image';
  ...
  function hideStopImage(street) {
    street?.querySelector('.horie-stop-image')?.remove();
  }
  ...
  async function drawGuidance(route, definition, system, token) {
    ...
    try {
      ...
      const message = base.includes('堀江線の走行データを確認できません')
        ? base
        : [
          '堀江線の走行データを確認できません。',
          `系統：${definition.key}`,
    ...
    const vehicle = new googleApi.maps.Marker({ map, position: center, icon: busIcon(googleApi), zIndex: 1000, title: `堀江線 ${definition.title}` });
        ...
        try {
          streetViewService.getPanorama(request, (data, status) => {
            try {
              ...
              console.warn('[horie] panorama update failed', error);
            }
          });
        } catch (error) {
          console.warn('[horie] getPanorama failed', error);
          applyPovOnly();
        }
        return true;
      }
    ...
    const updateDrivingVisual = (now, force = false) => {
      ...
      if (!point?.position || !Number.isFinite(point.position.lat) || !Number.isFinite(point.position.lng)) {
        ...
        console.error('[horie] invalid drive position', point, simulation);
        return;
      }
    ...
    const failStart = (reason) => {
      ...
      console.error('[horie] drive start failed', reason, {
        systemKey: definition.key,
        pathLength: path.length,
        metricsTotal: metrics.total,
    ...
    status.textContent = `堀江線｜系統${DISPLAY_CODE}｜${definition.title}｜${stops.length}停留所｜道路形状：OSM relation採用（要走行確認）`;
    publishDebugState();
    cleanupGuidance = () => {
      simulation.running = false;
      simulation.paused = false;
      ...
      telop.remove();
      hideStopImage(street);
      if (window.HORIE_DRIVE_STATE?.systemKey === definition.key) {
        window.HORIE_DRIVE_STATE = null;
      }
    };
  }

  routes = function routesHorieV1() {
    const route = ensureRoute();
    ...
    try { window.HOKUEI_GUIDANCE_V22?.cleanup?.(); } catch (error) {
      console.warn('[horie] hokuei cleanup failed', error);
    }
    ...
    shell(`<section class="guidance-page-v26">
      ...
      <div class="split guidance-v22-split">
        <div class="guidance-map-wrap-v22"><div id="routeMap" class="map guidance-map-v22"></div><div class="guidance-version-v22">堀江線・系統${DISPLAY_CODE}</div></div>
        <div id="street" class="street guidance-street-v22"></div>
      </div>
      <p id="mapStatus" class="status" hidden aria-hidden="true">堀江線を準備しています…</p>
    </section>`);
    ...
    drawGuidance(route, definition, system, token).catch((error) => {
      ...
      if (node) {
        ...
        node.textContent = message.includes('堀江線の走行データを確認できません') || message.includes('系統')
          ? message
          : (`系統=${key} | ${message}`);
      }
      console.error('[horie] drawGuidance failed', definition.key, error);
    });
  };
  ...
  function openRoute3StopEditor(systemKey, stopIndex) {
    ...
    const currentImage = route.horieStopImages?.[bankKey]?.dataUrl || '';
    ...
    dialog.innerHTML = `<section class="stop-edit-dialog" role="dialog" aria-modal="true">
      <div class="stop-edit-header"><div><h2>堀江線 停留所設定</h2><p>${DISPLAY_CODE}｜${escHtml(definition.title)}・${stopIndex + 1} / ${system.stops.length}</p></div><button type="button" id="closeStopEditor" class="stop-edit-close">×</button></div>
    ...
    const saveCurrent = () => {
      ...
      if (!route.horieStopImages) route.horieStopImages = {};
      if (state.removeImage) delete route.horieStopImages[bankKey];
      else if (state.dataUrl) route.horieStopImages[bankKey] = { dataUrl: state.dataUrl, updatedAt: new Date().toISOString(), sourceSystem: systemKey, sourceStopId: stop.id };
      route.horieStopImageUpdatedAt = new Date().toISOString();
      save();
      return true;
    };
  ...
  stopEditor = function stopEditorHorieV1() {
