(() => {
  ...
  function projectPoint(point, path, metrics, minimumDistance = 0) {
    ...
    for (let index = 0; index < path.length - 1; index += 1) {
      const segmentEndDistance = metrics.cumulative[index + 1];
      ...
      const segmentStartDistance = metrics.cumulative[index];
      const routeDistance = segmentStartDistance + ratio * (segmentEndDistance - segmentStartDistance);
      if (routeDistance < minimumDistance) continue;
      const px = ax + ratio * dx;
      const py = ay + ratio * dy;
      const distance = Math.hypot(px, py);
      if (!best || distance < best.distance) best = { routeDistance, distance };
    }
    return best || { routeDistance: minimumDistance, distance: Infinity };
  }
  ...
  function mapStopsToRoute(stops, path, metrics) {
    ...
    return stops.map((stop, index) => {
      ...
      const result = projectPoint(stop, path, metrics, minimumDistance + 3);
      minimumDistance = Math.max(minimumDistance + 3, result.routeDistance);
  ...
  function geometricTurns(path, metrics, existingTurns) {
    ...
    for (let index = 2; index < path.length - 2; index += 1) {
      if (distanceMeters(path[index - 2], path[index]) < 8) continue;
      if (distanceMeters(path[index], path[index + 2]) < 8) continue;
  ...
  async function drawGuidance(route, system, token) {
    ...
    stationTelop.className = 'station-name-telop guidance-station-v21';
    street.appendChild(stationTelop);
    const turnGuide = document.createElement('div');
    turnGuide.className = 'driving-turn-guide guidance-turn-v21';
    turnGuide.hidden = true;
    street.appendChild(turnGuide);
    const streetState = document.createElement('div');
    streetState.className = 'street-state-v21';
    streetState.hidden = true;
    street.appendChild(streetState);
    ...
    document.querySelector('.guidance-v21-split')?.insertAdjacentElement('afterend', controls);
    ...
    const findPanorama = (position, radius) => new Promise((resolve) => {
      ...
      streetViewService.getPanorama(request, (data, status) => {
        const ok = status === googleApi.maps.StreetViewStatus.OK && data?.location?.pano;
        resolve(ok ? data : null);
      });
    });

    const focusStopVisual = async (index) => {
      const stop = activeStops[index];
      if (!stop) return;
      const requestToken = ++streetRequestToken;
      const exactPosition = { lat: stop.lat, lng: stop.lng };
      const pathPosition = positionAtDistance(result.path, metrics, stopDistances[index]);
      vehicle.setPosition(exactPosition);
      map.panTo(exactPosition);
      map.setZoom(MAP_ZOOM);
      streetState.hidden = false;
      streetState.textContent = `${displayName(stop)}のStreet Viewを検索中…`;

      try {
        let data = await findPanorama(exactPosition, 60);
        if (!data) data = await findPanorama(exactPosition, 120);
        if (requestToken !== streetRequestToken) return;
        if (data?.location?.pano) {
          panorama.setPano(data.location.pano);
          panorama.setPov({ heading: heading(exactPosition, pathPosition.next), pitch: 0 });
          streetState.hidden = true;
        } else {
          panorama.setPosition(exactPosition);
          panorama.setPov({ heading: heading(exactPosition, pathPosition.next), pitch: 0 });
          streetState.textContent = `${displayName(stop)}付近のStreet Viewが見つかりません`;
        }
      } catch (error) {
        if (requestToken !== streetRequestToken) return;
        panorama.setPosition(exactPosition);
        streetState.textContent = `${displayName(stop)}のStreet Viewを取得できませんでした`;
    ...
    const update = (now, forceStopVisual = false) => {
      const current = positionAtDistance(result.path, metrics, traveled);
      if (!dwellUntil) vehicle.setPosition(current.position);
      if (!dwellUntil && (forceStopVisual || now - lastDrivingVisualUpdate >= 1100)) {
    ...
    const arriveAtStop = (index, autoContinue) => {
      ...
      if (autoContinue) {
        ...
        statusNode.textContent = `${displayName(activeStops[index])}へ移動しました`;
      }
    ...
    focusStopVisual(0);
    updateButtons();
    statusNode.textContent = `案内V21｜次の停留所ボタン修正｜停留所${activeStops.length}件｜右左折案内${turns.length}件`;
  ...
  routes = function routesGuidanceV21() {
    ...
    shell(`<section>
      ...
      <section class="manual-mode-card guidance-summary-v21">
        <div><strong>ルート案内 V21</strong><span>次の停留所ボタン修正・停留所で3秒停車・停留所名テロップ・右左折案内・地図約20m尺度｜位置設定 ${positioned}/${system.stops.length}件</span></div>
        <button id="manualSettings" class="secondary" type="button">設定画面で位置を修正</button>
      </section>
      <div class="split guidance-v21-split">
        <div class="guidance-map-wrap-v21"><div id="routeMap" class="map guidance-map-v21"></div><div class="guidance-version-v21">案内V21・約20m</div></div>
        <div id="street" class="street guidance-street-v21"></div>
      </div>
      <p id="mapStatus" class="status">案内V21を準備しています…</p>
    </section>`);
    ...
    drawGuidance(route, system, token).catch((error) => {
      ...
      console.error('案内V21エラー', error);
    });
  };

  window.HOKUEI_GUIDANCE_V21 = {
    speedKmh: SPEED_KMH,
    dwellSeconds: 3,
