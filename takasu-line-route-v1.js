// 高洲線（系統番号10・route-10）走行シミュレーション／停留所編集モジュール。
...
(() => {
  ...
  function platformsForSystem(systemKey) {
    const bank = window.TAKASU_LINE_PLATFORMS_V1 || {};
    return bank[systemKey] || {};
  }

  function pathDataForSystem(systemKey) {
    const bank = window.TAKASU_LINE_PATH_V1 || {};
  ...
  function ensureRoute() {
    ...
    const systems = {};
    let changed = route.takasuLineVersion !== VERSION;
    ...
    route.takasuLineVersion = VERSION;
    ...
    if (!route.takasuLineStopImages) {
      route.takasuLineStopImages = {};
      changed = true;
    }
  ...
  function pathIntegrityFail(system, definition, reasons) {
    const policy = window.TAKASU_LINE_PATH_POLICY_V1;
  ...
  async function resolveSystem(key, { force = false, statusCallback = null } = {}) {
    ...
    if (!route || !definition || !system) throw new Error('高洲線の系統データがありません。');
  ...
  function migrateStopImageBank(route) {
    const bank = route.takasuLineStopImages;
  ...
  function showStopImage(route, definition, stop, street) {
    ...
    const existing = street.querySelector('.stop-image-display-v25.takasuLine-stop-image');
    existing?.remove();
    if (!entry?.dataUrl) return false;
    const overlay = document.createElement('div');
    overlay.className = 'stop-image-display-v25 takasuLine-stop-image';
  ...
  function hideStopImage(street) {
    street?.querySelector('.takasuLine-stop-image')?.remove();
  }
  ...
  async function drawGuidance(route, definition, system, token) {
    ...
    try {
      ...
      const message = base.includes('高洲線の走行データを確認できません')
        ? base
        : [
          '高洲線の走行データを確認できません。',
          `系統：${definition.key}`,
    ...
    const vehicle = new googleApi.maps.Marker({ map, position: center, icon: busIcon(googleApi), zIndex: 1000, title: `高洲線 ${definition.title}` });
    ...
    const publishDebugState = () => {
      window.TAKASU_LINE_DRIVE_STATE = {
        ...simulation,
        ...
        try {
          streetViewService.getPanorama(request, (data, status) => {
            try {
              ...
              console.warn('[takasuLine] panorama update failed', error);
            }
          });
        } catch (error) {
          console.warn('[takasuLine] getPanorama failed', error);
          applyPovOnly();
        }
        return true;
      }
    ...
    const updateDrivingVisual = (now, force = false) => {
      ...
      if (!point?.position || !Number.isFinite(point.position.lat) || !Number.isFinite(point.position.lng)) {
        ...
        console.error('[takasuLine] invalid drive position', point, simulation);
        return;
      }
    ...
    const failStart = (reason) => {
      ...
      console.error('[takasuLine] drive start failed', reason, {
        systemKey: definition.key,
        pathLength: path.length,
        metricsTotal: metrics.total,
    ...
    status.textContent = `高洲線｜系統${DISPLAY_CODE}｜${definition.title}｜${stops.length}停留所｜道路形状：OSM relation採用（要走行確認）`;
    publishDebugState();
    cleanupGuidance = () => {
      simulation.running = false;
      simulation.paused = false;
      ...
      telop.remove();
      hideStopImage(street);
      if (window.TAKASU_LINE_DRIVE_STATE?.systemKey === definition.key) {
        window.TAKASU_LINE_DRIVE_STATE = null;
      }
    };
  }
  ...
  routes = function routesShiyakushoV1() {
    ...
    try { window.HOKUEI_GUIDANCE_V22?.cleanup?.(); } catch (error) {
      console.warn('[takasuLine] hokuei cleanup failed', error);
    }
    ...
    shell(`<section class="guidance-page-v26">
      ...
      <div class="split guidance-v22-split">
        <div class="guidance-map-wrap-v22"><div id="routeMap" class="map guidance-map-v22"></div><div class="guidance-version-v22">高洲線・系統${DISPLAY_CODE}</div></div>
        <div id="street" class="street guidance-street-v22"></div>
      </div>
      <p id="mapStatus" class="status" hidden aria-hidden="true">高洲線を準備しています…</p>
    </section>`);
    ...
    drawGuidance(route, definition, system, token).catch((error) => {
      ...
      if (node) {
        ...
        node.textContent = message.includes('高洲線の走行データを確認できません') || message.includes('系統')
          ? message
          : (`系統=${key} | ${message}`);
      }
      console.error('[takasuLine] drawGuidance failed', definition.key, error);
    });
  };
  ...
  function openRoute6StopEditor(systemKey, stopIndex) {
    ...
    const currentImage = route.takasuLineStopImages?.[bankKey]?.dataUrl || '';
    ...
    dialog.innerHTML = `<section class="stop-edit-dialog" role="dialog" aria-modal="true">
      <div class="stop-edit-header"><div><h2>高洲線 停留所設定</h2><p>${DISPLAY_CODE}｜${escHtml(definition.title)}・${stopIndex + 1} / ${system.stops.length}</p></div><button type="button" id="closeStopEditor" class="stop-edit-close">×</button></div>
    ...
    const saveCurrent = () => {
      ...
      if (!route.takasuLineStopImages) route.takasuLineStopImages = {};
      if (state.removeImage) delete route.takasuLineStopImages[bankKey];
      else if (state.dataUrl) route.takasuLineStopImages[bankKey] = { dataUrl: state.dataUrl, updatedAt: new Date().toISOString(), sourceSystem: systemKey, sourceStopId: stop.id };
      route.takasuLineStopImageUpdatedAt = new Date().toISOString();
      save();
      return true;
    };
