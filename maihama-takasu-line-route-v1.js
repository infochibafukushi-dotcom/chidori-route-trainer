// 舞浜・高洲線（系統番号25・route-25）走行シミュレーション／停留所編集モジュール。
// 停留所順：京成バスナビ個別便通過時刻表で確認（evidence/route25-maihama-takasu-implementation-2026-07-27/official-stop-orders.json）。
...
(() => {
  ...
  const SYSTEM_RESOLVED_VERSIONS = {
    '25-maihama-takasu-seaside': '2026-07-27-maihama25-maihama-takasu-seaside-v1',
    '25-takasu-seaside-maihama': '2026-07-27-maihama25-takasu-seaside-maihama-v1',
    '25-maihama-sogo': '2026-07-27-maihama25-maihama-sogo-v1',
    '25-sogo-maihama': '2026-07-27-maihama25-sogo-maihama-v1',
  };
  const SYSTEM_KEY = 'maihama-takasu-line-system-v1';
  const DISPLAY_CODE = '25';
  const INDEX_PLATFORM_SYSTEMS = new Set(["25-maihama-takasu-seaside","25-takasu-seaside-maihama","25-maihama-sogo","25-sogo-maihama"]);
  const SPEED_KMH = 20;
  const DWELL_MS = 3000;
  const MAP_ZOOM = 18;
  ...
  const NAMES_25_MAIHAMA_TAKASU_SEASIDE = [
    "舞浜駅",
    "オリエンタルランド本社前",
    "運動公園",
    "舞浜三丁目",
  ...
  const NAMES_25_TAKASU_SEASIDE_MAIHAMA = [
    "高洲海浜公園",
    "潮音の街",
    "高洲七丁目",
    "高洲四丁目",
  ...
  const SYSTEM_DEFINITIONS = {
    '25-maihama-takasu-seaside': {
      key: '25-maihama-takasu-seaside', displayCode: DISPLAY_CODE, directionGroup: 'outbound-seaside',
      title: '高洲海浜公園行き（舞浜駅発）',
      summary: '舞浜駅 → 高洲西児童公園 → 高洲海浜公園',
      relationId: 18352023,
      timetableSymbol: '',
      naviCourse: '0008200306',
      names: NAMES_25_MAIHAMA_TAKASU_SEASIDE,
    },
    '25-takasu-seaside-maihama': {
      key: '25-takasu-seaside-maihama', displayCode: DISPLAY_CODE, directionGroup: 'inbound-seaside',
      title: '舞浜駅行き（高洲海浜公園発）',
      summary: '高洲海浜公園 → 高洲西児童公園 → 舞浜駅',
      relationId: 18352022,
      timetableSymbol: '',
      naviCourse: '0008200305',
      names: NAMES_25_TAKASU_SEASIDE_MAIHAMA,
    },
    '25-maihama-sogo': {
      key: '25-maihama-sogo', displayCode: DISPLAY_CODE, directionGroup: 'outbound-sogo',
      title: '総合公園行き（舞浜駅発）',
      summary: '舞浜駅 → 高洲西児童公園 → 総合公園',
      relationId: 18352045,
      timetableSymbol: '',
      naviCourse: '0008200308',
      names: NAMES_25_MAIHAMA_SOGO,
    },
    '25-sogo-maihama': {
      key: '25-sogo-maihama', displayCode: DISPLAY_CODE, directionGroup: 'inbound-sogo',
      title: '舞浜駅行き（総合公園発）',
      summary: '総合公園 → 高洲西児童公園 → 舞浜駅',
      relationId: 18352044,
      timetableSymbol: '',
      naviCourse: '0008200307',
      names: NAMES_25_SOGO_MAIHAMA,
    },
  };

  const DEFAULT_SYSTEM_KEY = '25-maihama-takasu-seaside';
  ...
  function platformsForSystem(systemKey) {
    const bank = window.MAIHAMA_TAKASU_LINE_PLATFORMS_V1 || {};
  ...
  function pathDataForSystem(systemKey) {
    const bank = window.MAIHAMA_TAKASU_LINE_PATH_V1 || {};
    return bank[systemKey] || null;
  }

  /** 系統キー単位の停留所IDへ揃える。 */
  function migrateStopId(definition, stop, index) {
    const newId = `maihama25-${definition.key}-${String(index + 1).padStart(2, '0')}`;
    if (stop.id === newId) return false;
    if (!stop.id || /^maihama25-[\w-]+-\d{2}$/.test(stop.id)) {
      stop.id = newId;
      return true;
    }
    return false;
  ...
  function makeStop(definition, name, index) {
    ...
    return {
      id: `maihama25-${definition.key}-${String(index + 1).padStart(2, '0')}`,
      name,
  ...
  function ensureRoute() {
    ...
    const systems = {};
    let changed = route.maihamaTakasuLineVersion !== VERSION;
    ...
    route.maihamaTakasuLineVersion = VERSION;
    route.description = '舞浜・高洲線（25系統）：舞浜駅⇔高洲海浜公園／総合公園（4運行。10/15/18/19系統とは別系統）';
    route.sourcePolicy = '停留所順は京成バスナビ個別便通過時刻表で確認（出発のりば凡例で【２５系統】を判定し19系統を除外）。座標・道路はOSM relation 18396546/18396547 採用。';
    if (!route.maihamaTakasuLineStopImages) {
      route.maihamaTakasuLineStopImages = {};
      changed = true;
    }
  ...
  function pathIntegrityFail(system, definition, reasons) {
    const policy = window.MAIHAMA_TAKASU_LINE_PATH_POLICY_V1;
  ...
  async function resolveSystem(key, { force = false, statusCallback = null } = {}) {
    ...
    if (!route || !definition || !system) throw new Error('舞浜・高洲線の系統データがありません。');
  ...
  async function resolveAllSystems(force = false, statusCallback = null) {
    const order = ['25-maihama-takasu-seaside', '25-takasu-seaside-maihama', '25-maihama-sogo', '25-sogo-maihama'];
  ...
  function migrateStopImageBank(route) {
    const bank = route.maihamaTakasuLineStopImages;
  ...
  function showStopImage(route, definition, stop, street) {
    ...
    const existing = street.querySelector('.stop-image-display-v25.maihamaTakasuLine-stop-image');
    existing?.remove();
    if (!entry?.dataUrl) return false;
    const overlay = document.createElement('div');
    overlay.className = 'stop-image-display-v25 maihamaTakasuLine-stop-image';
  ...
  function hideStopImage(street) {
    street?.querySelector('.maihamaTakasuLine-stop-image')?.remove();
  }
  ...
  async function drawGuidance(route, definition, system, token) {
    ...
    try {
      ...
      const message = base.includes('舞浜・高洲線の走行データを確認できません')
        ? base
        : [
          '舞浜・高洲線の走行データを確認できません。',
          `系統：${definition.key}`,
    ...
    const vehicle = new googleApi.maps.Marker({ map, position: center, icon: busIcon(googleApi), zIndex: 1000, title: `舞浜・高洲線 ${definition.title}` });
    ...
    const publishDebugState = () => {
      window.MAIHAMA_TAKASU_LINE_DRIVE_STATE = {
        ...simulation,
        ...
        try {
          streetViewService.getPanorama(request, (data, status) => {
            try {
              ...
              console.warn('[maihamaTakasuLine] panorama update failed', error);
            }
          });
        } catch (error) {
          console.warn('[maihamaTakasuLine] getPanorama failed', error);
          applyPovOnly();
        }
        return true;
      }
    ...
    const updateDrivingVisual = (now, force = false) => {
      ...
      if (!point?.position || !Number.isFinite(point.position.lat) || !Number.isFinite(point.position.lng)) {
        ...
        console.error('[maihamaTakasuLine] invalid drive position', point, simulation);
        return;
      }
    ...
    const failStart = (reason) => {
      ...
      console.error('[maihamaTakasuLine] drive start failed', reason, {
        systemKey: definition.key,
        pathLength: path.length,
        metricsTotal: metrics.total,
    ...
    status.textContent = `舞浜・高洲線｜系統${DISPLAY_CODE}｜${definition.title}｜${stops.length}停留所｜道路形状：OSM relation採用（要走行確認）`;
    publishDebugState();
    cleanupGuidance = () => {
      simulation.running = false;
      simulation.paused = false;
      ...
      telop.remove();
      hideStopImage(street);
      if (window.MAIHAMA_TAKASU_LINE_DRIVE_STATE?.systemKey === definition.key) {
        window.MAIHAMA_TAKASU_LINE_DRIVE_STATE = null;
      }
    };
  }

  routes = function routesMaihamaTakasuLineLineV1() {
    const route = ensureRoute();
    ...
    try { window.HOKUEI_GUIDANCE_V22?.cleanup?.(); } catch (error) {
      console.warn('[maihamaTakasuLine] hokuei cleanup failed', error);
    }
    ...
    shell(`<section class="guidance-page-v26">
      ...
      <div class="split guidance-v22-split">
        <div class="guidance-map-wrap-v22"><div id="routeMap" class="map guidance-map-v22"></div><div class="guidance-version-v22">舞浜・高洲線・系統${DISPLAY_CODE}</div></div>
        <div id="street" class="street guidance-street-v22"></div>
      </div>
      <p id="mapStatus" class="status" hidden aria-hidden="true">舞浜・高洲線を準備しています…</p>
    </section>`);
    ...
    drawGuidance(route, definition, system, token).catch((error) => {
      ...
      if (node) {
        ...
        node.textContent = message.includes('舞浜・高洲線の走行データを確認できません') || message.includes('系統')
          ? message
          : (`系統=${key} | ${message}`);
      }
      console.error('[maihamaTakasuLine] drawGuidance failed', definition.key, error);
    });
  };
  ...
  function openRoute25StopEditor(systemKey, stopIndex) {
    closeEditorDialog();
    const route = ensureRoute();
    ...
    const currentImage = route.maihamaTakasuLineStopImages?.[bankKey]?.dataUrl || '';
    ...
    dialog.innerHTML = `<section class="stop-edit-dialog" role="dialog" aria-modal="true">
      <div class="stop-edit-header"><div><h2>舞浜・高洲線 停留所設定</h2><p>${DISPLAY_CODE}｜${escHtml(definition.title)}・${stopIndex + 1} / ${system.stops.length}</p></div><button type="button" id="closeStopEditor" class="stop-edit-close">×</button></div>
    ...
    const saveCurrent = () => {
      ...
      if (!route.maihamaTakasuLineStopImages) route.maihamaTakasuLineStopImages = {};
      if (state.removeImage) delete route.maihamaTakasuLineStopImages[bankKey];
      else if (state.dataUrl) route.maihamaTakasuLineStopImages[bankKey] = { dataUrl: state.dataUrl, updatedAt: new Date().toISOString(), sourceSystem: systemKey, sourceStopId: stop.id };
      route.maihamaTakasuLineStopImageUpdatedAt = new Date().toISOString();
      save();
      return true;
    };
    ...
    document.getElementById('previousStopEdit').onclick = () => { if (!saveCurrent()) return; close(); openRoute25StopEditor(systemKey, stopIndex - 1); };
    document.getElementById('nextStopEdit').onclick = () => { if (!saveCurrent()) return; close(); openRoute25StopEditor(systemKey, stopIndex + 1); };
  ...
  function renderMaihamaTakasuLineStopList() {
    const route = ensureRoute();
    ...
    box.querySelectorAll('[data-uhd-edit]').forEach((button) => { button.onclick = () => openRoute25StopEditor(definition.key, Number(button.dataset.uhdEdit)); });
  }

  function route25StopEditor() {
    ensureRoute();
    editorRouteId = ROUTE_ID;
    ...
    renderMaihamaTakasuLineStopList();
  ...
  stopEditor = function stopEditorMaihamaTakasuLineLineV1() {
  ...
  window.MAIHAMA_TAKASU_LINE_ROUTE_V1 = {
    VERSION,
