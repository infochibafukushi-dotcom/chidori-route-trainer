// 大三角線（系統番号37・route-37）走行シミュレーション／停留所編集モジュール。
// 停留所順：京成バスナビ個別便通過時刻表で確認（evidence/route37-daisankaku-implementation-2026-07-27/official-stop-orders.json）。
...
(() => {
  ...
  const SYSTEM_RESOLVED_VERSIONS = {
    '37-minamigyotoku-tds': '2026-07-27-daisankaku37-minamigyotoku-tds-v1',
    '37-minamigyotoku-maihama': '2026-07-27-daisankaku37-minamigyotoku-maihama-v1',
    '37-tds-minamigyotoku': '2026-07-27-daisankaku37-tds-minamigyotoku-v1',
    '37-maihama-minamigyotoku': '2026-07-27-daisankaku37-maihama-minamigyotoku-v1',
    '37-tds-horie6': '2026-07-27-daisankaku37-tds-horie6-v1',
    '37-fujimi3-tds': '2026-07-27-daisankaku37-fujimi3-tds-v1',
    '37-horie6-tds': '2026-07-27-daisankaku37-horie6-tds-v1',
  };
  const SYSTEM_KEY = 'daisankaku-line-system-v1';
  const DISPLAY_CODE = '37';
  const INDEX_PLATFORM_SYSTEMS = new Set(["37-minamigyotoku-tds","37-minamigyotoku-maihama","37-tds-minamigyotoku","37-maihama-minamigyotoku","37-tds-horie6","37-fujimi3-tds","37-horie6-tds"]);
  const SPEED_KMH = 20;
  const DWELL_MS = 3000;
  const MAP_ZOOM = 18;
  ...
  const NAMES_37_MINAMIGYOTOKU_TDS = [
    "南行徳駅",
    "南行徳三丁目",
    "やなぎ公園",
    "北部小入口",
  ...
  const NAMES_37_MINAMIGYOTOKU_MAIHAMA = [
    "南行徳駅",
    "南行徳三丁目",
    "やなぎ公園",
    "北部小入口",
  ...
  const NAMES_37_TDS_MINAMIGYOTOKU = [
    "「東京ディズニーシー（Ｒ）」",
    "オリエンタルランド本社前",
    "舞浜駅",
    "京成ローズタウン",
  ...
  const NAMES_37_MAIHAMA_MINAMIGYOTOKU = [
    "舞浜駅",
    "京成ローズタウン",
    "富士見五丁目",
    "富士見三丁目",
  ...
  const SYSTEM_DEFINITIONS = {
    '37-minamigyotoku-tds': {
      key: '37-minamigyotoku-tds', displayCode: DISPLAY_CODE, directionGroup: 'outbound-tds',
      title: '東京ディズニーシー行き（南行徳駅発）',
      summary: '南行徳駅 → 豊受神社 → 京成ローズタウン → 舞浜駅 → TDS',
      relationId: 18323271,
      timetableSymbol: '',
      naviCourse: '0008200310',
      names: NAMES_37_MINAMIGYOTOKU_TDS,
    },
    '37-minamigyotoku-maihama': {
      key: '37-minamigyotoku-maihama', displayCode: DISPLAY_CODE, directionGroup: 'outbound-maihama',
      title: '舞浜駅止まり（南行徳駅発）',
      summary: '南行徳駅 → 豊受神社 → 京成ローズタウン → 舞浜駅',
      relationId: 18323271,
      timetableSymbol: '',
      naviCourse: '0008200312',
      names: NAMES_37_MINAMIGYOTOKU_MAIHAMA,
    },
    '37-tds-minamigyotoku': {
      key: '37-tds-minamigyotoku', displayCode: DISPLAY_CODE, directionGroup: 'inbound-tds',
      title: '南行徳駅行き（TDS発）',
      summary: 'TDS → 舞浜駅 → 京成ローズタウン → 豊受神社 → 南行徳駅',
      relationId: 18323272,
      timetableSymbol: '',
      naviCourse: '0008200309',
      names: NAMES_37_TDS_MINAMIGYOTOKU,
    },
    '37-maihama-minamigyotoku': {
      key: '37-maihama-minamigyotoku', displayCode: DISPLAY_CODE, directionGroup: 'inbound-maihama',
      title: '南行徳駅行き（舞浜駅発）',
      summary: '舞浜駅 → 京成ローズタウン → 豊受神社 → 南行徳駅',
      relationId: 18323272,
      timetableSymbol: '',
      naviCourse: '0008200311',
      names: NAMES_37_MAIHAMA_MINAMIGYOTOKU,
    },
    '37-tds-horie6': {
      key: '37-tds-horie6', displayCode: DISPLAY_CODE, directionGroup: 'inbound-short-horie6',
      title: '堀江六丁目止まり（TDS発）',
      summary: 'TDS → 舞浜駅 → 京成ローズタウン → 堀江六丁目',
      relationId: 18323272,
      timetableSymbol: '',
      naviCourse: '0008200313',
      names: NAMES_37_TDS_HORIE6,
      shortTurn: true,
    },
    '37-fujimi3-tds': {
      key: '37-fujimi3-tds', displayCode: DISPLAY_CODE, directionGroup: 'outbound-short-fujimi3',
      title: 'TDS行き（富士見三丁目発）',
      summary: '富士見三丁目 → 京成ローズタウン → 舞浜駅 → TDS',
      relationId: 18323271,
      timetableSymbol: '',
      naviCourse: '0008200315',
      names: NAMES_37_FUJIMI3_TDS,
      shortTurn: true,
    },
    '37-horie6-tds': {
      key: '37-horie6-tds', displayCode: DISPLAY_CODE, directionGroup: 'outbound-short-horie6',
      title: 'TDS行き（堀江六丁目発）',
      summary: '堀江六丁目 → 富士見 → 京成ローズタウン → 舞浜駅 → TDS',
      relationId: 18323271,
      timetableSymbol: '',
      naviCourse: '0008200314',
      names: NAMES_37_HORIE6_TDS,
      shortTurn: true,
    },
  };

  const DEFAULT_SYSTEM_KEY = '37-minamigyotoku-tds';
  ...
  function platformsForSystem(systemKey) {
    const bank = window.DAISANKAKU_LINE_PLATFORMS_V1 || {};
  ...
  function pathDataForSystem(systemKey) {
    const bank = window.DAISANKAKU_LINE_PATH_V1 || {};
    return bank[systemKey] || null;
  }

  /** 系統キー単位の停留所IDへ揃える。 */
  function migrateStopId(definition, stop, index) {
    const newId = `daisankaku37-${definition.key}-${String(index + 1).padStart(2, '0')}`;
    if (stop.id === newId) return false;
    if (!stop.id || /^daisankaku37-[\w-]+-\d{2}$/.test(stop.id)) {
      stop.id = newId;
      return true;
    }
    return false;
  ...
  function makeStop(definition, name, index) {
    ...
    return {
      id: `daisankaku37-${definition.key}-${String(index + 1).padStart(2, '0')}`,
      name,
  ...
  function ensureRoute() {
    ...
    const systems = {};
    let changed = route.daisankakuLineVersion !== VERSION;
    ...
    route.daisankakuLineVersion = VERSION;
    route.description = '大三角線（37系統）：舞浜駅⇔総合公園（2運行。route-3とは別系統）';
    route.sourcePolicy = '停留所順は京成バスナビ個別便通過時刻表で確認（出発のりば凡例で【３７系統】を判定し3系統を除外）。座標・道路はOSM relation 18396546/18396547 採用。';
    if (!route.daisankakuLineStopImages) {
      route.daisankakuLineStopImages = {};
      changed = true;
    }
  ...
  function pathIntegrityFail(system, definition, reasons) {
    const policy = window.DAISANKAKU_LINE_PATH_POLICY_V1;
  ...
  async function resolveSystem(key, { force = false, statusCallback = null } = {}) {
    ...
    if (!route || !definition || !system) throw new Error('大三角線の系統データがありません。');
  ...
  async function resolveAllSystems(force = false, statusCallback = null) {
    const order = ['37-minamigyotoku-tds', '37-tds-minamigyotoku'];
  ...
  function migrateStopImageBank(route) {
    const bank = route.daisankakuLineStopImages;
  ...
  function showStopImage(route, definition, stop, street) {
    ...
    const existing = street.querySelector('.stop-image-display-v25.daisankakuLine-stop-image');
    existing?.remove();
    if (!entry?.dataUrl) return false;
    const overlay = document.createElement('div');
    overlay.className = 'stop-image-display-v25 daisankakuLine-stop-image';
  ...
  function hideStopImage(street) {
    street?.querySelector('.daisankakuLine-stop-image')?.remove();
  }
  ...
  async function drawGuidance(route, definition, system, token) {
    ...
    try {
      ...
      const message = base.includes('大三角線の走行データを確認できません')
        ? base
        : [
          '大三角線の走行データを確認できません。',
          `系統：${definition.key}`,
    ...
    const vehicle = new googleApi.maps.Marker({ map, position: center, icon: busIcon(googleApi), zIndex: 1000, title: `大三角線 ${definition.title}` });
    ...
    const publishDebugState = () => {
      window.DAISANKAKU_LINE_DRIVE_STATE = {
        ...simulation,
        ...
        try {
          streetViewService.getPanorama(request, (data, status) => {
            try {
              ...
              console.warn('[daisankakuLine] panorama update failed', error);
            }
          });
        } catch (error) {
          console.warn('[daisankakuLine] getPanorama failed', error);
          applyPovOnly();
        }
        return true;
      }
    ...
    const updateDrivingVisual = (now, force = false) => {
      ...
      if (!point?.position || !Number.isFinite(point.position.lat) || !Number.isFinite(point.position.lng)) {
        ...
        console.error('[daisankakuLine] invalid drive position', point, simulation);
        return;
      }
    ...
    const failStart = (reason) => {
      ...
      console.error('[daisankakuLine] drive start failed', reason, {
        systemKey: definition.key,
        pathLength: path.length,
        metricsTotal: metrics.total,
    ...
    status.textContent = `大三角線｜系統${DISPLAY_CODE}｜${definition.title}｜${stops.length}停留所｜道路形状：OSM relation採用（要走行確認）`;
    publishDebugState();
    cleanupGuidance = () => {
      simulation.running = false;
      simulation.paused = false;
      ...
      telop.remove();
      hideStopImage(street);
      if (window.DAISANKAKU_LINE_DRIVE_STATE?.systemKey === definition.key) {
        window.DAISANKAKU_LINE_DRIVE_STATE = null;
      }
    };
  }

  routes = function routesDaisankakuLineLineV1() {
    const route = ensureRoute();
    ...
    try { window.HOKUEI_GUIDANCE_V22?.cleanup?.(); } catch (error) {
      console.warn('[daisankakuLine] hokuei cleanup failed', error);
    }
    ...
    shell(`<section class="guidance-page-v26">
      ...
      <div class="split guidance-v22-split">
        <div class="guidance-map-wrap-v22"><div id="routeMap" class="map guidance-map-v22"></div><div class="guidance-version-v22">大三角線・系統${DISPLAY_CODE}</div></div>
        <div id="street" class="street guidance-street-v22"></div>
      </div>
      <p id="mapStatus" class="status" hidden aria-hidden="true">大三角線を準備しています…</p>
    </section>`);
    ...
    drawGuidance(route, definition, system, token).catch((error) => {
      ...
      if (node) {
        ...
        node.textContent = message.includes('大三角線の走行データを確認できません') || message.includes('系統')
          ? message
          : (`系統=${key} | ${message}`);
      }
      console.error('[daisankakuLine] drawGuidance failed', definition.key, error);
    });
  };
  ...
  function openRoute37StopEditor(systemKey, stopIndex) {
    closeEditorDialog();
    const route = ensureRoute();
    ...
    const currentImage = route.daisankakuLineStopImages?.[bankKey]?.dataUrl || '';
    ...
    dialog.innerHTML = `<section class="stop-edit-dialog" role="dialog" aria-modal="true">
      <div class="stop-edit-header"><div><h2>大三角線 停留所設定</h2><p>${DISPLAY_CODE}｜${escHtml(definition.title)}・${stopIndex + 1} / ${system.stops.length}</p></div><button type="button" id="closeStopEditor" class="stop-edit-close">×</button></div>
    ...
    const saveCurrent = () => {
      ...
      if (!route.daisankakuLineStopImages) route.daisankakuLineStopImages = {};
      if (state.removeImage) delete route.daisankakuLineStopImages[bankKey];
      else if (state.dataUrl) route.daisankakuLineStopImages[bankKey] = { dataUrl: state.dataUrl, updatedAt: new Date().toISOString(), sourceSystem: systemKey, sourceStopId: stop.id };
      route.daisankakuLineStopImageUpdatedAt = new Date().toISOString();
      save();
      return true;
    };
    ...
    document.getElementById('previousStopEdit').onclick = () => { if (!saveCurrent()) return; close(); openRoute37StopEditor(systemKey, stopIndex - 1); };
    document.getElementById('nextStopEdit').onclick = () => { if (!saveCurrent()) return; close(); openRoute37StopEditor(systemKey, stopIndex + 1); };
  ...
  function renderDaisankakuLineStopList() {
    const route = ensureRoute();
    ...
    box.querySelectorAll('[data-uhd-edit]').forEach((button) => { button.onclick = () => openRoute37StopEditor(definition.key, Number(button.dataset.uhdEdit)); });
  }

  function route37StopEditor() {
    ensureRoute();
    editorRouteId = ROUTE_ID;
  ...
  stopEditor = function stopEditorDaisankakuLineLineV1() {
  ...
  window.DAISANKAKU_LINE_ROUTE_V1 = {
    VERSION,
