// 日の出線（系統番号16・route-16）走行シミュレーション／停留所編集モジュール。
// 停留所順：京成バスナビ個別便通過時刻表で確認（evidence/route16-hinode-implementation-2026-07-26/official-stop-orders.json）。
...
(() => {
  ...
  const SYSTEM_RESOLVED_VERSIONS = {
    '16-hinode-nanachome': '2026-07-26-hinode-nanachome-v1',
    '16-shinurayasu': '2026-07-26-hinode-shinurayasu-v1',
  };
  ...
  const NAMES_16_HINODE_NANACHOME = [
    "新浦安駅",
    "入船中央エステート",
    "明海大学前",
    "海風の街",
  ...
  const SYSTEM_DEFINITIONS = {
    '16-hinode-nanachome': {
      key: '16-hinode-nanachome', displayCode: DISPLAY_CODE, directionGroup: 'outbound',
      title: '日の出七丁目行き',
      summary: '新浦安駅 → 海風の街 → ベイシティ浦安 → プラウド新浦安パークマリーナ → 順天堂大学・日の出 正門 → 日の出七丁目',
      relationId: 18396563,
      timetableSymbol: '無印',
      naviCourse: '0008200280',
      names: NAMES_16_HINODE_NANACHOME,
    },
    '16-shinurayasu': {
      key: '16-shinurayasu', displayCode: DISPLAY_CODE, directionGroup: 'inbound',
      title: '新浦安駅行き',
      summary: '日の出七丁目 → 順天堂大学・日の出 正門 → プラウド新浦安パークマリーナ → ベイシティ浦安 → 海風の街 → 新浦安駅',
      relationId: 18396562,
      timetableSymbol: '無印',
      naviCourse: '0008200279',
      names: NAMES_16_SHINURAYASU,
    },
  };
  ...
  function platformsForSystem(systemKey) {
    const bank = window.HINODE_LINE_PLATFORMS_V1 || {};
    return bank[systemKey] || {};
  }

  function pathDataForSystem(systemKey) {
    const bank = window.HINODE_LINE_PATH_V1 || {};
  ...
  function migrateStopId(definition, stop, index) {
    ...
    if (!stop.id || /^hinode-16-[\w-]+-\d{2}$/.test(stop.id)) {
      stop.id = newId;
      return true;
    }
    return false;
  ...
  function ensureRoute() {
    ...
    const systems = {};
    let changed = route.hinodeLineVersion !== VERSION;
    ...
    route.hinodeLineVersion = VERSION;
    ...
    if (!route.hinodeLineStopImages) {
      route.hinodeLineStopImages = {};
      changed = true;
    }
  ...
  function pathIntegrityFail(system, definition, reasons) {
    const policy = window.HINODE_LINE_PATH_POLICY_V1;
  ...
  async function resolveAllSystems(force = false, statusCallback = null) {
    const order = ['16-hinode-nanachome', '16-shinurayasu'];
  ...
  function migrateStopImageBank(route) {
    const bank = route.hinodeLineStopImages;
  ...
  function showStopImage(route, definition, stop, street) {
    ...
    const existing = street.querySelector('.stop-image-display-v25.hinodeLine-stop-image');
    existing?.remove();
    if (!entry?.dataUrl) return false;
    const overlay = document.createElement('div');
    overlay.className = 'stop-image-display-v25 hinodeLine-stop-image';
  ...
  function hideStopImage(street) {
    street?.querySelector('.hinodeLine-stop-image')?.remove();
  }
  ...
  async function drawGuidance(route, definition, system, token) {
    ...
    const publishDebugState = () => {
      window.HINODE_LINE_DRIVE_STATE = {
        ...simulation,
        ...
        try {
          streetViewService.getPanorama(request, (data, status) => {
            try {
              ...
              console.warn('[hinodeLine] panorama update failed', error);
            }
          });
        } catch (error) {
          console.warn('[hinodeLine] getPanorama failed', error);
          applyPovOnly();
        }
        return true;
      }
    ...
    const updateDrivingVisual = (now, force = false) => {
      ...
      if (!point?.position || !Number.isFinite(point.position.lat) || !Number.isFinite(point.position.lng)) {
        ...
        console.error('[hinodeLine] invalid drive position', point, simulation);
        return;
      }
    ...
    const failStart = (reason) => {
      ...
      console.error('[hinodeLine] drive start failed', reason, {
        systemKey: definition.key,
        pathLength: path.length,
        metricsTotal: metrics.total,
    ...
    cleanupGuidance = () => {
      ...
      telop.remove();
      hideStopImage(street);
      if (window.HINODE_LINE_DRIVE_STATE?.systemKey === definition.key) {
        window.HINODE_LINE_DRIVE_STATE = null;
      }
    };
  }

  routes = function routesHinodeLineV1() {
    const route = ensureRoute();
    ...
    try { window.HOKUEI_GUIDANCE_V22?.cleanup?.(); } catch (error) {
      console.warn('[hinodeLine] hokuei cleanup failed', error);
    }
    ...
    drawGuidance(route, definition, system, token).catch((error) => {
      ...
      console.error('[hinodeLine] drawGuidance failed', definition.key, error);
    });
  };
  ...
  function openRoute16StopEditor(systemKey, stopIndex) {
    closeEditorDialog();
    const route = ensureRoute();
    ...
    const currentImage = route.hinodeLineStopImages?.[bankKey]?.dataUrl || '';
    ...
    const saveCurrent = () => {
      ...
      if (!route.hinodeLineStopImages) route.hinodeLineStopImages = {};
      if (state.removeImage) delete route.hinodeLineStopImages[bankKey];
      else if (state.dataUrl) route.hinodeLineStopImages[bankKey] = { dataUrl: state.dataUrl, updatedAt: new Date().toISOString(), sourceSystem: systemKey, sourceStopId: stop.id };
      route.hinodeLineStopImageUpdatedAt = new Date().toISOString();
      save();
      return true;
    };
    ...
    document.getElementById('previousStopEdit').onclick = () => { if (!saveCurrent()) return; close(); openRoute16StopEditor(systemKey, stopIndex - 1); };
    document.getElementById('nextStopEdit').onclick = () => { if (!saveCurrent()) return; close(); openRoute16StopEditor(systemKey, stopIndex + 1); };
  ...
  function renderHinodeStopList() {
    ...
    box.querySelectorAll('[data-uhd-edit]').forEach((button) => { button.onclick = () => openRoute16StopEditor(definition.key, Number(button.dataset.uhdEdit)); });
  }

  function route16StopEditor() {
    ensureRoute();
    editorRouteId = ROUTE_ID;
  ...
  stopEditor = function stopEditorHinodeLineV1() {
