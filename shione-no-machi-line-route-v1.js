// 潮音の街線（系統番号15・route-15）走行シミュレーション／停留所編集モジュール。
// 停留所順：京成バスナビ個別便通過時刻表で確認（evidence/route15-shione-no-machi-implementation-2026-07-26/official-stop-orders.json）。
...
(() => {
  ...
  const SYSTEM_RESOLVED_VERSIONS = {
    '15-takasu-seaside': '2026-07-26-shione-no-machi-takasu-seaside-v1',
    '15-shinurayasu': '2026-07-26-shione-no-machi-shinurayasu-v1',
  };
  ...
  const SYSTEM_DEFINITIONS = {
    '15-takasu-seaside': {
      key: '15-takasu-seaside', displayCode: DISPLAY_CODE, directionGroup: 'outbound',
      title: '高洲海浜公園行き',
      summary: '新浦安駅 → 入船橋 → 東京学館前 → 高洲中央公園 → 潮音の街 → 高洲海浜公園',
      relationId: 18419865,
      timetableSymbol: '無印',
      naviCourse: '0008200278',
      names: NAMES_15_TAKASU_SEASIDE,
    },
    '15-shinurayasu': {
      key: '15-shinurayasu', displayCode: DISPLAY_CODE, directionGroup: 'inbound',
      title: '新浦安駅行き',
      summary: '高洲海浜公園 → 潮音の街 → 高洲中央公園 → 東京学館前 → 入船橋 → 新浦安駅',
      relationId: 18419864,
      timetableSymbol: '無印',
      naviCourse: '0008200277',
      names: NAMES_15_SHINURAYASU,
    },
  };
  ...
  function platformsForSystem(systemKey) {
    const bank = window.SHIONE_NO_MACHI_LINE_PLATFORMS_V1 || {};
    return bank[systemKey] || {};
  }

  function pathDataForSystem(systemKey) {
    const bank = window.SHIONE_NO_MACHI_LINE_PATH_V1 || {};
    return bank[systemKey] || null;
  }

  /** 系統キー単位の停留所IDへ揃える。 */
  function migrateStopId(definition, stop, index) {
    const newId = `shione-no-machi-${definition.key}-${String(index + 1).padStart(2, '0')}`;
  ...
  function makeStop(definition, name, index) {
    ...
    return {
      id: `shione-no-machi-${definition.key}-${String(index + 1).padStart(2, '0')}`,
      name,
  ...
  function ensureRoute() {
    ...
    const systems = {};
    let changed = route.shioneNoMachiLineVersion !== VERSION;
    ...
    route.shioneNoMachiLineVersion = VERSION;
    ...
    if (!route.shioneNoMachiLineStopImages) {
      route.shioneNoMachiLineStopImages = {};
      changed = true;
    }
  ...
  function pathIntegrityFail(system, definition, reasons) {
    const policy = window.SHIONE_NO_MACHI_LINE_PATH_POLICY_V1;
  ...
  async function resolveSystem(key, { force = false, statusCallback = null } = {}) {
    ...
    if (!route || !definition || !system) throw new Error('潮音の街線の系統データがありません。');
  ...
  function migrateStopImageBank(route) {
    const bank = route.shioneNoMachiLineStopImages;
  ...
  function showStopImage(route, definition, stop, street) {
    ...
    const existing = street.querySelector('.stop-image-display-v25.shioneNoMachiLine-stop-image');
    existing?.remove();
    if (!entry?.dataUrl) return false;
    const overlay = document.createElement('div');
    overlay.className = 'stop-image-display-v25 shioneNoMachiLine-stop-image';
  ...
  function hideStopImage(street) {
    street?.querySelector('.shioneNoMachiLine-stop-image')?.remove();
  }
  ...
  async function drawGuidance(route, definition, system, token) {
    ...
    try {
      ...
      const message = base.includes('潮音の街線の走行データを確認できません')
        ? base
        : [
          '潮音の街線の走行データを確認できません。',
          `系統：${definition.key}`,
    ...
    const vehicle = new googleApi.maps.Marker({ map, position: center, icon: busIcon(googleApi), zIndex: 1000, title: `潮音の街線 ${definition.title}` });
    ...
    const publishDebugState = () => {
      window.SHIONE_NO_MACHI_LINE_DRIVE_STATE = {
        ...simulation,
        ...
        try {
          streetViewService.getPanorama(request, (data, status) => {
            try {
              ...
              console.warn('[shioneNoMachiLine] panorama update failed', error);
            }
          });
        } catch (error) {
          console.warn('[shioneNoMachiLine] getPanorama failed', error);
          applyPovOnly();
        }
        return true;
      }
    ...
    const updateDrivingVisual = (now, force = false) => {
      ...
      if (!point?.position || !Number.isFinite(point.position.lat) || !Number.isFinite(point.position.lng)) {
        ...
        console.error('[shioneNoMachiLine] invalid drive position', point, simulation);
        return;
      }
    ...
    const failStart = (reason) => {
      ...
      console.error('[shioneNoMachiLine] drive start failed', reason, {
        systemKey: definition.key,
        pathLength: path.length,
        metricsTotal: metrics.total,
    ...
    status.textContent = `潮音の街線｜系統${DISPLAY_CODE}｜${definition.title}｜${stops.length}停留所｜道路形状：OSM relation採用（要走行確認）`;
    publishDebugState();
    cleanupGuidance = () => {
      simulation.running = false;
      simulation.paused = false;
      ...
      telop.remove();
      hideStopImage(street);
      if (window.SHIONE_NO_MACHI_LINE_DRIVE_STATE?.systemKey === definition.key) {
        window.SHIONE_NO_MACHI_LINE_DRIVE_STATE = null;
      }
    };
  }

  routes = function routesShioneNoMachiLineV1() {
    const route = ensureRoute();
    ...
    try { window.HOKUEI_GUIDANCE_V22?.cleanup?.(); } catch (error) {
      console.warn('[shioneNoMachiLine] hokuei cleanup failed', error);
    }
    ...
    shell(`<section class="guidance-page-v26">
      ...
      <div class="split guidance-v22-split">
        <div class="guidance-map-wrap-v22"><div id="routeMap" class="map guidance-map-v22"></div><div class="guidance-version-v22">潮音の街線・系統${DISPLAY_CODE}</div></div>
        <div id="street" class="street guidance-street-v22"></div>
      </div>
      <p id="mapStatus" class="status" hidden aria-hidden="true">潮音の街線を準備しています…</p>
    </section>`);
    ...
    drawGuidance(route, definition, system, token).catch((error) => {
      ...
      if (node) {
        ...
        node.textContent = message.includes('潮音の街線の走行データを確認できません') || message.includes('系統')
          ? message
          : (`系統=${key} | ${message}`);
      }
      console.error('[shioneNoMachiLine] drawGuidance failed', definition.key, error);
    });
  };
  ...
  function openRoute15StopEditor(systemKey, stopIndex) {
    closeEditorDialog();
    const route = ensureRoute();
    ...
    const currentImage = route.shioneNoMachiLineStopImages?.[bankKey]?.dataUrl || '';
    ...
    dialog.innerHTML = `<section class="stop-edit-dialog" role="dialog" aria-modal="true">
      <div class="stop-edit-header"><div><h2>潮音の街線 停留所設定</h2><p>${DISPLAY_CODE}｜${escHtml(definition.title)}・${stopIndex + 1} / ${system.stops.length}</p></div><button type="button" id="closeStopEditor" class="stop-edit-close">×</button></div>
    ...
    const saveCurrent = () => {
      ...
      if (!route.shioneNoMachiLineStopImages) route.shioneNoMachiLineStopImages = {};
      if (state.removeImage) delete route.shioneNoMachiLineStopImages[bankKey];
      else if (state.dataUrl) route.shioneNoMachiLineStopImages[bankKey] = { dataUrl: state.dataUrl, updatedAt: new Date().toISOString(), sourceSystem: systemKey, sourceStopId: stop.id };
      route.shioneNoMachiLineStopImageUpdatedAt = new Date().toISOString();
      save();
      return true;
    };
    ...
    document.getElementById('previousStopEdit').onclick = () => { if (!saveCurrent()) return; close(); openRoute15StopEditor(systemKey, stopIndex - 1); };
    document.getElementById('nextStopEdit').onclick = () => { if (!saveCurrent()) return; close(); openRoute15StopEditor(systemKey, stopIndex + 1); };
  ...
  function renderShioneNoMachiStopList() {
    const route = ensureRoute();
    ...
    box.querySelectorAll('[data-uhd-edit]').forEach((button) => { button.onclick = () => openRoute15StopEditor(definition.key, Number(button.dataset.uhdEdit)); });
  }

  function route15StopEditor() {
    ensureRoute();
    editorRouteId = ROUTE_ID;
  ...
  stopEditor = function stopEditorShioneNoMachiLineV1() {
