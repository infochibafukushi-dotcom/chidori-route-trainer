// シンボルロード線（系統番号11・route-11）走行シミュレーション／停留所編集モジュール。
// 停留所順：京成バスナビ通過時刻表で確認（evidence/route11-symbol-road-implementation-2026-07-25/official-stop-orders.json）。
// 記号なし。望海/明海五丁目経由は系統3。relation 18419852禁止。
// 停留所座標：OSM platform採用（往復で別platform）。OSM「日の出南（墓地公園）」→公式「日の出南」。
// 道路形状：OSM relation 18352884 / 18352883。Google Directionsは使用しない。
(() => {
  const ROUTE_ID = 'route-11';
  const VERSION = '2026-07-25-symbol-road-line-v1';
  const SYSTEM_RESOLVED_VERSIONS = {
    '11-urayasu-hinode': '2026-07-25-symbol-road-urayasu-hinode-v1',
    '11-urayasu-sogo-via-hinode-kominkan': '2026-07-25-symbol-road-urayasu-sogo-via-hinode-kominkan-v1',
    '11-urayasu-baypark': '2026-07-25-symbol-road-urayasu-baypark-v1',
    '11-urayasu-shinurayasu': '2026-07-25-symbol-road-urayasu-shinurayasu-v1',
    '11-shinurayasu-hinode': '2026-07-25-symbol-road-shinurayasu-hinode-v1',
    '11-shinurayasu-sogo': '2026-07-25-symbol-road-shinurayasu-sogo-v1',
    '11-shinurayasu-baypark': '2026-07-25-symbol-road-shinurayasu-baypark-v1',
    '11-hinode-urayasu': '2026-07-25-symbol-road-hinode-urayasu-v1',
    '11-hinode-shinurayasu': '2026-07-25-symbol-road-hinode-shinurayasu-v1',
    '11-sogo-shinurayasu': '2026-07-25-symbol-road-sogo-shinurayasu-v1',
    '11-sogo-urayasu': '2026-07-25-symbol-road-sogo-urayasu-v1',
  };
  const SYSTEM_KEY = 'chidori-symbol-road-line-system-v1';
  const DISPLAY_CODE = '11';
  const SPEED_KMH = 20;
  const DWELL_MS = 3000;
  const MAP_ZOOM = 18;
  ...
  const NAMES_URAYASU_SOGO_VIA_HINODE_KOMINKAN = [
      "浦安駅入口",
      "神明裏",
      "猫実",
      "消防本部前",
  ...
  const NAMES_URAYASU_SHINURAYASU = [
      "浦安駅入口",
      "神明裏",
      "猫実",
      "消防本部前",
  ...
  const NAMES_SHINURAYASU_HINODE = [
      "新浦安駅",
      "入船中央エステート",
      "明海大学前",
      "海風の街",
  ...
  const NAMES_SHINURAYASU_BAYPARK = [
      "新浦安駅",
      "入船中央エステート",
      "明海大学前",
      "海風の街",
  ...
  const NAMES_HINODE_SHINURAYASU = [
      "日の出南",
      "新浦安温泉",
      "総合公園",
      "ベイサイドホテルエリア",
  ...
  const SYSTEM_DEFINITIONS = {
    '11-urayasu-hinode': {
      key: '11-urayasu-hinode', displayCode: DISPLAY_CODE, directionGroup: 'outbound',
      title: "日の出南行き（浦安駅入口発）",
      summary: "浦安駅入口 → 新浦安駅 → 日の出公民館 → 日の出南",
      relationId: 18352884,
      names: NAMES_URAYASU_HINODE,
    },
    '11-urayasu-sogo-via-hinode-kominkan': {
      key: '11-urayasu-sogo-via-hinode-kominkan', displayCode: DISPLAY_CODE, directionGroup: 'outbound',
      title: "総合公園行き（浦安駅入口発）・日の出公民館経由",
      summary: "浦安駅入口 → 新浦安駅 → 日の出公民館 → 総合公園",
      relationId: 18352884,
      names: NAMES_URAYASU_SOGO_VIA_HINODE_KOMINKAN,
    },
    '11-urayasu-baypark': {
      key: '11-urayasu-baypark', displayCode: DISPLAY_CODE, directionGroup: 'outbound',
      title: "ベイパーク行き（浦安駅入口発）",
      summary: "浦安駅入口 → 新浦安駅 → 日の出公民館 → ベイパーク",
      relationId: 18352884,
      names: NAMES_URAYASU_BAYPARK,
    },
    '11-urayasu-shinurayasu': {
      key: '11-urayasu-shinurayasu', displayCode: DISPLAY_CODE, directionGroup: 'outbound',
      title: "新浦安駅行き（浦安駅入口発）",
      summary: "浦安駅入口 → 美浜東団地 → 新浦安駅",
      relationId: 18352884,
      names: NAMES_URAYASU_SHINURAYASU,
    },
    '11-shinurayasu-hinode': {
      key: '11-shinurayasu-hinode', displayCode: DISPLAY_CODE, directionGroup: 'outbound',
      title: "日の出南行き（新浦安駅発）",
      summary: "新浦安駅 → 日の出公民館 → 総合公園 → 日の出南",
      relationId: 18352884,
      names: NAMES_SHINURAYASU_HINODE,
    },
    '11-shinurayasu-sogo': {
      key: '11-shinurayasu-sogo', displayCode: DISPLAY_CODE, directionGroup: 'outbound',
      title: "総合公園行き（新浦安駅発）",
      summary: "新浦安駅 → 日の出公民館 → ベイパーク → 総合公園",
      relationId: 18352884,
      names: NAMES_SHINURAYASU_SOGO,
    },
    '11-shinurayasu-baypark': {
      key: '11-shinurayasu-baypark', displayCode: DISPLAY_CODE, directionGroup: 'outbound',
      title: "ベイパーク行き（新浦安駅発）",
      summary: "新浦安駅 → 日の出公民館 → ベイモール → ベイパーク",
      relationId: 18352884,
      names: NAMES_SHINURAYASU_BAYPARK,
    },
    '11-hinode-urayasu': {
      key: '11-hinode-urayasu', displayCode: DISPLAY_CODE, directionGroup: 'inbound',
      title: "浦安駅入口行き（日の出南発）",
      summary: "日の出南 → 日の出公民館 → 新浦安駅 → 浦安駅入口",
      relationId: 18352883,
      names: NAMES_HINODE_URAYASU,
    },
    '11-hinode-shinurayasu': {
      key: '11-hinode-shinurayasu', displayCode: DISPLAY_CODE, directionGroup: 'inbound',
      title: "新浦安駅行き（日の出南発）",
      summary: "日の出南 → 総合公園 → 日の出公民館 → 新浦安駅",
      relationId: 18352883,
      names: NAMES_HINODE_SHINURAYASU,
    },
    '11-sogo-shinurayasu': {
      key: '11-sogo-shinurayasu', displayCode: DISPLAY_CODE, directionGroup: 'inbound',
      title: "新浦安駅行き（総合公園発）",
      summary: "総合公園 → ベイパーク → 日の出公民館 → 新浦安駅",
      relationId: 18352883,
      names: NAMES_SOGO_SHINURAYASU,
    },
    '11-sogo-urayasu': {
      key: '11-sogo-urayasu', displayCode: DISPLAY_CODE, directionGroup: 'inbound',
      title: "浦安駅入口行き（総合公園発）",
      summary: "総合公園 → 日の出公民館 → 新浦安駅 → 浦安駅入口",
      relationId: 18352883,
      names: NAMES_SOGO_URAYASU,
    },
  };
  ...
  function platformsForSystem(systemKey) {
    const bank = window.SYMBOL_ROAD_LINE_PLATFORMS_V1 || {};
    return bank[systemKey] || {};
  }

  function pathDataForSystem(systemKey) {
    const bank = window.SYMBOL_ROAD_LINE_PATH_V1 || {};
    return bank[systemKey] || null;
  }

  /** 系統キー単位の停留所IDへ揃える。 */
  function migrateStopId(definition, stop, index) {
    const newId = `symbol-road-${definition.key}-${String(index + 1).padStart(2, '0')}`;
    if (stop.id === newId) return false;
    if (!stop.id || /^symbol-road-11-[\w-]+-\d{2}$/.test(stop.id)) {
      stop.id = newId;
      return true;
    }
    return false;
  ...
  function makeStop(definition, name, index) {
    ...
    return {
      id: `symbol-road-${definition.key}-${String(index + 1).padStart(2, '0')}`,
      name,
  ...
  function ensureRoute() {
    ...
    const systems = {};
    let changed = route.symbolRoadLineVersion !== VERSION;
    ...
    route.symbolRoadLineVersion = VERSION;
    ...
    if (!route.symbolRoadLineStopImages) {
      route.symbolRoadLineStopImages = {};
      changed = true;
    }
  ...
  function pathIntegrityFail(system, definition, reasons) {
    const policy = window.SYMBOL_ROAD_LINE_PATH_POLICY_V1;
  ...
  async function resolveSystem(key, { force = false, statusCallback = null } = {}) {
    ...
    if (!route || !definition || !system) throw new Error('シンボルロード線の系統データがありません。');
  ...
  function migrateStopImageBank(route) {
    const bank = route.symbolRoadLineStopImages;
  ...
  function showStopImage(route, definition, stop, street) {
    ...
    const existing = street.querySelector('.stop-image-display-v25.symbolRoadLine-stop-image');
    existing?.remove();
    if (!entry?.dataUrl) return false;
    const overlay = document.createElement('div');
    overlay.className = 'stop-image-display-v25 symbolRoadLine-stop-image';
  ...
  function hideStopImage(street) {
    street?.querySelector('.symbolRoadLine-stop-image')?.remove();
  }
  ...
  async function drawGuidance(route, definition, system, token) {
    ...
    try {
      ...
      const message = base.includes('シンボルロード線の走行データを確認できません')
        ? base
        : [
          'シンボルロード線の走行データを確認できません。',
          `系統：${definition.key}`,
    ...
    const vehicle = new googleApi.maps.Marker({ map, position: center, icon: busIcon(googleApi), zIndex: 1000, title: `シンボルロード線 ${definition.title}` });
        ...
        try {
          streetViewService.getPanorama(request, (data, status) => {
            try {
              ...
              console.warn('[symbolRoadLine] panorama update failed', error);
            }
          });
        } catch (error) {
          console.warn('[symbolRoadLine] getPanorama failed', error);
          applyPovOnly();
        }
        return true;
      }
    ...
    const updateDrivingVisual = (now, force = false) => {
      ...
      if (!point?.position || !Number.isFinite(point.position.lat) || !Number.isFinite(point.position.lng)) {
        ...
        console.error('[symbolRoadLine] invalid drive position', point, simulation);
        return;
      }
    ...
    const failStart = (reason) => {
      ...
      console.error('[symbolRoadLine] drive start failed', reason, {
        systemKey: definition.key,
        pathLength: path.length,
        metricsTotal: metrics.total,
    ...
    status.textContent = `シンボルロード線｜系統${DISPLAY_CODE}｜${definition.title}｜${stops.length}停留所｜道路形状：OSM relation採用（要走行確認）`;
    publishDebugState();
    cleanupGuidance = () => {
      simulation.running = false;
      simulation.paused = false;
      ...
      telop.remove();
      hideStopImage(street);
      if (window.SYMBOL_ROAD_LINE_DRIVE_STATE?.systemKey === definition.key) {
        window.SYMBOL_ROAD_LINE_DRIVE_STATE = null;
      }
    };
  }
  ...
  routes = function routesShiyakushoV1() {
    ...
    try { window.HOKUEI_GUIDANCE_V22?.cleanup?.(); } catch (error) {
      console.warn('[symbolRoadLine] hokuei cleanup failed', error);
    }
    ...
    shell(`<section class="guidance-page-v26">
      ...
      <div class="split guidance-v22-split">
        <div class="guidance-map-wrap-v22"><div id="routeMap" class="map guidance-map-v22"></div><div class="guidance-version-v22">シンボルロード線・系統${DISPLAY_CODE}</div></div>
        <div id="street" class="street guidance-street-v22"></div>
      </div>
      <p id="mapStatus" class="status" hidden aria-hidden="true">シンボルロード線を準備しています…</p>
    </section>`);
    ...
    drawGuidance(route, definition, system, token).catch((error) => {
      ...
      if (node) {
        ...
        node.textContent = message.includes('シンボルロード線の走行データを確認できません') || message.includes('系統')
          ? message
          : (`系統=${key} | ${message}`);
      }
      console.error('[symbolRoadLine] drawGuidance failed', definition.key, error);
    });
  };
  ...
  function openRoute6StopEditor(systemKey, stopIndex) {
    ...
    const currentImage = route.symbolRoadLineStopImages?.[bankKey]?.dataUrl || '';
    ...
    dialog.innerHTML = `<section class="stop-edit-dialog" role="dialog" aria-modal="true">
      <div class="stop-edit-header"><div><h2>シンボルロード線 停留所設定</h2><p>${DISPLAY_CODE}｜${escHtml(definition.title)}・${stopIndex + 1} / ${system.stops.length}</p></div><button type="button" id="closeStopEditor" class="stop-edit-close">×</button></div>
    ...
    const saveCurrent = () => {
      ...
      if (!route.symbolRoadLineStopImages) route.symbolRoadLineStopImages = {};
      if (state.removeImage) delete route.symbolRoadLineStopImages[bankKey];
      else if (state.dataUrl) route.symbolRoadLineStopImages[bankKey] = { dataUrl: state.dataUrl, updatedAt: new Date().toISOString(), sourceSystem: systemKey, sourceStopId: stop.id };
      route.symbolRoadLineStopImageUpdatedAt = new Date().toISOString();
      save();
      return true;
    };
  ...
  stopEditor = function stopEditorTakasuLineV1() {
  ...
  window.SYMBOL_ROAD_LINE_ROUTE_V1 = {
    VERSION,
