// 富士見循環線（系統番号24・route-24）走行シミュレーション／停留所編集モジュール。
// 停留所順：京成バスナビ個別便通過時刻表 course 0008200304 berth 24（evidence/route24-fujimi-loop-implementation-2026-07-27/official-stop-orders.json）。
...
(() => {
  ...
  const SYSTEM_RESOLVED_VERSIONS = {
    '24-fujimi-loop': '2026-07-27-fujimi24-fujimi-loop-v1',
  };
  ...
  const SYSTEM_DEFINITIONS = {
    '24-fujimi-loop': {
      key: '24-fujimi-loop', displayCode: DISPLAY_CODE, directionGroup: 'loop',
      title: '富士見循環（新浦安駅発着）',
      summary: '新浦安駅 → 富士見 → 見明川歩道橋 → 新浦安駅（1周）',
      relationId: null,
      timetableSymbol: '富士見',
      naviCourse: '0008200304',
      names: NAMES_24_FUJIMI_LOOP,
    },
  };

  const DEFAULT_SYSTEM_KEY = '24-fujimi-loop';
  ...
  function platformsForSystem(systemKey) {
    const bank = window.FUJIMI_LOOP_LINE_PLATFORMS_V1 || {};
  ...
  function pathDataForSystem(systemKey) {
    const bank = window.FUJIMI_LOOP_LINE_PATH_V1 || {};
    return bank[systemKey] || null;
  }

  /** 系統キー単位の停留所IDへ揃える。 */
  function migrateStopId(definition, stop, index) {
    const newId = `fujimi-24-${definition.key}-${String(index + 1).padStart(2, '0')}`;
    if (stop.id === newId) return false;
    if (!stop.id || /^fujimi-24-[\w-]+-\d{2}$/.test(stop.id)) {
      stop.id = newId;
      return true;
    }
    return false;
  ...
  function makeStop(definition, name, index) {
    ...
    return {
      id: `fujimi-24-${definition.key}-${String(index + 1).padStart(2, '0')}`,
      name,
  ...
  function ensureRoute() {
    ...
    const systems = {};
    let changed = route.fujimiLoopLineVersion !== VERSION;
    ...
    route.fujimiLoopLineVersion = VERSION;
    route.description = '富士見循環線：新浦安駅発着1周循環（24系統・course 0008200304）';
    route.sourcePolicy = '停留所順は京成バスナビ個別便通過時刻表（のりば24・凡例【２４系統】）。座標・道路は OSM relation 18323926 + highway Dijkstra。';
    if (!route.fujimiLoopLineStopImages) {
      route.fujimiLoopLineStopImages = {};
      changed = true;
    }
  ...
  function pathIntegrityFail(system, definition, reasons) {
    const policy = window.FUJIMI_LOOP_LINE_PATH_POLICY_V1;
  ...
  async function resolveSystem(key, { force = false, statusCallback = null } = {}) {
    ...
    if (!route || !definition || !system) throw new Error('富士見循環線の系統データがありません。');
  ...
  function migrateStopImageBank(route) {
    const bank = route.fujimiLoopLineStopImages;
  ...
  function showStopImage(route, definition, stop, street) {
    ...
    const existing = street.querySelector('.stop-image-display-v25.fujimiLoopLine-stop-image');
    existing?.remove();
    if (!entry?.dataUrl) return false;
    const overlay = document.createElement('div');
    overlay.className = 'stop-image-display-v25 fujimiLoopLine-stop-image';
  ...
  function hideStopImage(street) {
    street?.querySelector('.fujimiLoopLine-stop-image')?.remove();
  }
  ...
  async function drawGuidance(route, definition, system, token) {
    ...
    try {
      ...
      const message = base.includes('富士見循環線の走行データを確認できません')
        ? base
        : [
          '富士見循環線の走行データを確認できません。',
          `系統：${definition.key}`,
    ...
    const vehicle = new googleApi.maps.Marker({ map, position: center, icon: busIcon(googleApi), zIndex: 1000, title: `富士見循環線 ${definition.title}` });
    ...
    const publishDebugState = () => {
      window.FUJIMI_LOOP_LINE_DRIVE_STATE = {
        ...simulation,
        ...
        try {
          streetViewService.getPanorama(request, (data, status) => {
            try {
              ...
              console.warn('[fujimiLoopLine] panorama update failed', error);
            }
          });
        } catch (error) {
          console.warn('[fujimiLoopLine] getPanorama failed', error);
          applyPovOnly();
        }
        return true;
      }
    ...
    const updateDrivingVisual = (now, force = false) => {
      ...
      if (!point?.position || !Number.isFinite(point.position.lat) || !Number.isFinite(point.position.lng)) {
        ...
        console.error('[fujimiLoopLine] invalid drive position', point, simulation);
        return;
      }
    ...
    const failStart = (reason) => {
      ...
      console.error('[fujimiLoopLine] drive start failed', reason, {
        systemKey: definition.key,
        pathLength: path.length,
        metricsTotal: metrics.total,
    ...
    status.textContent = `富士見循環線｜系統${DISPLAY_CODE}｜${definition.title}｜${stops.length}停留所｜道路形状：OSM relation採用（要走行確認）`;
    publishDebugState();
    cleanupGuidance = () => {
      simulation.running = false;
      simulation.paused = false;
      ...
      telop.remove();
      hideStopImage(street);
      if (window.FUJIMI_LOOP_LINE_DRIVE_STATE?.systemKey === definition.key) {
        window.FUJIMI_LOOP_LINE_DRIVE_STATE = null;
      }
    };
  }

  routes = function routesFujimiLoopLineLineV1() {
    const route = ensureRoute();
    ...
    try { window.HOKUEI_GUIDANCE_V22?.cleanup?.(); } catch (error) {
      console.warn('[fujimiLoopLine] hokuei cleanup failed', error);
    }
    ...
    shell(`<section class="guidance-page-v26">
      ...
      <div class="split guidance-v22-split">
        <div class="guidance-map-wrap-v22"><div id="routeMap" class="map guidance-map-v22"></div><div class="guidance-version-v22">富士見循環線・系統${DISPLAY_CODE}</div></div>
        <div id="street" class="street guidance-street-v22"></div>
      </div>
      <p id="mapStatus" class="status" hidden aria-hidden="true">富士見循環線を準備しています…</p>
    </section>`);
    ...
    drawGuidance(route, definition, system, token).catch((error) => {
      ...
      if (node) {
        ...
        node.textContent = message.includes('富士見循環線の走行データを確認できません') || message.includes('系統')
          ? message
          : (`系統=${key} | ${message}`);
      }
      console.error('[fujimiLoopLine] drawGuidance failed', definition.key, error);
    });
  };
  ...
  function openRoute24StopEditor(systemKey, stopIndex) {
    closeEditorDialog();
    const route = ensureRoute();
    ...
    const currentImage = route.fujimiLoopLineStopImages?.[bankKey]?.dataUrl || '';
    ...
    dialog.innerHTML = `<section class="stop-edit-dialog" role="dialog" aria-modal="true">
      <div class="stop-edit-header"><div><h2>富士見循環線 停留所設定</h2><p>${DISPLAY_CODE}｜${escHtml(definition.title)}・${stopIndex + 1} / ${system.stops.length}</p></div><button type="button" id="closeStopEditor" class="stop-edit-close">×</button></div>
    ...
    const saveCurrent = () => {
      ...
      if (!route.fujimiLoopLineStopImages) route.fujimiLoopLineStopImages = {};
      if (state.removeImage) delete route.fujimiLoopLineStopImages[bankKey];
      else if (state.dataUrl) route.fujimiLoopLineStopImages[bankKey] = { dataUrl: state.dataUrl, updatedAt: new Date().toISOString(), sourceSystem: systemKey, sourceStopId: stop.id };
      route.fujimiLoopLineStopImageUpdatedAt = new Date().toISOString();
      save();
      return true;
    };
    ...
    document.getElementById('previousStopEdit').onclick = () => { if (!saveCurrent()) return; close(); openRoute24StopEditor(systemKey, stopIndex - 1); };
    document.getElementById('nextStopEdit').onclick = () => { if (!saveCurrent()) return; close(); openRoute24StopEditor(systemKey, stopIndex + 1); };
  ...
  function renderFujimiLoopLineStopList() {
    const route = ensureRoute();
    ...
    box.querySelectorAll('[data-uhd-edit]').forEach((button) => { button.onclick = () => openRoute24StopEditor(definition.key, Number(button.dataset.uhdEdit)); });
  }

  function route24StopEditor() {
    ensureRoute();
    editorRouteId = ROUTE_ID;
  ...
  stopEditor = function stopEditorFujimiLoopLineLineV1() {
  ...
  window.FUJIMI_LOOP_LINE_ROUTE_V1 = {
    VERSION,
