// 舞浜リゾート線（系統番号12・route-12）走行シミュレーション／停留所編集モジュール。
...
(() => {
  ...
  const NAMES_12_MAIHAMA_VIA_RESORT = [
    "浦安駅入口",
    "フラワー通り",
    "堀江三丁目",
    "南小入口",
  ...
  const NAMES_12_URAYASU_VIA_RESORT = [
    "舞浜駅",
    "リゾートホテルエリア・ノース",
    "リゾートホテルエリア・サウス",
    "「ベイサイド・ステーション」",
  ...
  function platformsForSystem(systemKey) {
    const bank = window.MAIHAMA_RESORT_LINE_PLATFORMS_V1 || {};
    return bank[systemKey] || {};
  }

  function pathDataForSystem(systemKey) {
    const bank = window.MAIHAMA_RESORT_LINE_PATH_V1 || {};
  ...
  function ensureRoute() {
    ...
    const systems = {};
    let changed = route.maihamaResortLineVersion !== VERSION;
    ...
    route.maihamaResortLineVersion = VERSION;
    ...
    if (!route.maihamaResortLineStopImages) {
      route.maihamaResortLineStopImages = {};
      changed = true;
    }
  ...
  function pathIntegrityFail(system, definition, reasons) {
    const policy = window.MAIHAMA_RESORT_LINE_PATH_POLICY_V1;
  ...
  async function resolveSystem(key, { force = false, statusCallback = null } = {}) {
    ...
    if (!route || !definition || !system) throw new Error('舞浜リゾート線の系統データがありません。');
  ...
  function migrateStopImageBank(route) {
    const bank = route.maihamaResortLineStopImages;
  ...
  function showStopImage(route, definition, stop, street) {
    ...
    const existing = street.querySelector('.stop-image-display-v25.maihamaResortLine-stop-image');
    existing?.remove();
    if (!entry?.dataUrl) return false;
    const overlay = document.createElement('div');
    overlay.className = 'stop-image-display-v25 maihamaResortLine-stop-image';
  ...
  function hideStopImage(street) {
    street?.querySelector('.maihamaResortLine-stop-image')?.remove();
  }
  ...
  async function drawGuidance(route, definition, system, token) {
    ...
    try {
      ...
      const message = base.includes('舞浜リゾート線の走行データを確認できません')
        ? base
        : [
          '舞浜リゾート線の走行データを確認できません。',
          `系統：${definition.key}`,
    ...
    const vehicle = new googleApi.maps.Marker({ map, position: center, icon: busIcon(googleApi), zIndex: 1000, title: `舞浜リゾート線 ${definition.title}` });
    ...
    const publishDebugState = () => {
      window.MAIHAMA_RESORT_LINE_DRIVE_STATE = {
        ...simulation,
        ...
        try {
          streetViewService.getPanorama(request, (data, status) => {
            try {
              ...
              console.warn('[maihamaResortLine] panorama update failed', error);
            }
          });
        } catch (error) {
          console.warn('[maihamaResortLine] getPanorama failed', error);
          applyPovOnly();
        }
        return true;
      }
    ...
    const updateDrivingVisual = (now, force = false) => {
      ...
      if (!point?.position || !Number.isFinite(point.position.lat) || !Number.isFinite(point.position.lng)) {
        ...
        console.error('[maihamaResortLine] invalid drive position', point, simulation);
        return;
      }
    ...
    const failStart = (reason) => {
      ...
      console.error('[maihamaResortLine] drive start failed', reason, {
        systemKey: definition.key,
        pathLength: path.length,
        metricsTotal: metrics.total,
    ...
    status.textContent = `舞浜リゾート線｜系統${DISPLAY_CODE}｜${definition.title}｜${stops.length}停留所｜道路形状：OSM relation採用（要走行確認）`;
    publishDebugState();
    cleanupGuidance = () => {
      simulation.running = false;
      simulation.paused = false;
      ...
      telop.remove();
      hideStopImage(street);
      if (window.MAIHAMA_RESORT_LINE_DRIVE_STATE?.systemKey === definition.key) {
        window.MAIHAMA_RESORT_LINE_DRIVE_STATE = null;
      }
    };
  }
  ...
  routes = function routesShiyakushoV1() {
    ...
    try { window.HOKUEI_GUIDANCE_V22?.cleanup?.(); } catch (error) {
      console.warn('[maihamaResortLine] hokuei cleanup failed', error);
    }
    ...
    shell(`<section class="guidance-page-v26">
      ...
      <div class="split guidance-v22-split">
        <div class="guidance-map-wrap-v22"><div id="routeMap" class="map guidance-map-v22"></div><div class="guidance-version-v22">舞浜リゾート線・系統${DISPLAY_CODE}</div></div>
        <div id="street" class="street guidance-street-v22"></div>
      </div>
      <p id="mapStatus" class="status" hidden aria-hidden="true">舞浜リゾート線を準備しています…</p>
    </section>`);
    ...
    drawGuidance(route, definition, system, token).catch((error) => {
      ...
      if (node) {
        ...
        node.textContent = message.includes('舞浜リゾート線の走行データを確認できません') || message.includes('系統')
          ? message
          : (`系統=${key} | ${message}`);
      }
      console.error('[maihamaResortLine] drawGuidance failed', definition.key, error);
    });
  };
  ...
  function openRoute6StopEditor(systemKey, stopIndex) {
    ...
    const currentImage = route.maihamaResortLineStopImages?.[bankKey]?.dataUrl || '';
    ...
    dialog.innerHTML = `<section class="stop-edit-dialog" role="dialog" aria-modal="true">
      <div class="stop-edit-header"><div><h2>舞浜リゾート線 停留所設定</h2><p>${DISPLAY_CODE}｜${escHtml(definition.title)}・${stopIndex + 1} / ${system.stops.length}</p></div><button type="button" id="closeStopEditor" class="stop-edit-close">×</button></div>
    ...
    const saveCurrent = () => {
      ...
      if (!route.maihamaResortLineStopImages) route.maihamaResortLineStopImages = {};
      if (state.removeImage) delete route.maihamaResortLineStopImages[bankKey];
      else if (state.dataUrl) route.maihamaResortLineStopImages[bankKey] = { dataUrl: state.dataUrl, updatedAt: new Date().toISOString(), sourceSystem: systemKey, sourceStopId: stop.id };
      route.maihamaResortLineStopImageUpdatedAt = new Date().toISOString();
      save();
      return true;
    };
  ...
  stopEditor = function stopEditorMaihamaResortLineV1() {
