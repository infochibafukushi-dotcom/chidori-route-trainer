// 明海クオン線（系統番号38・route-38）走行シミュレーション／停留所編集モジュール。
// 停留所順：京成バスナビ個別便通過時刻表 course 0008200316 berth 38（evidence/route38-akemi-quon-implementation-2026-07-27/official-stop-orders.json）。
// ★＜急行＞便。system.stops は乗車停留所のみ（海風の街 は path 通過のみで stops[] 除外）。
...
(() => {
  ...
  const SYSTEM_RESOLVED_VERSIONS = {
    '38-shinurayasu-quon-express': '2026-07-27-akemi38-shinurayasu-quon-express-v1',
  };
  const SYSTEM_KEY = 'akemi-quon-line-system-v1';
  const DISPLAY_CODE = '38';
  const INDEX_PLATFORM_SYSTEMS = new Set(["38-shinurayasu-quon-express"]);
  ...
  const MAX_DATA_URL_CHARS = 70000;
    ...
    const NAMES_38_SHINURAYASU_QUON_EXPRESS = [
    "新浦安駅",
    "明海小学校",
    "クオン新浦安",
    "新浦安駅"
  ...
  const SYSTEM_DEFINITIONS = {
    '38-shinurayasu-quon-express': {
      key: '38-shinurayasu-quon-express', displayCode: DISPLAY_CODE, directionGroup: 'loop-express',
      title: '明海小学校・クオン新浦安方面（新浦安駅発着・急行）',
      summary: '新浦安駅 → 明海小学校 → クオン新浦安 → 新浦安駅（海風の街通過）',
      relationId: 18396354,
      timetableSymbol: 'ク',
      naviCourse: '0008200316',
      names: NAMES_38_SHINURAYASU_QUON_EXPRESS,
    },
  };

  const DEFAULT_SYSTEM_KEY = '38-shinurayasu-quon-express';
  ...
  function platformsForSystem(systemKey) {
    const bank = window.AKEMI_QUON_LINE_PLATFORMS_V1 || {};
  ...
  function pathDataForSystem(systemKey) {
    const bank = window.AKEMI_QUON_LINE_PATH_V1 || {};
    return bank[systemKey] || null;
  }

  /** 系統キー単位の停留所IDへ揃える。 */
  function migrateStopId(definition, stop, index) {
    const newId = `akemi-38-${definition.key}-${String(index + 1).padStart(2, '0')}`;
    if (stop.id === newId) return false;
    if (!stop.id || /^akemi-38-[\w-]+-\d{2}$/.test(stop.id)) {
      stop.id = newId;
      return true;
    }
    return false;
  ...
  function makeStop(definition, name, index) {
    ...
    return {
      id: `akemi-38-${definition.key}-${String(index + 1).padStart(2, '0')}`,
      name,
  ...
  function ensureRoute() {
    ...
    const systems = {};
    let changed = route.akemiQuonLineVersion !== VERSION;
    ...
    route.akemiQuonLineVersion = VERSION;
    route.description = '明海クオン線：新浦安駅発着1周循環（38系統・course 0008200304）';
    route.sourcePolicy = '停留所順は京成バスナビ個別便通過時刻表（のりば24・凡例【３８系統】）。座標・道路は OSM relation 18323926 + highway Dijkstra。';
    if (!route.akemiQuonLineStopImages) {
      route.akemiQuonLineStopImages = {};
      changed = true;
    }
  ...
  function pathIntegrityFail(system, definition, reasons) {
    const policy = window.AKEMI_QUON_LINE_PATH_POLICY_V1;
  ...
  async function resolveSystem(key, { force = false, statusCallback = null } = {}) {
    ...
    if (!route || !definition || !system) throw new Error('明海クオン線の系統データがありません。');
  ...
  async function resolveAllSystems(force = false, statusCallback = null) {
    const order = ['38-shinurayasu-quon-express'];
  ...
  function migrateStopImageBank(route) {
    const bank = route.akemiQuonLineStopImages;
  ...
  function showStopImage(route, definition, stop, street) {
    ...
    const existing = street.querySelector('.stop-image-display-v25.akemiQuonLine-stop-image');
    existing?.remove();
    if (!entry?.dataUrl) return false;
    const overlay = document.createElement('div');
    overlay.className = 'stop-image-display-v25 akemiQuonLine-stop-image';
  ...
  function hideStopImage(street) {
    street?.querySelector('.akemiQuonLine-stop-image')?.remove();
  }
  ...
  async function drawGuidance(route, definition, system, token) {
    ...
    try {
      ...
      const message = base.includes('明海クオン線の走行データを確認できません')
        ? base
        : [
          '明海クオン線の走行データを確認できません。',
          `系統：${definition.key}`,
    ...
    const vehicle = new googleApi.maps.Marker({ map, position: center, icon: busIcon(googleApi), zIndex: 1000, title: `明海クオン線 ${definition.title}` });
    ...
    const publishDebugState = () => {
      window.AKEMI_QUON_LINE_DRIVE_STATE = {
        ...simulation,
        ...
        try {
          streetViewService.getPanorama(request, (data, status) => {
            try {
              ...
              console.warn('[akemiQuonLine] panorama update failed', error);
            }
          });
        } catch (error) {
          console.warn('[akemiQuonLine] getPanorama failed', error);
          applyPovOnly();
        }
        return true;
      }
    ...
    const updateDrivingVisual = (now, force = false) => {
      ...
      if (!point?.position || !Number.isFinite(point.position.lat) || !Number.isFinite(point.position.lng)) {
        ...
        console.error('[akemiQuonLine] invalid drive position', point, simulation);
        return;
      }
    ...
    const failStart = (reason) => {
      ...
      console.error('[akemiQuonLine] drive start failed', reason, {
        systemKey: definition.key,
        pathLength: path.length,
        metricsTotal: metrics.total,
    ...
    status.textContent = `明海クオン線｜系統${DISPLAY_CODE}｜${definition.title}｜${stops.length}停留所｜道路形状：OSM relation採用（要走行確認）`;
    publishDebugState();
    cleanupGuidance = () => {
      simulation.running = false;
      simulation.paused = false;
      ...
      telop.remove();
      hideStopImage(street);
      if (window.AKEMI_QUON_LINE_DRIVE_STATE?.systemKey === definition.key) {
        window.AKEMI_QUON_LINE_DRIVE_STATE = null;
      }
    };
  }

  routes = function routesAkemiQuonLineLineV1() {
    const route = ensureRoute();
    ...
    try { window.HOKUEI_GUIDANCE_V22?.cleanup?.(); } catch (error) {
      console.warn('[akemiQuonLine] hokuei cleanup failed', error);
    }
    ...
    shell(`<section class="guidance-page-v26">
      ...
      <div class="split guidance-v22-split">
        <div class="guidance-map-wrap-v22"><div id="routeMap" class="map guidance-map-v22"></div><div class="guidance-version-v22">明海クオン線・系統${DISPLAY_CODE}</div></div>
        <div id="street" class="street guidance-street-v22"></div>
      </div>
      <p id="mapStatus" class="status" hidden aria-hidden="true">明海クオン線を準備しています…</p>
    </section>`);
    ...
    drawGuidance(route, definition, system, token).catch((error) => {
      ...
      if (node) {
        ...
        node.textContent = message.includes('明海クオン線の走行データを確認できません') || message.includes('系統')
          ? message
          : (`系統=${key} | ${message}`);
      }
      console.error('[akemiQuonLine] drawGuidance failed', definition.key, error);
    });
  };
  ...
  function openRoute38StopEditor(systemKey, stopIndex) {
    closeEditorDialog();
    const route = ensureRoute();
    ...
    const currentImage = route.akemiQuonLineStopImages?.[bankKey]?.dataUrl || '';
    ...
    dialog.innerHTML = `<section class="stop-edit-dialog" role="dialog" aria-modal="true">
      <div class="stop-edit-header"><div><h2>明海クオン線 停留所設定</h2><p>${DISPLAY_CODE}｜${escHtml(definition.title)}・${stopIndex + 1} / ${system.stops.length}</p></div><button type="button" id="closeStopEditor" class="stop-edit-close">×</button></div>
    ...
    const saveCurrent = () => {
      ...
      if (!route.akemiQuonLineStopImages) route.akemiQuonLineStopImages = {};
      if (state.removeImage) delete route.akemiQuonLineStopImages[bankKey];
      else if (state.dataUrl) route.akemiQuonLineStopImages[bankKey] = { dataUrl: state.dataUrl, updatedAt: new Date().toISOString(), sourceSystem: systemKey, sourceStopId: stop.id };
      route.akemiQuonLineStopImageUpdatedAt = new Date().toISOString();
      save();
      return true;
    };
    ...
    document.getElementById('previousStopEdit').onclick = () => { if (!saveCurrent()) return; close(); openRoute38StopEditor(systemKey, stopIndex - 1); };
    document.getElementById('nextStopEdit').onclick = () => { if (!saveCurrent()) return; close(); openRoute38StopEditor(systemKey, stopIndex + 1); };
  ...
  function renderAkemiQuonLineStopList() {
    const route = ensureRoute();
    ...
    box.querySelectorAll('[data-uhd-edit]').forEach((button) => { button.onclick = () => openRoute38StopEditor(definition.key, Number(button.dataset.uhdEdit)); });
  }

  function route38StopEditor() {
    ensureRoute();
    editorRouteId = ROUTE_ID;
  ...
  stopEditor = function stopEditorAkemiQuonLineLineV1() {
  ...
  window.AKEMI_QUON_LINE_ROUTE_V1 = {
    VERSION,
