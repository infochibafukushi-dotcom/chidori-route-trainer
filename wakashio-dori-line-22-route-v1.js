// 若潮通り線（系統番号22・route-22 / 表示22千鳥東）走行シミュレーション／停留所編集モジュール。
// 停留所順：京成バスナビ個別便通過時刻表で確認（evidence/route22-wakashio-dori-implementation-2026-07-27/official-stop-orders.json）。
...
(() => {
  ...
  const SYSTEM_RESOLVED_VERSIONS = {
    '22-shinurayasu-chidori-garage': '2026-07-27-wakashio22-shinurayasu-chidori-garage-v1',
    '22-chidori-garage-shinurayasu': '2026-07-27-wakashio22-chidori-garage-shinurayasu-v1',
  };
  ...
  const NAMES_22_SHINURAYASU_CHIDORI_GARAGE = [
    "新浦安駅",
    "新浦安駅北口",
    "若潮公園",
    "順天堂病院前",
  ...
  const NAMES_22_CHIDORI_GARAGE_SHINURAYASU = [
    "千鳥車庫",
    "車庫裏",
    "千鳥東",
    "千鳥北",
  ...
  const SYSTEM_DEFINITIONS = {
    '22-shinurayasu-chidori-garage': {
      key: '22-shinurayasu-chidori-garage', displayCode: DISPLAY_CODE, directionGroup: 'outbound',
      title: '千鳥車庫行き（新浦安駅発・千鳥東経由）',
      summary: '新浦安駅 → 千鳥車庫',
      relationId: 18396547,
      timetableSymbol: '◎ち',
      naviCourse: '0008200301',
      names: NAMES_22_SHINURAYASU_CHIDORI_GARAGE,
    },
    '22-chidori-garage-shinurayasu': {
      key: '22-chidori-garage-shinurayasu', displayCode: DISPLAY_CODE, directionGroup: 'inbound',
      title: '新浦安駅行き（千鳥車庫発・順天堂病院前経由）',
      summary: '千鳥車庫 → 新浦安駅',
      relationId: 18396546,
      timetableSymbol: 'し',
      naviCourse: '0008200300',
      names: NAMES_22_CHIDORI_GARAGE_SHINURAYASU,
    },
  };

  const DEFAULT_SYSTEM_KEY = '22-shinurayasu-chidori-garage';
  ...
  function pathDataForSystem(systemKey) {
    const bank = window.WAKASHIO_DORI_LINE_22_PATH_V1 || {};
  ...
  function migrateStopId(definition, stop, index) {
    ...
    if (!stop.id || /^wakashio-22-[\w-]+-\d{2}$/.test(stop.id)) {
      stop.id = newId;
      return true;
    }
    return false;
  ...
  function makeStop(definition, name, index) {
    ...
    return {
      id: `wakashio-22-${definition.key}-${String(index + 1).padStart(2, '0')}`,
      name,
  ...
  function ensureRoute() {
    ...
    const systems = {};
    let changed = route.wakashioDoriLine22Version !== VERSION;
    ...
    route.wakashioDoriLine22Version = VERSION;
    ...
    if (!route.wakashioDoriLine22StopImages) {
      route.wakashioDoriLine22StopImages = {};
      changed = true;
    }
  ...
  function pathIntegrityFail(system, definition, reasons) {
    const policy = window.WAKASHIO_DORI_LINE_22_PATH_POLICY_V1;
  ...
  async function resolveSystem(key, { force = false, statusCallback = null } = {}) {
    ...
    if (!route || !definition || !system) throw new Error('若潮通り線の系統データがありません。');
  ...
  async function resolveAllSystems(force = false, statusCallback = null) {
    const order = ['22-shinurayasu-chidori-garage', '22-chidori-garage-shinurayasu'];
  ...
  function migrateStopImageBank(route) {
    const bank = route.wakashioDoriLine22StopImages;
  ...
  function showStopImage(route, definition, stop, street) {
    ...
    const existing = street.querySelector('.stop-image-display-v25.wakashioDoriLine22-stop-image');
    existing?.remove();
    if (!entry?.dataUrl) return false;
    const overlay = document.createElement('div');
    overlay.className = 'stop-image-display-v25 wakashioDoriLine22-stop-image';
  ...
  function hideStopImage(street) {
    street?.querySelector('.wakashioDoriLine22-stop-image')?.remove();
  }
  ...
  async function drawGuidance(route, definition, system, token) {
    ...
    try {
      ...
      const message = base.includes('若潮通り線の走行データを確認できません')
        ? base
        : [
          '若潮通り線の走行データを確認できません。',
          `系統：${definition.key}`,
    ...
    const vehicle = new googleApi.maps.Marker({ map, position: center, icon: busIcon(googleApi), zIndex: 1000, title: `若潮通り線 ${definition.title}` });
    ...
    const publishDebugState = () => {
      window.WAKASHIO_DORI_LINE_22_DRIVE_STATE = {
        ...simulation,
        ...
        try {
          streetViewService.getPanorama(request, (data, status) => {
            try {
              ...
              console.warn('[wakashioDoriLine22] panorama update failed', error);
            }
          });
        } catch (error) {
          console.warn('[wakashioDoriLine22] getPanorama failed', error);
          applyPovOnly();
        }
        return true;
      }
    ...
    const updateDrivingVisual = (now, force = false) => {
      ...
      if (!point?.position || !Number.isFinite(point.position.lat) || !Number.isFinite(point.position.lng)) {
        ...
        console.error('[wakashioDoriLine22] invalid drive position', point, simulation);
        return;
      }
    ...
    const failStart = (reason) => {
      ...
      console.error('[wakashioDoriLine22] drive start failed', reason, {
        systemKey: definition.key,
        pathLength: path.length,
        metricsTotal: metrics.total,
    ...
    status.textContent = `若潮通り線｜系統${DISPLAY_CODE}｜${definition.title}｜${stops.length}停留所｜道路形状：OSM relation採用（要走行確認）`;
    publishDebugState();
    cleanupGuidance = () => {
      simulation.running = false;
      simulation.paused = false;
      ...
      telop.remove();
      hideStopImage(street);
      if (window.WAKASHIO_DORI_LINE_22_DRIVE_STATE?.systemKey === definition.key) {
        window.WAKASHIO_DORI_LINE_22_DRIVE_STATE = null;
      }
    };
  }

  routes = function routesWakashioDoriLine22LineV1() {
    const route = ensureRoute();
    ...
    try { window.HOKUEI_GUIDANCE_V22?.cleanup?.(); } catch (error) {
      console.warn('[wakashioDoriLine22] hokuei cleanup failed', error);
    }
    ...
    shell(`<section class="guidance-page-v26">
      ...
      <div class="split guidance-v22-split">
        <div class="guidance-map-wrap-v22"><div id="routeMap" class="map guidance-map-v22"></div><div class="guidance-version-v22">若潮通り線・系統${DISPLAY_CODE}</div></div>
        <div id="street" class="street guidance-street-v22"></div>
      </div>
      <p id="mapStatus" class="status" hidden aria-hidden="true">若潮通り線を準備しています…</p>
    </section>`);
    ...
    drawGuidance(route, definition, system, token).catch((error) => {
      ...
      if (node) {
        ...
        node.textContent = message.includes('若潮通り線の走行データを確認できません') || message.includes('系統')
          ? message
          : (`系統=${key} | ${message}`);
      }
      console.error('[wakashioDoriLine22] drawGuidance failed', definition.key, error);
    });
  };
  ...
  function openRoute22StopEditor(systemKey, stopIndex) {
    closeEditorDialog();
    const route = ensureRoute();
    ...
    const currentImage = route.wakashioDoriLine22StopImages?.[bankKey]?.dataUrl || '';
    ...
    dialog.innerHTML = `<section class="stop-edit-dialog" role="dialog" aria-modal="true">
      <div class="stop-edit-header"><div><h2>若潮通り線 停留所設定</h2><p>${DISPLAY_CODE}｜${escHtml(definition.title)}・${stopIndex + 1} / ${system.stops.length}</p></div><button type="button" id="closeStopEditor" class="stop-edit-close">×</button></div>
    ...
    const saveCurrent = () => {
      ...
      if (!route.wakashioDoriLine22StopImages) route.wakashioDoriLine22StopImages = {};
      if (state.removeImage) delete route.wakashioDoriLine22StopImages[bankKey];
      else if (state.dataUrl) route.wakashioDoriLine22StopImages[bankKey] = { dataUrl: state.dataUrl, updatedAt: new Date().toISOString(), sourceSystem: systemKey, sourceStopId: stop.id };
      route.wakashioDoriLine22StopImageUpdatedAt = new Date().toISOString();
      save();
      return true;
    };
    ...
    document.getElementById('previousStopEdit').onclick = () => { if (!saveCurrent()) return; close(); openRoute22StopEditor(systemKey, stopIndex - 1); };
    document.getElementById('nextStopEdit').onclick = () => { if (!saveCurrent()) return; close(); openRoute22StopEditor(systemKey, stopIndex + 1); };
  ...
  function renderWakashioDoriLine22StopList() {
    const route = ensureRoute();
    ...
    box.querySelectorAll('[data-uhd-edit]').forEach((button) => { button.onclick = () => openRoute22StopEditor(definition.key, Number(button.dataset.uhdEdit)); });
  }

  function route22StopEditor() {
    ensureRoute();
    editorRouteId = ROUTE_ID;
    ...
    renderWakashioDoriLine22StopList();
  ...
  stopEditor = function stopEditorWakashioDoriLine22LineV1() {
  ...
  window.WAKASHIO_DORI_LINE_22_ROUTE_V1 = {
    VERSION,
