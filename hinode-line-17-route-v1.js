// 日の出線（系統番号17・route-17）走行シミュレーション／停留所編集モジュール。
// 停留所順：京成バスナビ個別便通過時刻表で確認（evidence/route17-hinode-implementation-2026-07-26/official-stop-orders.json）。
...
(() => {
  ...
  const SYSTEM_RESOLVED_VERSIONS = {
    '17-hinode-nanachome': '2026-07-26-hinode17-nanachome-v1',
    '17-baycity-urayasu': '2026-07-26-hinode17-baycity-v1',
    '17-shinurayasu': '2026-07-26-hinode17-shinurayasu-v1',
  };
  ...
  const NAMES_17_HINODE_NANACHOME = [
    "新浦安駅",
    "入船中央エステート",
    "明海大学前",
    "日の出保育園入口",
  ...
  const NAMES_17_BAYCITY_URAYASU = [
    "新浦安駅",
    "入船中央エステート",
    "明海大学前",
    "日の出保育園入口",
  ...
  const SYSTEM_DEFINITIONS = {
    '17-hinode-nanachome': {
      key: '17-hinode-nanachome', displayCode: DISPLAY_CODE, directionGroup: 'outbound',
      title: '日の出七丁目行き',
      summary: '新浦安駅 → 東京電力 → 日の出東 → 順天堂大学・日の出 東口 → 日の出七丁目',
      relationId: 18396569,
      timetableSymbol: '(無印)',
      naviCourse: '0008200282',
      names: NAMES_17_HINODE_NANACHOME,
    },
    '17-baycity-urayasu': {
      key: '17-baycity-urayasu', displayCode: DISPLAY_CODE, directionGroup: 'outbound',
      title: 'ベイシティ浦安行き',
      summary: '新浦安駅 → 東京電力 → 日の出東 → 順天堂大学・日の出 東口 → 日の出西 → 順天堂大学・日の出 正門 → プラウド新浦安パークマリーナ → ベイシティ浦安',
      relationId: 18396583,
      timetableSymbol: 'ベ',
      naviCourse: '0008200283',
      names: NAMES_17_BAYCITY_URAYASU,
    },
    '17-shinurayasu': {
      key: '17-shinurayasu', displayCode: DISPLAY_CODE, directionGroup: 'inbound',
      title: '新浦安駅行き',
      summary: '日の出七丁目 → 順天堂大学・日の出 東口 → 日の出東 → 東京電力 → 新浦安駅',
      relationId: 18396568,
      timetableSymbol: '(無印)',
      naviCourse: '0008200281',
      names: NAMES_17_SHINURAYASU,
    },
  };
  ...
  function platformsForSystem(systemKey) {
    const bank = window.HINODE_LINE_17_PLATFORMS_V1 || {};
    return bank[systemKey] || {};
  }

  function pathDataForSystem(systemKey) {
    const bank = window.HINODE_LINE_17_PATH_V1 || {};
    return bank[systemKey] || null;
  }

  /** 系統キー単位の停留所IDへ揃える。 */
  function migrateStopId(definition, stop, index) {
    const newId = `hinode-${definition.key}-${String(index + 1).padStart(2, '0')}`;
    if (stop.id === newId) return false;
    if (!stop.id || /^hinode-17-[\w-]+-\d{2}$/.test(stop.id)) {
      stop.id = newId;
      return true;
    }
    return false;
  ...
  function makeStop(definition, name, index) {
    ...
    return {
      id: `hinode-${definition.key}-${String(index + 1).padStart(2, '0')}`,
      name,
  ...
  function ensureRoute() {
    ...
    const systems = {};
    let changed = route.hinode17LineVersion !== VERSION;
    ...
    route.hinode17LineVersion = VERSION;
    ...
    if (!route.hinode17LineStopImages) {
      route.hinode17LineStopImages = {};
      changed = true;
    }
  ...
  async function resolveSystem(key, { force = false, statusCallback = null } = {}) {
    ...
    if (!route || !definition || !system) throw new Error('日の出線の系統データがありません。');
  ...
  async function resolveAllSystems(force = false, statusCallback = null) {
    const order = ['17-hinode-nanachome', '17-baycity-urayasu', '17-shinurayasu'];
  ...
  function migrateStopImageBank(route) {
    const bank = route.hinode17LineStopImages;
  ...
  function showStopImage(route, definition, stop, street) {
    ...
    const existing = street.querySelector('.stop-image-display-v25.hinode17Line-stop-image');
    existing?.remove();
    if (!entry?.dataUrl) return false;
    const overlay = document.createElement('div');
    overlay.className = 'stop-image-display-v25 hinode17Line-stop-image';
  ...
  function hideStopImage(street) {
    street?.querySelector('.hinode17Line-stop-image')?.remove();
  }
  ...
  async function drawGuidance(route, definition, system, token) {
    ...
    try {
      ...
      const message = base.includes('日の出線の走行データを確認できません')
        ? base
        : [
          '日の出線の走行データを確認できません。',
          `系統：${definition.key}`,
    ...
    const vehicle = new googleApi.maps.Marker({ map, position: center, icon: busIcon(googleApi), zIndex: 1000, title: `日の出線 ${definition.title}` });
    ...
    const publishDebugState = () => {
      window.HINODE_LINE_17_DRIVE_STATE = {
        ...simulation,
        ...
        try {
          streetViewService.getPanorama(request, (data, status) => {
            try {
              ...
              console.warn('[hinode17Line] panorama update failed', error);
            }
          });
        } catch (error) {
          console.warn('[hinode17Line] getPanorama failed', error);
          applyPovOnly();
        }
        return true;
      }
    ...
    const updateDrivingVisual = (now, force = false) => {
      ...
      if (!point?.position || !Number.isFinite(point.position.lat) || !Number.isFinite(point.position.lng)) {
        ...
        console.error('[hinode17Line] invalid drive position', point, simulation);
        return;
      }
    ...
    const failStart = (reason) => {
      ...
      console.error('[hinode17Line] drive start failed', reason, {
        systemKey: definition.key,
        pathLength: path.length,
        metricsTotal: metrics.total,
    ...
    status.textContent = `日の出線｜系統${DISPLAY_CODE}｜${definition.title}｜${stops.length}停留所｜道路形状：OSM relation採用（要走行確認）`;
    publishDebugState();
    cleanupGuidance = () => {
      simulation.running = false;
      simulation.paused = false;
      ...
      telop.remove();
      hideStopImage(street);
      if (window.HINODE_LINE_17_DRIVE_STATE?.systemKey === definition.key) {
        window.HINODE_LINE_17_DRIVE_STATE = null;
      }
    };
  }

  routes = function routesHinode17LineV1() {
    const route = ensureRoute();
    ...
    try { window.HOKUEI_GUIDANCE_V22?.cleanup?.(); } catch (error) {
      console.warn('[hinode17Line] hokuei cleanup failed', error);
    }
    ...
    shell(`<section class="guidance-page-v26">
      ...
      <div class="split guidance-v22-split">
        <div class="guidance-map-wrap-v22"><div id="routeMap" class="map guidance-map-v22"></div><div class="guidance-version-v22">日の出線・系統${DISPLAY_CODE}</div></div>
        <div id="street" class="street guidance-street-v22"></div>
      </div>
      <p id="mapStatus" class="status" hidden aria-hidden="true">日の出線を準備しています…</p>
    </section>`);
    ...
    drawGuidance(route, definition, system, token).catch((error) => {
      ...
      if (node) {
        ...
        node.textContent = message.includes('日の出線の走行データを確認できません') || message.includes('系統')
          ? message
          : (`系統=${key} | ${message}`);
      }
      console.error('[hinode17Line] drawGuidance failed', definition.key, error);
    });
  };
  ...
  function openRoute17StopEditor(systemKey, stopIndex) {
    closeEditorDialog();
    const route = ensureRoute();
    ...
    const currentImage = route.hinode17LineStopImages?.[bankKey]?.dataUrl || '';
    ...
    dialog.innerHTML = `<section class="stop-edit-dialog" role="dialog" aria-modal="true">
      <div class="stop-edit-header"><div><h2>日の出線 停留所設定</h2><p>${DISPLAY_CODE}｜${escHtml(definition.title)}・${stopIndex + 1} / ${system.stops.length}</p></div><button type="button" id="closeStopEditor" class="stop-edit-close">×</button></div>
    ...
    const saveCurrent = () => {
      ...
      if (!route.hinode17LineStopImages) route.hinode17LineStopImages = {};
      if (state.removeImage) delete route.hinode17LineStopImages[bankKey];
      else if (state.dataUrl) route.hinode17LineStopImages[bankKey] = { dataUrl: state.dataUrl, updatedAt: new Date().toISOString(), sourceSystem: systemKey, sourceStopId: stop.id };
      route.hinode17LineStopImageUpdatedAt = new Date().toISOString();
      save();
      return true;
    };
    ...
    document.getElementById('previousStopEdit').onclick = () => { if (!saveCurrent()) return; close(); openRoute17StopEditor(systemKey, stopIndex - 1); };
    document.getElementById('nextStopEdit').onclick = () => { if (!saveCurrent()) return; close(); openRoute17StopEditor(systemKey, stopIndex + 1); };
  ...
  function renderHinode17StopList() {
    const route = ensureRoute();
    ...
    box.querySelectorAll('[data-uhd-edit]').forEach((button) => { button.onclick = () => openRoute17StopEditor(definition.key, Number(button.dataset.uhdEdit)); });
  }

  function route17StopEditor() {
    ensureRoute();
    editorRouteId = ROUTE_ID;
  ...
  stopEditor = function stopEditorHinode17LineV1() {
