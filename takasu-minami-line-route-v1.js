// 高洲南線（系統番号19・route-19）走行シミュレーション／停留所編集モジュール。
// 停留所順：京成バスナビ個別便通過時刻表で確認（evidence/route19-takasu-minami-implementation-2026-07-26/official-stop-orders.json）。
...
(() => {
  ...
  const SYSTEM_RESOLVED_VERSIONS = {
    '19-takasu-seaside': '2026-07-26-takasuminami19-takasu-seaside-v1',
    '19-shinurayasu': '2026-07-26-takasuminami19-shinurayasu-v1',
  };
  ...
  const SYSTEM_DEFINITIONS = {
    '19-takasu-seaside': {
      key: '19-takasu-seaside', displayCode: DISPLAY_CODE, directionGroup: 'outbound',
      title: '高洲海浜公園行き',
      summary: '新浦安駅 → 東京学館前 → 高洲四丁目 → 浦安南高校特養ホーム → 高洲海浜公園',
      relationId: 18381771,
      timetableSymbol: '無印',
      naviCourse: '0008200292',
      names: NAMES_19_TAKASU_SEASIDE,
    },
    '19-shinurayasu': {
      key: '19-shinurayasu', displayCode: DISPLAY_CODE, directionGroup: 'inbound',
      title: '新浦安駅行き',
      summary: '高洲海浜公園 → 浦安南高校特養ホーム → 高洲四丁目 → 東京学館前 → 新浦安駅',
      relationId: 18381770,
      timetableSymbol: '無印',
      naviCourse: '0008200291',
      names: NAMES_19_SHINURAYASU,
    },
  };

  const DEFAULT_SYSTEM_KEY = '19-takasu-seaside';
  ...
  function platformsForSystem(systemKey) {
    const bank = window.TAKASU_MINAMI_LINE_PLATFORMS_V1 || {};
    return bank[systemKey] || {};
  }

  function pathDataForSystem(systemKey) {
    const bank = window.TAKASU_MINAMI_LINE_PATH_V1 || {};
    return bank[systemKey] || null;
  }

  /** 系統キー単位の停留所IDへ揃える。 */
  function migrateStopId(definition, stop, index) {
    const newId = `takasu-minami-${definition.key}-${String(index + 1).padStart(2, '0')}`;
    if (stop.id === newId) return false;
    if (!stop.id || /^takasu-minami-19-[\w-]+-\d{2}$/.test(stop.id)) {
      stop.id = newId;
      return true;
    }
    return false;
  ...
  function makeStop(definition, name, index) {
    ...
    return {
      id: `takasu-minami-${definition.key}-${String(index + 1).padStart(2, '0')}`,
      name,
  ...
  function ensureRoute() {
    ...
    const systems = {};
    let changed = route.takasuMinamiLineVersion !== VERSION;
    ...
    route.takasuMinamiLineVersion = VERSION;
    ...
    if (!route.takasuMinamiLineStopImages) {
      route.takasuMinamiLineStopImages = {};
      changed = true;
    }
  ...
  async function resolveSystem(key, { force = false, statusCallback = null } = {}) {
    ...
    if (!route || !definition || !system) throw new Error('高洲南線の系統データがありません。');
  ...
  function migrateStopImageBank(route) {
    const bank = route.takasuMinamiLineStopImages;
  ...
  function showStopImage(route, definition, stop, street) {
    ...
    const existing = street.querySelector('.stop-image-display-v25.takasuMinamiLine-stop-image');
    existing?.remove();
    if (!entry?.dataUrl) return false;
    const overlay = document.createElement('div');
    overlay.className = 'stop-image-display-v25 takasuMinamiLine-stop-image';
  ...
  function hideStopImage(street) {
    street?.querySelector('.takasuMinamiLine-stop-image')?.remove();
  }
  ...
  async function drawGuidance(route, definition, system, token) {
    ...
    try {
      ...
      const message = base.includes('高洲南線の走行データを確認できません')
        ? base
        : [
          '高洲南線の走行データを確認できません。',
          `系統：${definition.key}`,
    ...
    const vehicle = new googleApi.maps.Marker({ map, position: center, icon: busIcon(googleApi), zIndex: 1000, title: `高洲南線 ${definition.title}` });
    ...
    const publishDebugState = () => {
      window.TAKASU_MINAMI_LINE_DRIVE_STATE = {
        ...simulation,
        ...
        try {
          streetViewService.getPanorama(request, (data, status) => {
            try {
              ...
              console.warn('[takasuMinamiLine] panorama update failed', error);
            }
          });
        } catch (error) {
          console.warn('[takasuMinamiLine] getPanorama failed', error);
          applyPovOnly();
        }
        return true;
      }
    ...
    const updateDrivingVisual = (now, force = false) => {
      ...
      if (!point?.position || !Number.isFinite(point.position.lat) || !Number.isFinite(point.position.lng)) {
        ...
        console.error('[takasuMinamiLine] invalid drive position', point, simulation);
        return;
      }
    ...
    const failStart = (reason) => {
      ...
      console.error('[takasuMinamiLine] drive start failed', reason, {
        systemKey: definition.key,
        pathLength: path.length,
        metricsTotal: metrics.total,
    ...
    status.textContent = `高洲南線｜系統${DISPLAY_CODE}｜${definition.title}｜${stops.length}停留所｜道路形状：OSM relation採用（要走行確認）`;
    publishDebugState();
    cleanupGuidance = () => {
      simulation.running = false;
      simulation.paused = false;
      ...
      telop.remove();
      hideStopImage(street);
      if (window.TAKASU_MINAMI_LINE_DRIVE_STATE?.systemKey === definition.key) {
        window.TAKASU_MINAMI_LINE_DRIVE_STATE = null;
      }
    };
  }

  routes = function routesTakasuMinamiLineV1() {
    const route = ensureRoute();
    ...
    try { window.HOKUEI_GUIDANCE_V22?.cleanup?.(); } catch (error) {
      console.warn('[takasuMinamiLine] hokuei cleanup failed', error);
    }
    ...
    shell(`<section class="guidance-page-v26">
      ...
      <div class="split guidance-v22-split">
        <div class="guidance-map-wrap-v22"><div id="routeMap" class="map guidance-map-v22"></div><div class="guidance-version-v22">高洲南線・系統${DISPLAY_CODE}</div></div>
        <div id="street" class="street guidance-street-v22"></div>
      </div>
      <p id="mapStatus" class="status" hidden aria-hidden="true">高洲南線を準備しています…</p>
    </section>`);
    ...
    drawGuidance(route, definition, system, token).catch((error) => {
      ...
      if (node) {
        ...
        node.textContent = message.includes('高洲南線の走行データを確認できません') || message.includes('系統')
          ? message
          : (`系統=${key} | ${message}`);
      }
      console.error('[takasuMinamiLine] drawGuidance failed', definition.key, error);
    });
  };
  ...
  function openRoute19StopEditor(systemKey, stopIndex) {
    closeEditorDialog();
    const route = ensureRoute();
    ...
    const currentImage = route.takasuMinamiLineStopImages?.[bankKey]?.dataUrl || '';
    ...
    dialog.innerHTML = `<section class="stop-edit-dialog" role="dialog" aria-modal="true">
      <div class="stop-edit-header"><div><h2>高洲南線 停留所設定</h2><p>${DISPLAY_CODE}｜${escHtml(definition.title)}・${stopIndex + 1} / ${system.stops.length}</p></div><button type="button" id="closeStopEditor" class="stop-edit-close">×</button></div>
    ...
    const saveCurrent = () => {
      ...
      if (!route.takasuMinamiLineStopImages) route.takasuMinamiLineStopImages = {};
      if (state.removeImage) delete route.takasuMinamiLineStopImages[bankKey];
      else if (state.dataUrl) route.takasuMinamiLineStopImages[bankKey] = { dataUrl: state.dataUrl, updatedAt: new Date().toISOString(), sourceSystem: systemKey, sourceStopId: stop.id };
      route.takasuMinamiLineStopImageUpdatedAt = new Date().toISOString();
      save();
      return true;
    };
    ...
    document.getElementById('previousStopEdit').onclick = () => { if (!saveCurrent()) return; close(); openRoute19StopEditor(systemKey, stopIndex - 1); };
    document.getElementById('nextStopEdit').onclick = () => { if (!saveCurrent()) return; close(); openRoute19StopEditor(systemKey, stopIndex + 1); };
  ...
  function renderTakasuMinamiStopList() {
    const route = ensureRoute();
    ...
    box.querySelectorAll('[data-uhd-edit]').forEach((button) => { button.onclick = () => openRoute19StopEditor(definition.key, Number(button.dataset.uhdEdit)); });
  }

  function route19StopEditor() {
    ensureRoute();
    editorRouteId = ROUTE_ID;
  ...
  stopEditor = function stopEditorTakasuMinamiLineV1() {
  ...
  window.TAKASU_MINAMI_LINE_ROUTE_V1 = {
    VERSION,
